...
static void cyrus_sasl_process_init(pn_transport_t *transport,
                                    const char *mechanism,
                                    const pn_bytes_t *recv);
static void cyrus_sasl_process_response(pn_transport_t *transport,
                                        const pn_bytes_t *recv);

static bool cyrus_sasl_init_client(pn_transport_t *transport);
static bool cyrus_sasl_process_mechanisms(pn_transport_t *transport,
                                          const char *mechs);
static void cyrus_sasl_process_challenge(pn_transport_t *transport,
                                         const pn_bytes_t *recv);
...
static const pnx_sasl_implementation sasl_impl = {cyrus_sasl_free,
...
// sasl_client_done()/sasl_server_done() don't do any global clean up as it's
// not safe to use just sasl_done() for an executable that uses both client and
// server parts of Cyrus SASL, because it can't be called twice.
...
static inline bool pni_check_result(sasl_conn_t *conn, int r,
                                    pn_transport_t *logger,
                                    const char *condition_name) {
    if (r == SASL_OK) return true;

    const char *err =
        conn ? sasl_errdetail(conn) : sasl_errstring(r, NULL, NULL);
    pnx_sasl_error(logger, err, condition_name);
    return false;
}

static bool pni_check_io_result(sasl_conn_t *conn, int r,
                                pn_transport_t *logger) {
    return pni_check_result(conn, r, logger, "proton:io:sasl_error");
}

static bool pni_check_sasl_result(sasl_conn_t *conn, int r,
                                  pn_transport_t *logger) {
    return pni_check_result(conn, r, logger, "amqp:unauthorized-access");
}

// Cyrus wrappers
static void pni_cyrus_interact(pn_transport_t *transport,
...
// environment Relies on pthreads.
...
bool pn_sasl_extended(void) { return true; }
...
__attribute__((destructor)) static void pni_cyrus_finish(void) {
...
static void pni_cyrus_server_once(void) {
    ...
    if (result == SASL_OK) {
        result =
            sasl_server_init(NULL, pni_cyrus_config_name ? pni_cyrus_config_name
                                                         : default_config_name);
    }
    ...
    do {
        ...
        const sasl_callback_t *callbacks =
            pnx_sasl_get_username(transport) ? pnx_sasl_get_password(transport)
                                                   ? pni_user_password_callbacks
                                                   : pni_user_callbacks
                                             : NULL;
        result =
            sasl_client_new(amqp_service, pnx_sasl_get_remote_fqdn(transport),
                            NULL, NULL, callbacks, 0, &cyrus_conn);
        if (result != SASL_OK) break;
...
static int pni_wrap_client_start(pn_transport_t *transport, const char *mechs,
    ...
    do {
        ...
        result = sasl_client_start(cyrus_conn, mechs, &client_interact, &out,
                                   &outlen, mechusing);
...
bool cyrus_sasl_process_mechanisms(pn_transport_t *transport,
                                   const char *mechs) {
    sasl_conn_t *cyrus_conn = (sasl_conn_t *)pnx_sasl_get_context(transport);
...
static int pni_wrap_client_step(pn_transport_t *transport,
                                const pn_bytes_t *in) {
    sasl_conn_t *cyrus_conn = (sasl_conn_t *)pnx_sasl_get_context(transport);
    sasl_interact_t *client_interact = NULL;
    const char *out;
    ...
    do {
        ...
        result = sasl_client_step(cyrus_conn, in->start, in->size,
...
void cyrus_sasl_process_challenge(pn_transport_t *transport,
                                  const pn_bytes_t *recv) {
    sasl_conn_t *cyrus_conn = (sasl_conn_t *)pnx_sasl_get_context(transport);
    ...
    case SASL_OK:
        // Authenticated
        // TODO: Documented that we need to call sasl_client_step() again to be
        // sure!;
    case SASL_CONTINUE:
...
static int pni_wrap_server_start(pn_transport_t *transport,
                                 const char *mech_selected,
                                 const pn_bytes_t *in) {
    int result;
    const char *out;
    ...
    // Interop hack for ANONYMOUS - some of the earlier versions of proton will
    // send and no data with an anonymous init because it is optional. It seems
    // that Cyrus wants an empty string here or it will challenge, which the
    // earlier implementation is not prepared for. However we can't just always
    // use an empty string as the CRAM-MD5 mech won't allow any data in the
    // server start
    ...
    result = sasl_server_start(cyrus_conn, mech_selected, in_bytes, in_size,
                               &out, &outlen);
...
static int pni_wrap_server_step(pn_transport_t *transport,
                                const pn_bytes_t *in) {
    int result;
    const char *out;
    unsigned outlen;
    sasl_conn_t *cyrus_conn = (sasl_conn_t *)pnx_sasl_get_context(transport);
    result = sasl_server_step(cyrus_conn, in->start, in->size, &out, &outlen);

    pnx_sasl_set_bytes_out(transport, pn_bytes(outlen, out));
    return result;
}

void cyrus_sasl_process_response(pn_transport_t *transport,
                                 const pn_bytes_t *recv) {
...
ssize_t cyrus_sasl_max_encrypt_size(pn_transport_t *transport) {
    ...
    return outbuf_size -
           // XXX: this  is a clientside workaround/hack to make GSSAPI work as
           // the Cyrus SASL GSSAPI plugin seems to return an incorrect value
           // for the buffer size on the client side, which is greater than the
           // value returned on the server side. Actually using the entire
           // client side buffer will cause a server side error due to a buffer
           // overrun.
           (pnx_sasl_is_client(transport) ? 60 : 0);
}

ssize_t cyrus_sasl_encode(pn_transport_t *transport, pn_bytes_t in,
                          pn_bytes_t *out) {
...
ssize_t cyrus_sasl_decode(pn_transport_t *transport, pn_bytes_t in,
                          pn_bytes_t *out) {
    if (in.size == 0) return 0;
    sasl_conn_t *cyrus_conn = (sasl_conn_t *)pnx_sasl_get_context(transport);
    const char *output;
