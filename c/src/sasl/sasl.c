...
void pnx_sasl_logf(pn_transport_t *logger, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    if (logger->trace & PN_TRACE_DRV) pn_transport_vlogf(logger, fmt, ap);
    va_end(ap);
}

void pnx_sasl_error(pn_transport_t *logger, const char *err,
...
void pnx_sasl_set_selected_mechanism(pn_transport_t *transport,
...
void pnx_sasl_succeed_authentication(pn_transport_t *transport,
                                     const char *username) {
    if (transport->sasl) {
...
void pnx_sasl_set_implementation(pn_transport_t *transport,
                                 const pnx_sasl_implementation *i,
                                 void *context) {
...
//-----------------------------------------------------------------------------
// pni_sasl_impl: Abstract the entry points to the SASL implementation (virtual
// function calls)
...
static inline void pni_sasl_impl_process_init(pn_transport_t *transport,
                                              const char *mechanism,
...
pni_sasl_impl_max_encrypt_size(pn_transport_t *transport) {
...
static ssize_t pn_input_read_sasl_header(pn_transport_t *transport,
                                         unsigned int layer, const char *bytes,
                                         size_t available);
static ssize_t pn_input_read_sasl(pn_transport_t *transport, unsigned int layer,
                                  const char *bytes, size_t available);
static ssize_t pn_input_read_sasl_encrypt(pn_transport_t *transport,
                                          unsigned int layer, const char *bytes,
                                          size_t available);
static ssize_t pn_output_write_sasl_header(pn_transport_t *transport,
                                           unsigned int layer, char *bytes,
                                           size_t size);
static ssize_t pn_output_write_sasl(pn_transport_t *transport,
                                    unsigned int layer, char *bytes,
                                    size_t available);
...
const pn_io_layer_t sasl_header_layer = {pn_input_read_sasl_header,
                                         pn_output_write_sasl_header,
                                         pn_error_sasl, NULL, NULL};

const pn_io_layer_t sasl_write_header_layer = {
    pn_input_read_sasl, pn_output_write_sasl_header, pn_error_sasl, NULL, NULL};

const pn_io_layer_t sasl_read_header_layer = {
    pn_input_read_sasl_header, pn_output_write_sasl, pn_error_sasl, NULL, NULL};

const pn_io_layer_t sasl_layer = {pn_input_read_sasl, pn_output_write_sasl,
                                  pn_error_sasl, NULL, NULL};

const pn_io_layer_t sasl_encrypt_layer = {
    pn_input_read_sasl_encrypt, pn_output_write_sasl_encrypt, NULL, NULL, NULL};
...
static bool pni_sasl_is_server_state(enum pnx_sasl_state state) {
    return state == SASL_NONE || state == SASL_POSTED_MECHANISMS ||
           state == SASL_POSTED_CHALLENGE || state == SASL_POSTED_OUTCOME ||
           state == SASL_ERROR;
}

static bool pni_sasl_is_client_state(enum pnx_sasl_state state) {
    return state == SASL_NONE || state == SASL_POSTED_INIT ||
           state == SASL_POSTED_RESPONSE ||
           state == SASL_RECVED_OUTCOME_SUCCEED ||
           state == SASL_RECVED_OUTCOME_FAIL || state == SASL_ERROR;
}
...
static bool pni_sasl_is_final_input_state(pni_sasl_t *sasl) {
    ...
    return desired_state == SASL_RECVED_OUTCOME_SUCCEED ||
           ...
           desired_state == SASL_ERROR || desired_state == SASL_POSTED_OUTCOME;
}
...
static bool pni_sasl_is_final_output_state(pni_sasl_t *sasl) {
    ...
    return (desired_state == SASL_RECVED_OUTCOME_SUCCEED &&
           ...
           last_state == SASL_RECVED_OUTCOME_FAIL || last_state == SASL_ERROR ||
...
void pnx_sasl_set_desired_state(pn_transport_t *transport,
    ...
    if (sasl->last_state > desired_state) {
        if (transport->trace & PN_TRACE_DRV)
            pn_transport_logf(transport,
                              "Trying to send SASL frame (%d), but illegal: "
                              "already in later state (%d)",
                              desired_state, sasl->last_state);
    } else if (sasl->client && !pni_sasl_is_client_state(desired_state)) {
        if (transport->trace & PN_TRACE_DRV)
            pn_transport_logf(
                transport, "Trying to send server SASL frame (%d) on a client",
                desired_state);
    } else if (!sasl->client && !pni_sasl_is_server_state(desired_state)) {
        if (transport->trace & PN_TRACE_DRV)
            pn_transport_logf(
                transport, "Trying to send client SASL frame (%d) on a server",
                desired_state);
    } else {
        // If we need to repeat CHALLENGE or RESPONSE frames adjust current
        // state to seem like they haven't been sent yet
        if (sasl->last_state == desired_state &&
        ...
        // Don't emit transport event on error as there will be a
...
// included.
static bool pni_sasl_included_mech(const char *included_mech_list,
                                   pn_bytes_t s) {
    ...
    while (c != NULL) {
        ...
        if (pn_strncasecmp(c, s.start, len) == 0 &&
            (c[len] == ' ' || c[len] == 0))
            return true;
...
static bool pni_sasl_client_included_mech(const char *included_mech_list,
                                          pn_bytes_t s) {
    if (!included_mech_list && pni_excluded_mechs)
        return !pni_sasl_included_mech(pni_excluded_mechs, s);
...
static void pni_split_mechs(char *mechlist, const char *included_mechs,
                            char *mechs[], int *count) {
    char *start = mechlist;
    char *end = start;
    ...
    while (*end) {
        if (*end == ' ') {
            if (start != end) {
                *end = '\0';
                if (pni_sasl_server_included_mech(
                        included_mechs, pn_bytes(end - start, start))) {
                    mechs[(*count)++] = start;
                }
            }
            end++;
    ...
    if (start != end) {
        if (pni_sasl_server_included_mech(included_mechs,
                                          pn_bytes(end - start, start))) {
            mechs[(*count)++] = start;
        }
    }
}
    ...
    while (sasl->desired_state > sasl->last_state) {
        ...
        case SASL_POSTED_INIT:
            pn_post_frame(transport, SASL_FRAME_TYPE, 0, "DL[szS]", SASL_INIT,
                          sasl->selected_mechanism, out.size, out.start,
                          sasl->local_fqdn);
            pni_emit(transport);
            break;
        case SASL_POSTED_MECHANISMS: {
            ...
            int count = 0;
            if (mechlist) {
                pni_split_mechs(mechlist, sasl->included_mechanisms, mechs,
                                &count);
            }
        ...
        case SASL_POSTED_OUTCOME:
            ...
            if (sasl->outcome != PN_SASL_OK) {
                pn_do_error(transport, "amqp:unauthorized-access",
                            "Failed to authenticate client [mech=%s]",
...
static ssize_t pn_input_read_sasl_encrypt(pn_transport_t *transport,
    ...
    for (size_t processed = 0; processed < available;) {
        ...
        size_t decode_size = (available - processed) <= max_buffer
                                 ? (available - processed)
                                 : max_buffer;
        ssize_t size = pni_sasl_impl_decode(
            transport, pn_bytes(decode_size, bytes + processed), &decoded);
    ...
    while (processed_size < decoded.size) {
        ssize_t size = pni_passthru_layer.process_input(
            transport, layer, decoded.start + processed_size,
            decoded.size - processed_size);
        if (size == 0) break;
        if (size < 0) return size;
...
static ssize_t pn_output_write_sasl_encrypt(pn_transport_t *transport,
                                            ...
                                            size_t available) {
    ssize_t clear_size =
        pni_passthru_layer.process_output(transport, layer, bytes, available);
    ...
    for (ssize_t processed = 0; processed < clear_size;) {
        ...
        ssize_t encode_size = (clear_size - processed) <= max_buffer
                                  ? (clear_size - processed)
                                  : max_buffer;
        ssize_t size = pni_sasl_impl_encode(
            transport, pn_bytes(encode_size, bytes + processed), &encoded);
        if (size < 0) return size;
        if (size > 0) {
...
pn_sasl_t *pn_sasl(pn_transport_t *transport) {
    if (!transport->sasl) {
        ...
        sasl->impl = global_sasl_impl ? global_sasl_impl
                                      : cyrus_sasl_impl ? cyrus_sasl_impl
                                                        : &default_sasl_impl;
...
int pn_do_init(pn_transport_t *transport, uint8_t frame_type, uint16_t channel,
               pn_data_t *args, const pn_bytes_t *payload) {
    pni_sasl_t *sasl = transport->sasl;

    // If we haven't got an sasl struct yet we've in an error state
    // This can only happen if our peer sent SASL frames even though he didn't
    ...
    if (!pni_sasl_server_included_mech(sasl->included_mechanisms, mech)) {
        pnx_sasl_error(transport,
                       "Client mechanism not in mechanism inclusion list.",
...
int pn_do_mechanisms(pn_transport_t *transport, uint8_t frame_type,
                     uint16_t channel, pn_data_t *args,
                     const pn_bytes_t *payload) {
    pni_sasl_t *sasl = transport->sasl;

    // If we haven't got an sasl struct yet we've in an error state
    // This can only happen if our peer sent SASL frames even though we didn't
    // send the SASL header
    if (!sasl) return PN_ERR;
    ...
    if (!(pni_sasl_impl_init_client(transport) && pn_string_size(mechs) &&
...
// Received client side
int pn_do_challenge(pn_transport_t *transport, uint8_t frame_type,
                    uint16_t channel, pn_data_t *args,
                    const pn_bytes_t *payload) {
    pni_sasl_t *sasl = transport->sasl;
...
// Received server side
int pn_do_response(pn_transport_t *transport, uint8_t frame_type,
                   uint16_t channel, pn_data_t *args,
                   const pn_bytes_t *payload) {
    pni_sasl_t *sasl = transport->sasl;
...
// Received client side
int pn_do_outcome(pn_transport_t *transport, uint8_t frame_type,
                  uint16_t channel, pn_data_t *args,
                  const pn_bytes_t *payload) {
    pni_sasl_t *sasl = transport->sasl;
