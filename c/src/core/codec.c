...
static const pn_fields_t *pni_node_fields(pn_data_t *data, pni_node_t *node) {
    ...
    if (descriptor->atom.u.as_ulong >= FIELD_MIN &&
        descriptor->atom.u.as_ulong <= FIELD_MAX) {
...
int pni_inspect_atom(pn_atom_t *atom, pn_string_t *str) {
    ...
    case PN_DECIMAL128:
        return pn_string_addf(
            str,
            "D128(%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx"
            "%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx"
            "%02hhx%02hhx)",
            atom->u.as_decimal128.bytes[0], atom->u.as_decimal128.bytes[1],
            atom->u.as_decimal128.bytes[2], atom->u.as_decimal128.bytes[3],
            atom->u.as_decimal128.bytes[4], atom->u.as_decimal128.bytes[5],
            atom->u.as_decimal128.bytes[6], atom->u.as_decimal128.bytes[7],
            atom->u.as_decimal128.bytes[8], atom->u.as_decimal128.bytes[9],
            atom->u.as_decimal128.bytes[10], atom->u.as_decimal128.bytes[11],
            atom->u.as_decimal128.bytes[12], atom->u.as_decimal128.bytes[13],
            atom->u.as_decimal128.bytes[14], atom->u.as_decimal128.bytes[15]);
    case PN_UUID:
        return pn_string_addf(
            str,
            ...
            "%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx)",
            atom->u.as_uuid.bytes[0], atom->u.as_uuid.bytes[1],
            atom->u.as_uuid.bytes[2], atom->u.as_uuid.bytes[3],
            atom->u.as_uuid.bytes[4], atom->u.as_uuid.bytes[5],
            atom->u.as_uuid.bytes[6], atom->u.as_uuid.bytes[7],
            atom->u.as_uuid.bytes[8], atom->u.as_uuid.bytes[9],
            atom->u.as_uuid.bytes[10], atom->u.as_uuid.bytes[11],
            atom->u.as_uuid.bytes[12], atom->u.as_uuid.bytes[13],
            atom->u.as_uuid.bytes[14], atom->u.as_uuid.bytes[15]);
    case PN_BINARY:
    case PN_STRING:
    case PN_SYMBOL: {
        int err;
        ...
        if (quote)
            if ((err = pn_string_addf(str, "\""))) return err;
...
int pni_inspect_enter(void *ctx, pn_data_t *data, pni_node_t *node) {
    ...
    pni_node_t *grandparent =
        parent ? pn_data_node(data, parent->parent) : NULL;
    ...
    if (grandfields) {
        ...
        const char *name =
            (index < grandfields->field_count)
                ? (const char *)FIELD_STRINGPOOL.STRING0 +
    ...
    if (!grandfields || node->atom.type != PN_NULL) {
        if (next) {
            ...
            if (parent && parent->atom.type == PN_MAP && (index % 2) == 0) {
                err = pn_string_addf(str, "=");
            } else if (parent && parent->atom.type == PN_DESCRIBED &&
                       index == 0) {
                err = pn_string_addf(str, " ");
                if (err) return err;
            } else {
...
pn_data_t *pn_data(size_t capacity) {
    ...
    data->size = 0;
    data->nodes =
        capacity ? (pni_node_t *)malloc(capacity * sizeof(pni_node_t)) : NULL;
...
void pn_data_free(pn_data_t *data) { pn_free(data); }

int pn_data_errno(pn_data_t *data) { return pn_error_code(data->error); }

pn_error_t *pn_data_error(pn_data_t *data) { return data->error; }

size_t pn_data_size(pn_data_t *data) { return data ? data->size : 0; }
...
static int pni_data_grow(pn_data_t *data) {
    ...
    if (capacity >= PNI_NID_MAX)
        return PN_OUT_OF_MEMORY;
    else if (capacity < PNI_NID_MAX / 2)
        capacity *= 2;
    else
        capacity = PNI_NID_MAX;

    pni_node_t *new_nodes =
        (pni_node_t *)realloc(data->nodes, capacity * sizeof(pni_node_t));
...
static ssize_t pni_data_intern(pn_data_t *data, const char *start,
                               size_t size) {
...
/* Format codes:
   ...
   matching ']' are elements. T: type (pn_type_t) - set array type while in
   array
...
int pn_data_vfill(pn_data_t *data, const char *fmt, va_list ap) {
    ...
    while (*fmt) {
        ...
        {
            // For maximum portability, caller must pass these as two separate
            // args, not a single struct
        ...
        case 'M': {
            ...
            err = (src && pn_data_size(src) > 0)
                      ? pni_normalize_multiple(data, src)
...
int pn_data_vscan(pn_data_t *data, const char *fmt, va_list ap) {
    ...
    while (*fmt) {
        ...
        case '?':
            if (!*fmt || *fmt == '?')
                return pn_error_format(data->error, PN_ARG_ERR,
                                       "codes must follow a ?");
        ...
        default:
            return pn_error_format(data->error, PN_ARG_ERR,
                                   "unrecognized scan code: 0x%.2X '%c'", code,
                                   code);
        }
...
void pn_data_dump(pn_data_t *data) {
    ...
    for (unsigned i = 0; i < data->size; i++) {
        ...
        printf("Node %i: prev=%" PN_ZI ", next=%" PN_ZI ", parent=%" PN_ZI
               ", down=%" PN_ZI ", children=%" PN_ZI ", type=%s (%s)\n",
               i + 1, (size_t)node->prev, (size_t)node->next,
               (size_t)node->parent, (size_t)node->down, (size_t)node->children,
...
pn_bytes_t pn_data_get_bytes(pn_data_t *data) {
    pni_node_t *node = pni_data_current(data);
    if (node && (node->atom.type == PN_BINARY || node->atom.type == PN_STRING ||
                 node->atom.type == PN_SYMBOL)) {
        return node->atom.u.as_bytes;
    } else {
        pn_bytes_t t = {0};
