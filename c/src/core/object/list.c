...
static void pni_list_ensure(pn_list_t *list, size_t capacity) {
    ...
    if (list->capacity < capacity) {
        ...
        list->elements =
            (void **)realloc(list->elements, newcap * sizeof(void *));
...
void *pn_list_minpop(pn_list_t *list) {
    ...
    for (now = 1; now * 2 <= size; now = child) {
        child = now * 2;
        if (child != size &&
            pn_class_compare(list->clazz, heap[child], heap[child + 1]) > 0) {
            child++;
        }
...
void pn_list_iterator(pn_list_t *list, pn_iterator_t *iter) {
    pni_list_iter_t *liter = (pni_list_iter_t *)pn_iterator_start(
        iter, pni_list_next, sizeof(pni_list_iter_t));
    liter->list = list;
    liter->index = 0;
}
