...
static pni_entry_t *pni_map_entry(pn_map_t *map, void *key, pni_entry_t **pprev,
                                  bool create) {
...
static uintptr_t pni_identity_hashcode(void *obj) { return (uintptr_t)obj; }

static bool pni_identity_equals(void *a, void *b) { return a == b; }
...
pn_hash_t *pn_hash(const pn_class_t *clazz, size_t capacity,
                   float load_factor) {
    pn_hash_t *hash =
        (pn_hash_t *)pn_map(PN_UINTPTR, clazz, capacity, load_factor);
    hash->map.hashcode = pni_identity_hashcode;
    hash->map.equals = pni_identity_equals;
    return hash;
}

size_t pn_hash_size(pn_hash_t *hash) { return pn_map_size(&hash->map); }
...
pn_handle_t pn_hash_head(pn_hash_t *hash) { return pn_map_head(&hash->map); }
