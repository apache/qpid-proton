...
pn_string_t *pn_stringn(const char *bytes, size_t n) {
    ...
    pn_string_t *string =
        (pn_string_t *)pn_class_new(&clazz, sizeof(pn_string_t));
...
void pn_string_clear(pn_string_t *string) { pn_string_set(string, NULL); }
...
int pn_string_vaddf(pn_string_t *string, const char *format, va_list ap) {
    ...
    while (true) {
        va_copy(copy, ap);
        int err = pni_vsnprintf(string->bytes + string->size,
                                string->capacity - string->size, format, copy);
        va_end(copy);
        if (err < 0) {
            return err;
