...
int pni_bad_frame(pn_transport_t *transport, uint8_t frame_type,
                  uint16_t channel, pn_data_t *args,
                  const pn_bytes_t *payload) {
    pn_transport_logf(transport,
                      "Error dispatching frame: type: %d: Unknown performative",
                      frame_type);
    return PN_ERR;
}

int pni_bad_frame_type(pn_transport_t *transport, uint8_t frame_type,
                       uint16_t channel, pn_data_t *args,
                       const pn_bytes_t *payload) {
    pn_transport_logf(transport,
                      "Error dispatching frame: Unknown frame type: %d",
                      frame_type);
    return PN_ERR;
}
...
static inline int pni_dispatch_action(pn_transport_t *transport, uint64_t lcode,
    ...
    case AMQP_FRAME_TYPE:
        ...
        case FLOW:
            ...
            break;
        case TRANSFER:
            action = pn_do_transfer;
            break;
        case DISPOSITION:
            action = pn_do_disposition;
            break;
        case DETACH:
            action = pn_do_detach;
            break;
    ...
    case SASL_FRAME_TYPE:
        ...
        case SASL_MECHANISMS:
            action = pn_do_mechanisms;
            break;
        case SASL_INIT:
            action = pn_do_init;
            break;
        case SASL_CHALLENGE:
            action = pn_do_challenge;
            break;
        case SASL_RESPONSE:
            action = pn_do_response;
            break;
        case SASL_OUTCOME:
            action = pn_do_outcome;
            break;
        default:
            action = pni_bad_frame;
            break;
...
static int pni_dispatch_frame(pn_transport_t *transport, pn_data_t *args,
    ...
    if (dsize < 0) {
        pn_string_format(transport->scratch, "Error decoding frame: %s %s\n",
...
ssize_t pn_dispatcher_output(pn_transport_t *transport, char *bytes,
                             size_t size) {
