...
static pn_event_t *batch_next(pn_event_batch_t *batch) {
    pn_connection_driver_t *d =
        (pn_connection_driver_t *)((char *)batch -
                                   offsetof(pn_connection_driver_t, batch));
...
int pn_connection_driver_init(pn_connection_driver_t *d, pn_connection_t *c,
                              pn_transport_t *t) {
    memset(d, 0, sizeof(*d));
...
pn_rwbytes_t pn_connection_driver_read_buffer(pn_connection_driver_t *d) {
    ...
    return (cap > 0) ? pn_rwbytes(cap, pn_transport_tail(d->transport))
                     : pn_rwbytes(0, 0);
}
...
pn_bytes_t pn_connection_driver_write_buffer(pn_connection_driver_t *d) {
    ...
    return (pending > 0) ? pn_bytes(pending, pn_transport_head(d->transport))
                         : pn_bytes_null;
}
...
bool pn_connection_driver_has_event(pn_connection_driver_t *d) {
    return d->connection &&
           pn_collector_peek(pn_connection_collector(d->connection));
}

bool pn_connection_driver_finished(pn_connection_driver_t *d) {
    return pn_transport_closed(d->transport) &&
           !pn_connection_driver_has_event(d);
}
...
pn_event_batch_connection_driver(pn_event_batch_t *batch) {
    return (batch->next_event == batch_next)
               ? (pn_connection_driver_t *)((char *)batch -
                                            offsetof(pn_connection_driver_t,
                                                     batch))
               : NULL;
}

pn_connection_driver_t **pn_connection_driver_ptr(pn_connection_t *c) {
    return &c->driver;
}
