...
static void pni_calculate_channel_max(pn_transport_t *transport) {
    ...
    transport->channel_max =
        (PN_IMPL_CHANNEL_MAX < transport->local_channel_max)
            ? PN_IMPL_CHANNEL_MAX
    ...
    if (transport->open_rcvd) {
        transport->channel_max =
            (transport->channel_max < transport->remote_channel_max)
                ? transport->channel_max
...
void pn_delivery_map_free(pn_delivery_map_t *db) { pn_free(db->deliveries); }
...
static pn_delivery_t *pni_delivery_map_get(pn_delivery_map_t *db,
                                           pn_sequence_t id) {
    return (pn_delivery_t *)pn_hash_get(db->deliveries,
                                        pni_sequence_make_hash(id));
}

static void pn_delivery_state_init(pn_delivery_state_t *ds,
                                   pn_delivery_t *delivery, pn_sequence_t id) {
    ds->id = id;
    ds->sending = false;
    ds->sent = false;
    ds->init = true;
...
static void pni_delivery_map_clear(pn_delivery_map_t *dm) {
    ...
    for (pn_handle_t entry = pn_hash_head(hash); entry;
...
static ssize_t pn_io_layer_input_error(pn_transport_t *, unsigned int,
                                       const char *, size_t);
static ssize_t pn_io_layer_output_error(pn_transport_t *, unsigned int, char *,
                                        size_t);

static ssize_t pn_io_layer_input_setup(pn_transport_t *transport,
                                       unsigned int layer, const char *bytes,
                                       size_t available);
static ssize_t pn_io_layer_output_setup(pn_transport_t *transport,
                                        unsigned int layer, char *bytes,
                                        size_t available);

static ssize_t pn_input_read_amqp_header(pn_transport_t *transport,
                                         unsigned int layer, const char *bytes,
                                         size_t available);
static ssize_t pn_input_read_amqp(pn_transport_t *transport, unsigned int layer,
                                  const char *bytes, size_t available);
static ssize_t pn_output_write_amqp_header(pn_transport_t *transport,
                                           unsigned int layer, char *bytes,
                                           size_t available);
static ssize_t pn_output_write_amqp(pn_transport_t *transport,
                                    unsigned int layer, char *bytes,
                                    size_t available);
static void pn_error_amqp(pn_transport_t *transport, unsigned int layer);
static pn_timestamp_t pn_tick_amqp(pn_transport_t *transport,
                                   unsigned int layer, pn_timestamp_t now);

static ssize_t pn_io_layer_input_autodetect(pn_transport_t *transport,
                                            unsigned int layer,
                                            const char *bytes,
                                            size_t available);
static ssize_t pn_io_layer_output_null(pn_transport_t *transport,
                                       unsigned int layer, char *bytes,
                                       size_t available);

const pn_io_layer_t amqp_header_layer = {pn_input_read_amqp_header,
                                         pn_output_write_amqp_header, NULL,
                                         pn_tick_amqp, NULL};

const pn_io_layer_t amqp_write_header_layer = {
    pn_input_read_amqp, pn_output_write_amqp_header, NULL, pn_tick_amqp, NULL};

const pn_io_layer_t amqp_read_header_layer = {
    pn_input_read_amqp_header, pn_output_write_amqp, pn_error_amqp,
    pn_tick_amqp, NULL};

const pn_io_layer_t amqp_layer = {pn_input_read_amqp, pn_output_write_amqp,
                                  pn_error_amqp, pn_tick_amqp, NULL};

const pn_io_layer_t pni_setup_layer = {
    pn_io_layer_input_setup, pn_io_layer_output_setup, NULL, NULL, NULL};

const pn_io_layer_t pni_autodetect_layer = {
    pn_io_layer_input_autodetect, pn_io_layer_output_null, NULL, NULL, NULL};

const pn_io_layer_t pni_passthru_layer = {
    pn_io_layer_input_passthru, pn_io_layer_output_passthru, NULL, NULL, NULL};

const pn_io_layer_t pni_header_error_layer = {
    pn_io_layer_input_error, pn_output_write_amqp_header, NULL, NULL, NULL};

const pn_io_layer_t pni_error_layer = {pn_io_layer_input_error,
                                       pn_io_layer_output_error, pn_error_amqp,
                                       NULL, NULL};
...
ssize_t pn_io_layer_input_setup(pn_transport_t *transport, unsigned int layer,
                                const char *bytes, size_t available) {
...
ssize_t pn_io_layer_output_setup(pn_transport_t *transport, unsigned int layer,
...
ssize_t pn_io_layer_input_autodetect(pn_transport_t *transport,
                                     unsigned int layer, const char *bytes,
                                     size_t available) {
    const char *error;
    ...
    case PNI_PROTOCOL_AMQP_SSL:
        if (!(transport->allowed_layers & LAYER_AMQPSSL)) {
            error =
                "AMQP SSL protocol header not allowed (maybe detected twice)";
            break;
        }
    ...
    case PNI_PROTOCOL_AMQP_SASL:
        if (!(transport->allowed_layers & LAYER_AMQPSASL)) {
            error =
                "AMQP SASL protocol header not allowed (maybe detected twice)";
            break;
        }
    ...
    pn_do_error(transport, "amqp:connection:framing-error", "%s: '%s'%s", error,
                quoted, !eos ? "" : " (connection aborted)");
    return 0;
}

// We don't know what the output should be - do nothing
ssize_t pn_io_layer_output_null(pn_transport_t *transport, unsigned int layer,
                                char *bytes, size_t available) {
    return 0;
}

/** Pass through input handler */
ssize_t pn_io_layer_input_passthru(pn_transport_t *transport,
                                   unsigned int layer, const char *data,
                                   size_t available) {
    if (layer + 1 < PN_IO_LAYER_CT)
        return transport->io_layers[layer + 1]->process_input(
            transport, layer + 1, data, available);
    return PN_EOS;
}

/** Pass through output handler */
ssize_t pn_io_layer_output_passthru(pn_transport_t *transport,
                                    unsigned int layer, char *data,
                                    size_t available) {
    if (layer + 1 < PN_IO_LAYER_CT)
        return transport->io_layers[layer + 1]->process_output(
            transport, layer + 1, data, available);
    return PN_EOS;
}

/** Input handler after detected error */
ssize_t pn_io_layer_input_error(pn_transport_t *transport, unsigned int layer,
                                const char *data, size_t available) {
    return PN_EOS;
}

/** Output handler after detected error */
ssize_t pn_io_layer_output_error(pn_transport_t *transport, unsigned int layer,
                                 char *data, size_t available) {
    return PN_EOS;
}
...
static void pn_transport_initialize(void *object) {
    ...
    transport->allowed_layers =
        LAYER_AMQP1 | LAYER_AMQPSASL | LAYER_AMQPSSL | LAYER_SSL;
...
static pn_session_t *pni_channel_state(pn_transport_t *transport,
                                       uint16_t channel) {
...
const char *pn_transport_get_user(pn_transport_t *transport) {
    ...
    if (transport->present_layers & LAYER_AMQPSASL)
        return pn_sasl_get_user((pn_sasl_t *)transport);
    // No SASL but we may have a SSL remote_subject
    if (transport->present_layers & (LAYER_AMQPSSL | LAYER_SSL))
        return pn_ssl_get_remote_subject((pn_ssl_t *)transport);
...
bool pn_transport_is_encrypted(pn_transport_t *transport) {
    return transport && transport->ssl &&
           pn_ssl_get_ssf((pn_ssl_t *)transport) > 0;
}
...
static void pni_post_remote_open_events(pn_transport_t *transport,
    ...
    if (transport->remote_idle_timeout) {
        pn_collector_put(connection->collector, PN_OBJECT, transport,
                         PN_TRANSPORT);
    }
}
...
int pn_transport_bind(pn_transport_t *transport, pn_connection_t *connection) {
    ...
    if (pn_string_size(connection->hostname)) {
        if (transport->sasl) {
            pni_sasl_set_remote_hostname(transport,
        ...
        if (transport->ssl) {
            ...
            if (name_len == 0) {
                pn_ssl_set_peer_hostname((pn_ssl_t *)transport,
                                         pn_string_get(connection->hostname));
            }
        }
    }
...
void pni_transport_unbind_handles(pn_hash_t *handles, bool reset_state) {
    for (pn_handle_t h = pn_hash_head(handles); h;
         h = pn_hash_next(handles, h)) {
...
void pni_transport_unbind_channels(pn_hash_t *channels) {
    for (pn_handle_t h = pn_hash_head(channels); h;
         h = pn_hash_next(channels, h)) {
...
static int pni_disposition_encode(pn_disposition_t *disposition,
                                  pn_data_t *data) {
    ...
    case PN_RELEASED:
        return 0;
    case PN_REJECTED:
        return pn_data_fill(data, "[?DL[sSC]]", pn_condition_is_set(cond),
                            ERROR, pn_condition_get_name(cond),
                            pn_condition_get_description(cond),
                            pn_condition_info(cond));
    case PN_MODIFIED:
        return pn_data_fill(data, "[ooC]", disposition->failed,
...
void pn_do_trace(pn_transport_t *transport, uint16_t ch, pn_dir_t dir,
    ...
    if (transport->trace & PN_TRACE_FRM) {
        pn_string_format(transport->scratch, "%u %s ", ch,
                         dir == OUT ? "->" : "<-");
        ...
        if (size) {
            ...
            pn_string_addf(transport->scratch, " (%" PN_ZU ") \"%s\"%s", size,
                           buf, e == PN_OVERFLOW ? "... (truncated)" : "");
        }
...
int pn_post_frame(pn_transport_t *transport, uint8_t type, uint16_t ch,
                  const char *fmt, ...) {
    ...
    va_end(ap);
    if (err) {
        pn_transport_logf(transport, "error posting frame: %s, %s: %s", fmt,
                          pn_code(err),
...
static int pni_post_amqp_transfer_frame(
    pn_transport_t *transport, uint16_t ch, uint32_t handle, pn_sequence_t id,
    pn_bytes_t *payload, const pn_bytes_t *tag, uint32_t message_format,
    bool settled, bool more, pn_sequence_t frame_limit, uint64_t code,
    pn_data_t *state, bool resume, bool aborted, bool batchable) {
    bool more_flag = more;
...
compute_performatives:
    ...
    int err = pn_data_fill(transport->output_args, "DL[IIzI?o?on?DLC?o?o?o]",
                           TRANSFER, handle, id, tag->size, tag->start,
                           message_format, settled, settled, more_flag,
                           more_flag, (bool)code, code, state, resume, resume,
                           aborted, aborted, batchable, batchable);
    if (err) {
        pn_transport_logf(transport, "error posting transfer frame: %s: %s",
                          pn_code(err),
                          pn_error_text(pn_data_error(transport->output_args)));
        return PN_ERR;
    }
    ...
    encode_performatives:
        ...
        ssize_t wr =
            pn_data_encode(transport->output_args, buf.start, buf.size);
        if (wr < 0) {
            if (wr == PN_OVERFLOW) {
                pn_buffer_ensure(frame, pn_buffer_available(frame) * 2);
                goto encode_performatives;
            }
            pn_transport_logf(transport, "error posting frame: %s",
                              pn_code(wr));
            return PN_ERR;
        }
        buf.size = wr;
...
int pn_do_open(pn_transport_t *transport, uint8_t frame_type, uint16_t channel,
    ...
    int err = pn_data_scan(
        args, "D.[?S?S?I?HI..CCC]", &container_q, &remote_container,
        &hostname_q, &remote_hostname, &remote_max_frame_q, &remote_max_frame,
        &remote_channel_max_q, &remote_channel_max,
        &transport->remote_idle_timeout, transport->remote_offered_capabilities,
        transport->remote_desired_capabilities, transport->remote_properties);
    if (err) return err;
    /*
     ...
     */
    transport->remote_channel_max =
        remote_channel_max_q ? remote_channel_max : OPEN_CHANNEL_MAX_DEFAULT;
    transport->remote_max_frame =
        remote_max_frame_q ? remote_max_frame : OPEN_MAX_FRAME_SIZE_DEFAULT;

    if (transport->remote_max_frame > 0) {
        if (transport->remote_max_frame < AMQP_MIN_MAX_FRAME_SIZE) {
            pn_transport_logf(
                transport, "Peer advertised bad max-frame (%u), forcing to %u",
                transport->remote_max_frame, AMQP_MIN_MAX_FRAME_SIZE);
            transport->remote_max_frame = AMQP_MIN_MAX_FRAME_SIZE;
        }
    }
    free(transport->remote_container);
    transport->remote_container =
        container_q ? pn_bytes_strdup(remote_container) : NULL;
    free(transport->remote_hostname);
...
int pn_do_begin(pn_transport_t *transport, uint8_t frame_type, uint16_t channel,
                pn_data_t *args, const pn_bytes_t *payload) {
    bool reply;
    ...
    if (reply) {
        ssn = (pn_session_t *)pn_hash_get(transport->local_channels,
                                          remote_channel);
        if (ssn == 0) {
            pn_do_error(transport, "amqp:invalid-field",
                        "begin reply to unknown channel %d.", remote_channel);
            return PN_ARG_ERR;
        }
    } else {
...
static pn_link_t *pni_find_link(pn_session_t *ssn, pn_bytes_t name,
                                bool is_sender) {
    ...
    for (size_t i = 0; i < pn_list_size(ssn->links); i++) {
        ...
        if (link->endpoint.type == type &&
            ...
            (!(link->endpoint.state & PN_REMOTE_CLOSED) &&
             ((int32_t)link->state.remote_handle != -2)) &&
...
static void set_expiry_policy_from_symbol(pn_terminus_t *terminus,
                                          pn_bytes_t symbol) {
    if (symbol.start) {
...
static pn_distribution_mode_t symbol2dist_mode(const pn_bytes_t symbol) {
    if (!symbol.start) return PN_DIST_MODE_UNSPECIFIED;
...
int pn_do_attach(pn_transport_t *transport, uint8_t frame_type,
    ...
    uint64_t max_msgsz;
    int err = pn_data_scan(
        args, "D.[SIo?B?BD.[SIsIo.s]D.[SIsIo]..IL]", &name, &handle, &is_sender,
        &snd_settle, &snd_settle_mode, &rcv_settle, &rcv_settle_mode, &source,
        &src_dr, &src_exp, &src_timeout, &src_dynamic, &dist_mode, &target,
        &tgt_dr, &tgt_exp, &tgt_timeout, &tgt_dynamic, &idc, &max_msgsz);
    if (err) return err;
    char strbuf[128]; // avoid malloc for most link names
    char *strheap =
        (name.size >= sizeof(strbuf)) ? (char *)malloc(name.size + 1) : NULL;
    ...
    if (link && (int32_t)link->state.remote_handle >= 0) {
        pn_do_error(transport, "amqp:invalid-field",
                    "link name already attached: %s", strname);
        if (strheap) free(strheap);
        return PN_EOS;
    }
    ...
    if (snd_settle) link->remote_snd_settle_mode = snd_settle_mode;
    if (rcv_settle) link->remote_rcv_settle_mode = rcv_settle_mode;
    ...
    err = pn_data_scan(
        args, "D.[.....D.[.....C.C.CC]D.[.....CC]",
        link->remote_source.properties, link->remote_source.filter,
        link->remote_source.outcomes, link->remote_source.capabilities,
        link->remote_target.properties, link->remote_target.capabilities);
    if (err) return err;
...
int pn_do_transfer(pn_transport_t *transport, uint8_t frame_type,
                   uint16_t channel, pn_data_t *args,
                   const pn_bytes_t *payload) {
    // XXX: multi transfer
    uint32_t handle;
    pn_bytes_t tag;
    bool id_present;
    ...
    int err =
        pn_data_scan(args, "D.[I?Iz.?oo.D?LCooo]", &handle, &id_present, &id,
                     &tag, &settled_set, &settled, &more, &has_type, &type,
                     transport->disp_data, &resume, &aborted, &batchable);
    if (err) return err;
    pn_session_t *ssn = pni_channel_state(transport, channel);
    if (!ssn) {
    ...
    if (!ssn->state.incoming_window) {
        return pn_do_error(transport, "amqp:session:window-violation",
                           "incoming session window exceeded");
    }

    pn_link_t *link = pni_handle_state(ssn, handle);
    if (!link) {
        return pn_do_error(transport, "amqp:invalid-field",
                           "no such handle: %u", handle);
    }
    pn_delivery_t *delivery;
    if (link->unsettled_tail && !link->unsettled_tail->done) {
        ...
        if (settled_set && !settled && delivery->remote.settled)
            return pn_do_error(transport, "amqp:invalid-field",
                               "invalid transition from settled to unsettled");
    } else {
        ...
        if (id_present && id != state->id) {
            return pn_do_error(
                transport, "amqp:session:invalid-field",
                "sequencing error, expected delivery-id %u, got %u", state->id,
                id);
        }
        if (has_type) {
...
int pn_do_flow(pn_transport_t *transport, uint8_t frame_type, uint16_t channel,
               pn_data_t *args, const pn_bytes_t *payload) {
    ...
    if (inext_init) {
        ssn->state.remote_incoming_window =
            inext + iwin - ssn->state.outgoing_transfer_count;
    } else {
    ...
    if (handle_init) {
        ...
        if (link->endpoint.type == SENDER) {
            ...
            link->state.link_credit =
                receiver_count + link_credit - link->state.delivery_count;
...
static int pn_scan_error(pn_data_t *data, pn_condition_t *condition,
                         const char *fmt) {
    pn_bytes_t cond;
    pn_bytes_t desc;
...
int pn_do_disposition(pn_transport_t *transport, uint8_t frame_type,
                      uint16_t channel, pn_data_t *args,
                      const pn_bytes_t *payload) {
    bool role;
    ...
    pn_data_clear(transport->disp_data);
    int err =
        pn_data_scan(args, "D.[oI?IoD?LC]", &role, &first, &last_init, &last,
                     &settled, &type_init, &type, transport->disp_data);
    if (err) return err;
    ...
    // keeping track of the earliest unsettled delivery sequence no
    ...
    for (pn_sequence_t id = first; sequence_lte(id, last); ++id) {
        ...
        if (delivery) {
            ...
            if (remote_data) {
                ...
                case PN_RECEIVED:
                    ...
                    if (pn_data_next(transport->disp_data))
                        remote->section_number =
                            pn_data_get_uint(transport->disp_data);
                    if (pn_data_next(transport->disp_data))
                        remote->section_offset =
                            pn_data_get_ulong(transport->disp_data);
                    break;
                case PN_ACCEPTED:
                    break;
                case PN_REJECTED:
                    err = pn_scan_error(transport->disp_data,
                                        &remote->condition, SCAN_ERROR_DISP);
                    if (err) return err;
                    break;
                case PN_RELEASED:
                    break;
                case PN_MODIFIED:
                    ...
                    if (pn_data_next(transport->disp_data))
                        remote->undeliverable =
                            pn_data_get_bool(transport->disp_data);
                    pn_data_narrow(transport->disp_data);
                    pn_data_clear(remote->data);
                    pn_data_appendn(remote->annotations, transport->disp_data,
                                    1);
            ...
            pn_collector_put(transport->connection->collector, PN_OBJECT,
...
int pn_do_detach(pn_transport_t *transport, uint8_t frame_type,
                 uint16_t channel, pn_data_t *args, const pn_bytes_t *payload) {
    uint32_t handle;
    bool closed;
...
int pn_do_close(pn_transport_t *transport, uint8_t frame_type, uint16_t channel,
                pn_data_t *args, const pn_bytes_t *payload) {
    pn_connection_t *conn = transport->connection;
    int err =
        pn_scan_error(args, &transport->remote_condition, SCAN_ERROR_DEFAULT);
    if (err) return err;
...
static ssize_t transport_consume(pn_transport_t *transport) {
    ...
    while (transport->input_pending || transport->tail_closed) {
        ssize_t n;
        n = transport->io_layers[0]->process_input(
            transport, 0, transport->input_buf + consumed,
...
static int pni_process_conn_setup(pn_transport_t *transport,
    ...
    if (endpoint->type == CONNECTION) {
        if (!(endpoint->state & PN_LOCAL_UNINIT) && !transport->open_sent) {
            ...
            pni_calculate_channel_max(transport);
            int err = pn_post_frame(
                transport, AMQP_FRAME_TYPE, 0, "DL[SS?I?H?InnMMC]", OPEN,
                cid ? cid : "", pn_string_get(connection->hostname),
                // TODO: This is messy, because we also have to allow
                // local_max_frame_ to be 0 to mean unlimited otherwise flow
                // control goes wrong
                transport->local_max_frame != 0 &&
                    transport->local_max_frame != OPEN_MAX_FRAME_SIZE_DEFAULT,
                transport->local_max_frame,
                transport->channel_max != OPEN_CHANNEL_MAX_DEFAULT,
                transport->channel_max, (bool)idle_timeout, idle_timeout,
                connection->offered_capabilities,
                connection->desired_capabilities, connection->properties);
            if (err) return err;
...
static uint16_t allocate_alias(pn_hash_t *aliases, uint32_t max_index,
                               int *valid) {
...
static size_t pni_session_incoming_window(pn_session_t *ssn) {
    ...
    if (!size || !capacity) { /* session flow control is not enabled */
        ...
        pn_condition_format(pn_transport_condition(t), "amqp:internal-error",
                            "session capacity %" PN_ZU
                            " is less than frame size %" PRIu32,
                            capacity, size);
...
static int pni_process_ssn_setup(pn_transport_t *transport,
                                 pn_endpoint_t *endpoint) {
    if (endpoint->type == SESSION && transport->open_sent) {
        pn_session_t *ssn = (pn_session_t *)endpoint;
        pn_session_state_t *state = &ssn->state;
        if (!(endpoint->state & PN_LOCAL_UNINIT) &&
            state->local_channel == (uint16_t)-1) {
            if (!pni_map_local_channel(ssn)) {
                pn_transport_logf(transport,
                                  "unable to find an open available channel "
                                  "within limit of %d",
                                  transport->channel_max);
                return PN_ERR;
            }
            ...
            pn_post_frame(transport, AMQP_FRAME_TYPE, state->local_channel,
                          ...
                          state->remote_channel, state->outgoing_transfer_count,
                          state->incoming_window, state->outgoing_window);
        }
    }
...
static int pni_map_local_handle(pn_link_t *link) {
    ...
    state->local_handle =
        allocate_alias(ssn_state->local_handles, 65536, &valid);
    if (!valid) return 0;
...
static int pni_process_link_setup(pn_transport_t *transport,
                                  pn_endpoint_t *endpoint) {
    if (transport->open_sent &&
        (endpoint->type == SENDER || endpoint->type == RECEIVER)) {
        pn_link_t *link = (pn_link_t *)endpoint;
        pn_session_state_t *ssn_state = &link->session->state;
        pn_link_state_t *state = &link->state;
        if (((int16_t)ssn_state->local_channel >= 0) &&
            !(endpoint->state & PN_LOCAL_UNINIT) &&
            state->local_handle == (uint32_t)-1) {
            pni_map_local_handle(link);
            const pn_distribution_mode_t dist_mode =
                link->source.distribution_mode;
            if (link->target.type == PN_COORDINATOR) {
                int err = pn_post_frame(
                    transport, AMQP_FRAME_TYPE, ssn_state->local_channel,
                    "DL[SIoBB?DL[SIsIoC?sCnCC]DL[C]nnI]", ATTACH,
                    pn_string_get(link->name), state->local_handle,
                    endpoint->type == RECEIVER, link->snd_settle_mode,
                    link->rcv_settle_mode, (bool)link->source.type, SOURCE,
                    pn_string_get(link->source.address),
                    link->source.durability, expiry_symbol(&link->source),
                    link->source.timeout, link->source.dynamic,
                    link->source.properties,
                    (dist_mode != PN_DIST_MODE_UNSPECIFIED),
                    dist_mode2symbol(dist_mode), link->source.filter,
                    link->source.outcomes, link->source.capabilities,
                    COORDINATOR, link->target.capabilities, 0);
                if (err) return err;
            } else {
                int err = pn_post_frame(
                    transport, AMQP_FRAME_TYPE, ssn_state->local_channel,
                    ...
                    link->target.durability, expiry_symbol(&link->target),
                    link->target.timeout, link->target.dynamic,
                    link->target.properties, link->target.capabilities,
...
static int pni_post_flow(pn_transport_t *transport, pn_session_t *ssn,
    ...
    pn_link_state_t *state = &link->state;
    return pn_post_frame(
        transport, AMQP_FRAME_TYPE, ssn->state.local_channel,
        "DL[?IIII?I?I?In?o]", FLOW, (int16_t)ssn->state.remote_channel >= 0,
        ssn->state.incoming_transfer_count, ssn->state.incoming_window,
        ssn->state.outgoing_transfer_count, ssn->state.outgoing_window, linkq,
        linkq ? state->local_handle : 0, linkq,
        linkq ? state->delivery_count : 0, linkq,
        linkq ? state->link_credit : 0, linkq, linkq ? link->drain : false);
}

static int pni_process_flow_receiver(pn_transport_t *transport,
                                     pn_endpoint_t *endpoint) {
...
static int pni_post_disp(pn_transport_t *transport, pn_delivery_t *delivery) {
    ...
    if (!pni_disposition_batchable(&delivery->local)) {
        ...
        return pn_post_frame(transport, AMQP_FRAME_TYPE,
                             ssn->state.local_channel, "DL[oIn?o?DLC]",
                             DISPOSITION, role, state->id,
...
static int pni_process_tpwork_sender(pn_transport_t *transport,
                                     pn_delivery_t *delivery, bool *settle) {
    pn_link_t *link = delivery->link;
    pn_delivery_state_t *state = &delivery->state;
    if (delivery->aborted && !delivery->state.sending) {
        // Aborted delivery with no data yet sent, drop it and issue a FLOW as
        // we may have credit.
        *settle = true;
        state->sent = true;
        pn_collector_put(transport->connection->collector, PN_OBJECT, link,
    ...
    *settle = false;
    ...
    if ((int16_t)ssn_state->local_channel >= 0 &&
        ...
        if (!state->sent &&
            (delivery->done || pn_buffer_size(delivery->bytes) > 0) &&
            ...
            PN_RETURN_IF_ERROR(
                pni_disposition_encode(&delivery->local, transport->disp_data));
            int count = pni_post_amqp_transfer_frame(
                transport, ssn_state->local_channel, link_state->local_handle,
                state->id, &bytes, &tag,
                0, // message-format
                delivery->local.settled, !delivery->done,
                ssn_state->remote_incoming_window, delivery->local.type,
                transport->disp_data, false, /* Resume */
                delivery->aborted, false     /* Batchable */
            );
...
static int pni_process_tpwork_receiver(pn_transport_t *transport,
                                       pn_delivery_t *delivery, bool *settle) {
    *settle = false;
    pn_link_t *link = delivery->link;
...
static int pni_process_tpwork(pn_transport_t *transport,
                              pn_endpoint_t *endpoint) {
    if (endpoint->type == CONNECTION && !transport->close_sent) {
        ...
        while (delivery) {
            ...
            if (pn_link_is_sender(link)) {
                ...
                int err =
                    pni_process_tpwork_sender(transport, delivery, &settle);
                if (err) return err;
            } else {
                dm = &link->session->state.incoming;
                int err =
                    pni_process_tpwork_receiver(transport, delivery, &settle);
                if (err) return err;
            }
...
static int pni_process_flush_disp(pn_transport_t *transport,
                                  pn_endpoint_t *endpoint) {
...
static int pni_process_flow_sender(pn_transport_t *transport,
                                   pn_endpoint_t *endpoint) {
    if (endpoint->type == SENDER && endpoint->state & PN_LOCAL_ACTIVE) {
        ...
        if ((int16_t)ssn->state.local_channel >= 0 &&
            (int32_t)state->local_handle >= 0 && snd->drain && snd->drained) {
...
static int pni_process_link_teardown(pn_transport_t *transport,
                                     pn_endpoint_t *endpoint) {
    if (endpoint->type == SENDER || endpoint->type == RECEIVER) {
        ...
        pn_link_state_t *state = &link->state;
        if (((endpoint->state & PN_LOCAL_CLOSED) || link->detached) &&
            (int32_t)state->local_handle >= 0 &&
            ...
            if (pn_condition_is_set(&endpoint->condition)) {
                ...
                description =
                    pn_condition_get_description(&endpoint->condition);
                info = pn_condition_info(&endpoint->condition);
            }

            int err = pn_post_frame(
                transport, AMQP_FRAME_TYPE, ssn_state->local_channel,
                "DL[I?o?DL[sSC]]", DETACH, state->local_handle, !link->detached,
                !link->detached, (bool)name, ERROR, name, description, info);
            if (err) return err;
...
static bool pni_pointful_buffering(pn_transport_t *transport,
...
static int pni_process_ssn_teardown(pn_transport_t *transport,
                                    pn_endpoint_t *endpoint) {
    if (endpoint->type == SESSION) {
        pn_session_t *session = (pn_session_t *)endpoint;
        pn_session_state_t *state = &session->state;
        if (endpoint->state & PN_LOCAL_CLOSED &&
            (int16_t)state->local_channel >= 0 && !transport->close_sent) {
...
static int pni_process_conn_teardown(pn_transport_t *transport,
                                     pn_endpoint_t *endpoint) {
    if (endpoint->type == CONNECTION) {
...
static int pni_phase(pn_transport_t *transport,
                     int (*phase)(pn_transport_t *, pn_endpoint_t *)) {
    pn_connection_t *conn = transport->connection;
...
static pn_timestamp_t pn_tick_amqp(pn_transport_t *transport,
    ...
    if (transport->local_idle_timeout) {
        if (transport->dead_remote_deadline == 0 ||
            ...
            transport->dead_remote_deadline =
                now + transport->local_idle_timeout;
            transport->last_bytes_input = transport->bytes_input;
            ...
            if (!transport->posted_idle_timeout) {
                ...
                // Note: AMQP-1.0 really should define a generic "timeout"
                // error, but does not.
    ...
    if (transport->remote_idle_timeout && !transport->close_sent) {
        if (transport->keepalive_deadline == 0 ||
            ...
            transport->keepalive_deadline =
                now + (pn_timestamp_t)(transport->remote_idle_timeout / 2.0);
            transport->last_bytes_output = transport->bytes_output;
        } else if (transport->keepalive_deadline <= now) {
            transport->keepalive_deadline =
                now + (pn_timestamp_t)(transport->remote_idle_timeout / 2.0);
            if (pn_buffer_size(transport->output_buffer) ==
                0) { // no outbound data pending
...
static ssize_t transport_produce(pn_transport_t *transport) {
    ...
    if (space <= 0) { // can we expand the buffer?
        ...
        if (!transport->remote_max_frame) // no limit, so double it
            ...
            more = pn_min(transport->output_size,
                          transport->remote_max_frame - transport->output_size);
        if (more) {
    ...
    while (space > 0) {
        ssize_t n;
        n = transport->io_layers[0]->process_output(
            transport, 0, &transport->output_buf[transport->output_pending],
            space);
        if (n > 0) {
            space -= n;
            ...
            if (transport->output_pending) break; // return what is available
            if (transport->trace & (PN_TRACE_RAW | PN_TRACE_FRM)) {
                pn_transport_log(transport, "  -> EOS");
            }
...
int pn_transport_set_channel_max(pn_transport_t *transport,
                                 uint16_t requested_channel_max) {
    /*
    ...
    if (transport->open_sent) {
        pn_transport_logf(
            transport,
            "Cannot change local channel-max after OPEN frame sent.");
        return PN_STATE_ERR;
    }
...
void pn_transport_set_max_frame(pn_transport_t *transport, uint32_t size) {
    ...
    if (size && size < AMQP_MIN_MAX_FRAME_SIZE) size = AMQP_MIN_MAX_FRAME_SIZE;
...
pn_timestamp_t pn_transport_tick(pn_transport_t *transport,
    ...
    for (int i = 0; i < PN_IO_LAYER_CT; ++i) {
        if (transport->io_layers[i] && transport->io_layers[i]->process_tick)
            r = pn_timestamp_min(
                r, transport->io_layers[i]->process_tick(transport, i, now));
    }
    return r;
}

uint64_t pn_transport_get_frames_output(const pn_transport_t *transport) {
    if (transport) return transport->output_frames_ct;
    return 0;
}

uint64_t pn_transport_get_frames_input(const pn_transport_t *transport) {
    if (transport) return transport->input_frames_ct;
    return 0;
}
...
ssize_t pn_transport_capacity(pn_transport_t *transport) /* <0 == done */
{
    ...
    if (capacity <= 0) {
        ...
        if (!transport->local_max_frame) { // no limit (ha!)
            ...
            more = pn_min(transport->input_size,
                          transport->local_max_frame - transport->input_size);
        }
        if (more) {
...
ssize_t pn_transport_push(pn_transport_t *transport, const char *src,
                          size_t size) {
    assert(transport);
...
void pn_transport_pop(pn_transport_t *transport, size_t size) {
    if (transport) {
        ...
        if (transport->output_pending == 0 &&
            pn_transport_pending(transport) < 0) {
            // TODO: It looks to me that this is a NOP as iff we ever get here
            // TODO: pni_close_head() will always have been already called
            // before leaving pn_transport_pending()
            pni_close_head(transport);
        }
    }
}
...
int pn_transport_close_head(pn_transport_t *transport) {
    ssize_t pending = pn_transport_pending(transport);
    pni_close_head(transport);
    if (pending > 0) pn_transport_pop(transport, pending);
    return 0;
}
...
bool pn_transport_head_closed(pn_transport_t *transport) {
    return transport->head_closed;
}

bool pn_transport_tail_closed(pn_transport_t *transport) {
    return transport->tail_closed;
}
