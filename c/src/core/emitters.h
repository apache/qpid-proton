...
static inline pni_compound_context emit_list(pni_emitter_t* emitter, pni_compound_context* compound, bool small_encoding) {
  if (small_encoding) {
...
static inline void emit_atom(pni_emitter_t* emitter, pni_compound_context* compound, pn_atom_t* atom) {
  switch (atom->type) {
    ...
    case PN_UUID:
      ...
      pni_emitter_writef128(emitter, &atom->u.as_uuid);
      compound->count++;
      return;
    case PN_BINARY:
