...
static uint8_t pn_type2code(pn_encoder_t *encoder, pn_type_t type) {
    ...
    case PN_DOUBLE:
        return PNE_DOUBLE;
    case PN_DECIMAL32:
        return PNE_DECIMAL32;
    case PN_DECIMAL64:
        return PNE_DECIMAL64;
    case PN_DECIMAL128:
        return PNE_DECIMAL128;
    case PN_UUID:
        return PNE_UUID;
    case PN_ULONG:
        return PNE_ULONG;
...
static inline void pn_encoder_writev8(pn_encoder_t *encoder,
                                      const pn_bytes_t *value) {
...
static inline void pn_encoder_writev32(pn_encoder_t *encoder,
                                       const pn_bytes_t *value) {
...
static bool pn_is_in_array(pn_data_t *data, pni_node_t *parent,
                           pni_node_t *node) {
 ...
 */
static bool pn_is_first_in_array(pn_data_t *data, pni_node_t *parent,
                                 pni_node_t *node) {
 ...
 */
static bool pn_is_in_described_list(pn_data_t *data, pni_node_t *parent,
                                    pni_node_t *node) {
...
static int pni_encoder_enter(void *ctx, pn_data_t *data, pni_node_t *node) {
    ...
    case PNE_FALSE:
        return 0;
    case PNE_BOOLEAN:
        pn_encoder_writef8(encoder, atom->u.as_bool);
        return 0;
    case PNE_UBYTE:
        pn_encoder_writef8(encoder, atom->u.as_ubyte);
        return 0;
    case PNE_BYTE:
        pn_encoder_writef8(encoder, atom->u.as_byte);
        return 0;
    case PNE_USHORT:
        pn_encoder_writef16(encoder, atom->u.as_ushort);
        return 0;
    case PNE_SHORT:
        pn_encoder_writef16(encoder, atom->u.as_short);
        return 0;
    case PNE_UINT0:
        return 0;
    case PNE_SMALLUINT:
        pn_encoder_writef8(encoder, atom->u.as_uint);
        return 0;
    case PNE_UINT:
        pn_encoder_writef32(encoder, atom->u.as_uint);
        return 0;
    case PNE_SMALLINT:
        pn_encoder_writef8(encoder, atom->u.as_int);
        return 0;
    case PNE_INT:
        pn_encoder_writef32(encoder, atom->u.as_int);
        return 0;
    case PNE_UTF32:
        pn_encoder_writef32(encoder, atom->u.as_char);
        return 0;
    case PNE_ULONG:
        pn_encoder_writef64(encoder, atom->u.as_ulong);
        return 0;
    case PNE_SMALLULONG:
        pn_encoder_writef8(encoder, atom->u.as_ulong);
        return 0;
    case PNE_LONG:
        pn_encoder_writef64(encoder, atom->u.as_long);
        return 0;
    case PNE_SMALLLONG:
        pn_encoder_writef8(encoder, atom->u.as_long);
        return 0;
    case PNE_MS64:
        pn_encoder_writef64(encoder, atom->u.as_timestamp);
        return 0;
    case PNE_FLOAT:
        c.f = atom->u.as_float;
        pn_encoder_writef32(encoder, c.i);
        return 0;
    case PNE_DOUBLE:
        c.d = atom->u.as_double;
        pn_encoder_writef64(encoder, c.l);
        return 0;
    case PNE_DECIMAL32:
        pn_encoder_writef32(encoder, atom->u.as_decimal32);
        return 0;
    case PNE_DECIMAL64:
        pn_encoder_writef64(encoder, atom->u.as_decimal64);
        return 0;
    case PNE_DECIMAL128:
    ...
    case PNE_UUID:
        ...
        return 0;
    case PNE_VBIN8:
        pn_encoder_writev8(encoder, &atom->u.as_bytes);
        return 0;
    case PNE_VBIN32:
        pn_encoder_writev32(encoder, &atom->u.as_bytes);
        return 0;
    case PNE_STR8_UTF8:
        pn_encoder_writev8(encoder, &atom->u.as_bytes);
        return 0;
    ...
    case PNE_ARRAY32:
        ...
        encoder->position += 4;
        pn_encoder_writef32(encoder, node->described ? node->children - 1
                                                     : node->children);
        if (node->described) pn_encoder_writef8(encoder, 0);
        return 0;
    case PNE_LIST32:
    case PNE_MAP32:
        node->start = encoder->position;
...
static int pni_encoder_exit(void *ctx, pn_data_t *data, pni_node_t *node) {
    ...
    if (node->atom.type == PN_LIST &&
        node->children - encoder->null_count == 0) {
    ...
    case PN_MAP:
        ...
        if (node->small) {
            ...
            if (encoder->null_count) {
                pn_encoder_writef8(encoder,
                                   node->children - encoder->null_count);
            }
        } else {
            // backfill size
...
ssize_t pn_encoder_encode(pn_encoder_t *encoder, pn_data_t *src, char *dst,
                          size_t size) {
    encoder->output = dst;
    ...
    if (encoded > size) {
        pn_error_format(pn_data_error(src), PN_OVERFLOW,
