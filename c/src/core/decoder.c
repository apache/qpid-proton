...
static int pni_decoder_decode_value(pn_decoder_t *decoder, pn_data_t *data,
    ...
    case PNE_MAP32: {
        ...
        case PNE_ARRAY8:
            min_expected_size +=
                1; // Array has a constructor of at least 1 byte
        case PNE_LIST8:
        case PNE_MAP8:
            min_expected_size += 1; // All these types have a count
            if (pn_decoder_remaining(decoder) < min_expected_size + 1)
                return PN_UNDERFLOW;
            size = pn_decoder_readf8(decoder);
        ...
        case PNE_MAP32:
            ...
            if (pn_decoder_remaining(decoder) < min_expected_size + 4)
                return PN_UNDERFLOW;
            size = pn_decoder_readf32(decoder);
            // size must be at least big enough for count or count+constructor
            if (size < min_expected_size) return PN_ARG_ERR;
    ...
    default:
        return pn_error_format(decoder->error, PN_ARG_ERR,
                               "unrecognized typecode: %u", code);
    }
...
int pni_decoder_single_described(pn_decoder_t *decoder, pn_data_t *data) {
    ...
    if (pni_data_parent_type(data) == PN_DESCRIBED &&
        pn_data_siblings(data) > 1) {
        pn_data_exit(data);
    }
    return 0;
}
...
ssize_t pn_decoder_decode(pn_decoder_t *decoder, const char *src, size_t size,
    ...
    if (err == PN_UNDERFLOW)
        return pn_error_format(pn_data_error(dst), PN_UNDERFLOW,
