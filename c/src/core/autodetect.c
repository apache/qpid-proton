...
pni_protocol_type_t pni_sniff_header(const char *buf, size_t len) {
    ...
    bool isSSL3Handshake = buf[0] == 22 && // handshake
                           buf[1] == 3 &&
                           buf[2] <= 3; // SSL 3.0 & TLS 1.0-1.2 (v3.1-3.3)
    ...
    bool isFirst4SSL2ClientHello =
        isFirst3SSL2CLientHello && (buf[3] == 2 || buf[3] == 3);
    if (!isAMQP && !isFirst4SSL2ClientHello) return PNI_PROTOCOL_UNKNOWN;

    if (len < 5) return PNI_PROTOCOL_INSUFFICIENT;
    bool isSSL2Handshake =
        buf[2] == 1 &&                   // MSG-CLIENT-HELLO
...
const char *pni_protocol_name(pni_protocol_type_t p) {
    static const char *names[] = {"Insufficient data to determine protocol",
                                  "Unknown protocol",
                                  "SSL/TLS connection",
                                  "AMQP TLS layer",
                                  "AMQP SASL layer",
