...
struct pn_collector_t {
    ...
    pn_event_t *tail;
    pn_event_t
        *prev; /* event returned by previous call to pn_collector_next() */
    bool freed;
};
...
pn_collector_t *pn_collector(void) { return pn_collector_new(); }
...
pn_event_t *pn_collector_put(pn_collector_t *collector, const pn_class_t *clazz,
                             void *context, pn_event_type_t type) {
    if (!collector) {
        return NULL;
    }
...
pn_event_t *pn_event(void) { return pn_event_new(); }
