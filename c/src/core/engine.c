...
void pn_link_detach(pn_link_t *link) {
    ...
    link->detached = true;
    pn_collector_put(link->session->connection->collector, PN_OBJECT, link,
...
void pn_endpoint_init(pn_endpoint_t *endpoint, int type,
...
void pn_ep_incref(pn_endpoint_t *endpoint) { endpoint->refcount++; }
...
pn_connection_t *pn_connection() {
    ...
    pn_connection_t *conn =
        (pn_connection_t *)pn_class_new(&clazz, sizeof(pn_connection_t));
    if (!conn) return NULL;
...
void pn_connection_collect(pn_connection_t *connection,
    ...
    while (endpoint) {
        pn_collector_put(connection->collector, PN_OBJECT, endpoint,
                         endpoint_init_event_map[endpoint->type]);
...
pn_connection_remote_offered_capabilities(pn_connection_t *connection) {
    assert(connection);
    return connection->transport
               ? connection->transport->remote_offered_capabilities
               : NULL;
}
...
pn_connection_remote_desired_capabilities(pn_connection_t *connection) {
    assert(connection);
    return connection->transport
               ? connection->transport->remote_desired_capabilities
               : NULL;
}

pn_data_t *pn_connection_remote_properties(pn_connection_t *connection) {
    assert(connection);
    return connection->transport ? connection->transport->remote_properties
                                 : NULL;
}

const char *pn_connection_remote_container(pn_connection_t *connection) {
    assert(connection);
    return connection->transport ? connection->transport->remote_container
                                 : NULL;
}

const char *pn_connection_remote_hostname(pn_connection_t *connection) {
    assert(connection);
    return connection->transport ? connection->transport->remote_hostname
                                 : NULL;
}
...
static void pni_clear_work(pn_connection_t *connection,
...
void pn_modified(pn_connection_t *connection, pn_endpoint_t *endpoint,
    ...
    if (emit && connection->transport) {
        pn_collector_put(connection->collector, PN_OBJECT,
                         connection->transport, PN_TRANSPORT);
    }
}
...
static bool pni_matches(pn_endpoint_t *endpoint, pn_endpoint_type_t type,
                        pn_state_t state) {
...
pn_endpoint_t *pn_find(pn_endpoint_t *endpoint, pn_endpoint_type_t type,
                       pn_state_t state) {
    while (endpoint) {
        if (pni_matches(endpoint, type, state)) return endpoint;
        endpoint = endpoint->endpoint_next;
    }
    return NULL;
}
...
pn_link_t *pn_link_head(pn_connection_t *conn, pn_state_t state) {
    ...
    while (endpoint) {
        if (pni_matches(endpoint, SENDER, state) ||
            pni_matches(endpoint, RECEIVER, state))
...
static bool pn_ep_bound(pn_endpoint_t *endpoint) {
    ...
    case SESSION:
        ...
        return (((int16_t)ssn->state.local_channel) >= 0 ||
                ((int16_t)ssn->state.remote_channel) >= 0);
    case SENDER:
    case RECEIVER:
        lnk = (pn_link_t *)endpoint;
        return ((int32_t)lnk->state.local_handle) >= 0 ||
               ((int32_t)lnk->state.remote_handle) >= 0;
    default:
        assert(false);
        return false;
    }
...
static bool pni_preserve_child(pn_endpoint_t *endpoint) {
    ...
    if (pni_endpoint_live(parent) &&
        (!endpoint->freed || (pn_ep_bound(endpoint))) && endpoint->referenced) {
...
#undef pn_session_free
    pn_session_t *ssn =
        (pn_session_t *)pn_class_new(&clazz, sizeof(pn_session_t));
    if (!ssn) return NULL;
...
static void pni_terminus_init(pn_terminus_t *terminus,
...
#undef pn_link_free
    ...
    pn_collector_put(session->connection->collector, PN_OBJECT, link,
                     PN_LINK_INIT);
    if (session->connection->transport) {
        pni_link_bound(link);
    }
...
pn_state_t pn_link_state(pn_link_t *link) { return link->endpoint.state; }

pn_error_t *pn_link_error(pn_link_t *link) { return link->endpoint.error; }
...
static bool pni_preserve_delivery(pn_delivery_t *delivery) {
    ...
    return !delivery->local.settled ||
           (conn->transport && (delivery->state.init || delivery->tpwork));
}
...
static void pn_delivery_finalize(void *object) {
    ...
    if (link) {
        if (pni_link_live(link) && pni_preserve_delivery(delivery) &&
            delivery->referenced) {
...
int pn_delivery_inspect(void *obj, pn_string_t *dst) {
    ...
    int err = pn_string_addf(dst, "pn_delivery<%p>{%s, tag=b\"", obj, dir) ||
...
pn_delivery_t *pn_delivery(pn_link_t *link, pn_delivery_tag_t tag) {
    ...
    if (!link->current) link->current = delivery;
...
int pn_link_unsettled(pn_link_t *link) { return link->unsettled_count; }
...
void pn_delivery_dump(pn_delivery_t *d) {
    ...
    printf(
        "{tag=%s, local.type=%" PRIu64 ", remote.type=%" PRIu64
        ", local.settled=%u, "
        "remote.settled=%u, updated=%u, current=%u, writable=%u, readable=%u, "
        "work=%u}",
        tag, d->local.type, d->remote.type, d->local.settled, d->remote.settled,
        d->updated, pn_delivery_current(d), pn_delivery_writable(d),
        pn_delivery_readable(d), d->work);
}
...
static void pni_advance_sender(pn_link_t *link) {
    ...
    /* Skip accounting if the link is aborted and has not sent any frames.
       A delivery that was aborted before sending the first frame was not
       accounted for in pni_process_tpwork_sender() so we don't need to account
       for it being sent here.
    */
...
int pn_link_credit(pn_link_t *link) { return link ? link->credit : 0; }

int pn_link_available(pn_link_t *link) { return link ? link->available : 0; }

int pn_link_queued(pn_link_t *link) { return link ? link->queued : 0; }
...
pn_snd_settle_mode_t pn_link_snd_settle_mode(pn_link_t *link) {
    return link ? (pn_snd_settle_mode_t)link->snd_settle_mode : PN_SND_MIXED;
}

pn_rcv_settle_mode_t pn_link_rcv_settle_mode(pn_link_t *link) {
    return link ? (pn_rcv_settle_mode_t)link->rcv_settle_mode : PN_RCV_FIRST;
}
...
void pn_link_set_snd_settle_mode(pn_link_t *link, pn_snd_settle_mode_t mode) {
    if (link) link->snd_settle_mode = (uint8_t)mode;
}
void pn_link_set_rcv_settle_mode(pn_link_t *link, pn_rcv_settle_mode_t mode) {
    if (link) link->rcv_settle_mode = (uint8_t)mode;
}
...
bool pn_link_draining(pn_link_t *receiver) {
    ...
    return receiver->drain &&
           (pn_link_credit(receiver) > pn_link_queued(receiver));
}
...
size_t pn_delivery_pending(pn_delivery_t *delivery) {
    /* Aborted deliveries: for clients that don't check pn_delivery_aborted(),
       return 1 rather than 0. This will force them to call pn_link_recv() and
       get the PN_ABORTED error return code.
    */
    if (delivery->aborted) return 1;
    return pn_buffer_size(delivery->bytes);
}

bool pn_delivery_partial(pn_delivery_t *delivery) { return !delivery->done; }
...
bool pn_delivery_aborted(pn_delivery_t *delivery) { return delivery->aborted; }
...
int pn_condition_vformat(pn_condition_t *condition, const char *name,
    ...
    if (n >= sizeof(text)) text[sizeof(text) - 1] = '\0';
    err = pn_condition_set_description(condition, text);
    return err;
}

int pn_condition_format(pn_condition_t *condition, const char *name,
                        const char *fmt, ...) {
    assert(condition);
    va_list ap;
    va_start(ap, fmt);
...
pn_connection_t *pn_event_connection(pn_event_t *event) {
    ...
    default:
        ...
        if (ssn) return pn_session_connection(ssn);
    }
    return NULL;
}
...
pn_link_t *pn_event_link(pn_event_t *event) {
    ...
    default:
        ...
        if (dlv) return pn_delivery_link(dlv);
    }
    return NULL;
}
...
pn_condition_t *pn_event_condition(pn_event_t *e) {
    ...
    case CID_pn_connection: {
        ...
        return cond2_set(pn_connection_remote_condition(c),
                         pn_connection_condition(c));
    }
    case CID_pn_session: {
        pn_session_t *s = (pn_session_t *)ctx;
        return cond2_set(pn_session_remote_condition(s),
                         pn_session_condition(s));
    }
    case CID_pn_link: {
