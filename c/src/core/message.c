...
int pn_message_inspect(void *obj, pn_string_t *dst) {
    ...
    if (msg->expiry_time) {
        err =
            pn_string_addf(dst, "expiry_time=%" PRIi64 ", ", msg->expiry_time);
        if (err) return err;
        comma = true;
    }
...
pn_message_t *pn_message() { return pni_message_new(sizeof(pn_message_t)); }
...
void pn_message_free(pn_message_t *msg) { pn_free(msg); }
...
int pn_message_decode(pn_message_t *msg, const char *bytes, size_t size) {
    ...
    while (size) {
        ...
        case HEADER: {
            bool priority_q;
            uint8_t priority;
            err = pn_data_scan(msg->data, "D.[o?BIoI]", &msg->durable,
                               &priority_q, &priority, &msg->ttl,
                               &msg->first_acquirer, &msg->delivery_count);
            if (err)
                return pn_error_format(msg->error, err, "data error: %s",
                                       pn_error_text(pn_data_error(msg->data)));
        ...
        case PROPERTIES: {
            ...
            pn_data_clear(msg->correlation_id);
            err = pn_data_scan(
                msg->data, "D.[CzSSSCssttSIS]", msg->id, &user_id, &address,
                &subject, &reply_to, msg->correlation_id, &ctype, &cencoding,
            ...
            if (err)
                return pn_error_format(msg->error, err,
                                       "error setting user_id");
...
int pn_message_data(pn_message_t *msg, pn_data_t *data) {
    ...
    int err = pn_data_fill(
        data, "DL[?o?B?I?o?I]", HEADER, msg->durable, msg->durable,
        msg->priority != HEADER_PRIORITY_DEFAULT, msg->priority, (bool)msg->ttl,
        msg->ttl, msg->first_acquirer, msg->first_acquirer,
    ...
    err = pn_data_fill(
        data, "DL[CzSSSCss?t?tS?IS]", PROPERTIES, msg->id,
        pn_string_size(msg->user_id), pn_string_get(msg->user_id),
        pn_string_get(msg->address), pn_string_get(msg->subject),
        pn_string_get(msg->reply_to), msg->correlation_id,
        pn_string_get(msg->content_type), pn_string_get(msg->content_encoding),
        (bool)msg->expiry_time, msg->expiry_time, (bool)msg->creation_time,
        msg->creation_time, pn_string_get(msg->group_id),
        /*
         ...
         * group semantics
         */
        (bool)pn_string_get(msg->group_id) || (bool)msg->group_sequence,
        msg->group_sequence, pn_string_get(msg->reply_to_group_id));
    if (err)
        return pn_error_format(msg->error, err, "data error: %s",
                               pn_error_text(pn_data_error(data)));
...
ssize_t pn_message_encode2(pn_message_t *msg, pn_rwbytes_t *buffer) {
    ...
    size = buffer->size;
    while ((err = pn_message_encode(msg, buffer->start, &size)) ==
           PN_OVERFLOW) {
        buffer->size *= 2;
...
ssize_t pn_message_send(pn_message_t *msg, pn_link_t *sender,
                        pn_rwbytes_t *buffer) {
    ...
    if (ret >= 0) {
        ...
        if (ret < 0)
            pn_error_copy(pn_message_error(msg), pn_link_error(sender));
    }
