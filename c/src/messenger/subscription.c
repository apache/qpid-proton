...
pn_subscription_t *pn_subscription(pn_messenger_t *messenger,
    ...
    pn_subscription_t *sub =
        (pn_subscription_t *)pn_class_new(&clazz, sizeof(pn_subscription_t));
...
int pni_subscription_set_address(pn_subscription_t *sub, const char *address) {
    ...
    if (absolute) {
        ...
        if (scheme) {
            int e =
                pn_string_addf(sub->address, "%s:", pn_string_get(sub->scheme));
            if (e) return e;
        }
        if (pn_string_get(sub->host)) {
            int e = pn_string_addf(sub->address, scheme ? "//%s" : "%s",
                                   pn_string_get(sub->host));
            if (e) return e;
        }
        if (pn_string_get(sub->port)) {
            int e =
                pn_string_addf(sub->address, ":%s", pn_string_get(sub->port));
            if (e) return e;
        }
