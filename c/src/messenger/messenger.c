...
static pn_connection_ctx_t *pni_context(pn_selectable_t *sel) {
    ...
    pn_connection_ctx_t *ctx =
        (pn_connection_ctx_t *)pni_selectable_get_context(sel);
    assert(ctx);
    return ctx;
}
...
void pni_lnr_modified(pn_listener_ctx_t *lnr) { pni_modified((pn_ctx_t *)lnr); }
...
static void pni_messenger_reclaim(pn_messenger_t *messenger,
...
pn_connection_t *pn_messenger_connection(pn_messenger_t *messenger,
                                         pn_socket_t sock, const char *scheme,
                                         char *user, char *pass, char *host,
                                         char *port, pn_listener_ctx_t *lnr);

static void pni_listener_readable(pn_selectable_t *sel) {
    pn_listener_ctx_t *ctx =
        (pn_listener_ctx_t *)pni_selectable_get_context(sel);
    pn_subscription_t *sub = ctx->subscription;
    const char *scheme = pn_subscription_scheme(sub);
    char name[1024];
    pn_socket_t sock =
        pn_accept(ctx->messenger->io, pn_selectable_get_fd(sel), name, 1024);
    ...
    pn_connection_t *conn = pn_messenger_connection(
        ctx->messenger, sock, scheme, NULL, NULL, NULL, NULL, ctx);
    pn_transport_bind(t, conn);
    pn_decref(t);
    pni_conn_modified((pn_connection_ctx_t *)pn_connection_get_context(conn));
}

static void pn_listener_ctx_free(pn_messenger_t *messenger,
                                 pn_listener_ctx_t *ctx);
...
static pn_listener_ctx_t *pn_listener_ctx(pn_messenger_t *messenger,
                                          ...
                                          const char *port) {
    pn_socket_t socket =
        pn_listen(messenger->io, host, port ? port : default_port(scheme));
    ...
    pn_listener_ctx_t *ctx =
        (pn_listener_ctx_t *)pn_class_new(PN_OBJECT, sizeof(pn_listener_ctx_t));
    ctx->messenger = messenger;
    ctx->domain = pn_ssl_domain(PN_SSL_MODE_SERVER);
    if (messenger->certificate) {
        int err = pn_ssl_domain_set_credentials(
            ctx->domain, messenger->certificate, messenger->private_key,
            messenger->password);
...
static pn_connection_ctx_t *
pn_connection_ctx(pn_messenger_t *messenger, pn_connection_t *conn,
                  pn_socket_t sock, const char *scheme, const char *user,
                  const char *pass, const char *host, const char *port,
                  pn_listener_ctx_t *lnr) {
    pn_connection_ctx_t *ctx =
        (pn_connection_ctx_t *)pn_connection_get_context(conn);
    assert(!ctx);
...
#define pn_tracker_sequence(tracker)                                           \
    ((pn_sequence_t)((tracker) & (0x00000000FFFFFFFF)))
...
static char *build_name(const char *name) {
    ...
    sprintf(generated, uuid_fmt, bytes[0], bytes[1], bytes[2], bytes[3],
...
static void link_ctx_setup(pn_messenger_t *messenger,
                           pn_connection_t *connection, pn_link_t *link) {
...
static void pni_interruptor_readable(pn_selectable_t *sel) {
    pn_messenger_t *messenger =
        (pn_messenger_t *)pni_selectable_get_context(sel);
    char buf[1024];
...
pn_messenger_t *pn_messenger(const char *name) {
    ...
    if (m) {
        ...
        // pipe fails, if we don't do this m->ctrl[0] could default to 0 - which
        // is stdin.
        m->ctrl[0] = -1;
        m->ctrl[1] = -1;
        pn_pipe(m->io, m->ctrl);
...
static void pni_reclaim(pn_messenger_t *messenger) {
    while (pn_list_size(messenger->listeners)) {
        pn_listener_ctx_t *l =
            (pn_listener_ctx_t *)pn_list_get(messenger->listeners, 0);
        pn_listener_ctx_free(messenger, l);
    }

    while (pn_list_size(messenger->connections)) {
        pn_connection_t *c =
            (pn_connection_t *)pn_list_get(messenger->connections, 0);
...
bool pn_messenger_flow(pn_messenger_t *messenger) {
    ...
    if (messenger->credit_mode == LINK_CREDIT_AUTO) {
        ...
        const int used =
            messenger->distributed + pn_messenger_incoming(messenger);
        if (max > used) messenger->credit = max - used;
        ...
        if (!messenger->draining) {
            ...
            if (messenger->next_drain == 0) {
                ...
                for (size_t i = 0; i < pn_list_size(messenger->credited); i++) {
                    pn_link_t *link =
                        (pn_link_t *)pn_list_get(messenger->credited, i);
                    if (!pn_link_get_drain(link)) {
...
static int pn_transport_config(pn_messenger_t *messenger,
    ...
    if (ctx->scheme && !strcmp(ctx->scheme, "amqps")) {
        ...
        if (messenger->trusted_certificates) {
            int err = pn_ssl_domain_set_trusted_ca_db(
                d, messenger->trusted_certificates);
            if (err) {
                pn_ssl_domain_free(d);
...
static void pn_condition_report(const char *pfx, pn_condition_t *condition) {
    if (pn_condition_is_redirect(condition)) {
        pn_logf("%s NOTICE (%s) redirecting to %s:%i", pfx,
        ...
        char error[1024];
        pni_snprintf(error, 1024, "(%s) %s", pn_condition_get_name(condition),
                     pn_condition_get_description(condition));
        pn_error_report(pfx, error);
    }
}

int pni_pump_in(pn_messenger_t *messenger, const char *address,
                pn_link_t *receiver) {
    ...
    if (err)
        return pn_error_format(messenger->error, err,
                               "get: error growing buffer");
    char *encoded = pn_buffer_memory(buf).start;
    ssize_t n = pn_link_recv(receiver, encoded, pending);
    if (n != (ssize_t)pending) {
        return pn_error_format(
            messenger->error, n,
            "didn't receive pending bytes: %" PN_ZI " %" PN_ZI, n, pending);
    }
    ...
    if (messenger->credit_mode != LINK_CREDIT_MANUAL) {
        ...
        if (!pn_link_get_drain(link) && pn_list_size(messenger->blocked) == 0 &&
            ...
            if (pn_link_remote_credit(link) < lo_thresh) {
                const int more = pn_min(messenger->credit,
                                        max - pn_link_remote_credit(link));
                messenger->credit -= more;
                messenger->distributed += more;
                pn_link_flow(link, more);
            }
...
void pn_messenger_process_connection(pn_messenger_t *messenger,
                                     pn_event_t *event) {
...
void pn_messenger_process_session(pn_messenger_t *messenger,
                                  pn_event_t *event) {
...
void pn_messenger_process_link(pn_messenger_t *messenger, pn_event_t *event) {
    ...
    if (pn_link_state(link) & PN_LOCAL_UNINIT) {
        ...
        if (pn_link_is_receiver(link)) {
            ...
            ((pn_link_ctx_t *)pn_link_get_context(link))->subscription =
                lnr ? lnr->subscription : NULL;
        }
    }
...
void pn_messenger_process_delivery(pn_messenger_t *messenger,
                                   pn_event_t *event) {
    ...
    if (pn_delivery_readable(d)) {
        int err = pni_pump_in(
            messenger, pn_terminus_get_address(pn_link_source(link)), link);
        if (err) {
...
void pn_messenger_process_transport(pn_messenger_t *messenger,
                                    pn_event_t *event) {
    pn_connection_t *conn = pn_event_connection(event);
    pn_connection_ctx_t *ctx =
        (pn_connection_ctx_t *)pn_connection_get_context(conn);
        ...
        if (transport) {
            ...
            if (pending > 0) {
                pn_connection_ctx_t *cctx =
                    (pn_connection_ctx_t *)pn_connection_get_context(
                        connection);
                pn_messenger_process_events(messenger);
                pn_messenger_flow(messenger);
...
int pn_messenger_tsync(pn_messenger_t *messenger,
                       bool (*predicate)(pn_messenger_t *), int timeout) {
    ...
    if (messenger->flags & PN_FLAGS_CHECK_ROUTES) {
        ...
        for (size_t i = 0; i < pn_list_size(substitutions) && error == 0; i++) {
            pn_string_t *substitution =
                (pn_string_t *)pn_list_get(substitutions, i);
            if (substitution) {
                pn_address_t addr;
                ...
                if (!error) {
                    ...
                    if (addr.scheme && strlen(addr.scheme) > 0 &&
                        !strstr(addr.scheme, "$") && addr.host &&
                        strlen(addr.host) > 0 && !strstr(addr.host, "$") &&
                        addr.port && strlen(addr.port) > 0 &&
                        !strstr(addr.port, "$")) {
                        pn_string_t *check_addr = pn_string(NULL);
                        // ipv6 hosts need to be wrapped in [] within a URI
                        if (strstr(addr.host, ":")) {
                            pn_string_format(check_addr, "%s://[%s]:%s/",
                        ...
                        if (!connection) {
                            if (pn_error_code(messenger->error) == 0)
                                pn_error_copy(messenger->error,
                                              pn_io_error(messenger->io));
                            pn_error_format(messenger->error, PN_ERR,
                                            "CONNECTION ERROR (%s:%s): %s\n",
                                            messenger->address.host,
                                            messenger->address.port,
                                            pn_error_text(messenger->error));
                            error = pn_error_code(messenger->error);
                        } else {
                            // Send and receive outstanding messages until
                            // connection completes or an error occurs
                            int work = pn_messenger_work(messenger, -1);
                            pn_connection_ctx_t *cctx = (pn_connection_ctx_t *)
                                pn_connection_get_context(connection);
                            while ((work > 0 ||
                                    (pn_connection_state(connection) &
                                     PN_REMOTE_UNINIT) ||
                                    pni_connection_pending(cctx->selectable) !=
                                        (ssize_t)0) &&
...
static void pni_parse(pn_address_t *address) {
    ...
    pni_parse_url(pn_string_buffer(address->text), &address->scheme,
                  &address->user, &address->pass, &address->host,
                  &address->port, &address->name);
    if (address->host[0] == '~') {
...
pn_connection_t *pn_messenger_resolve(pn_messenger_t *messenger,
    ...
    *name = messenger->address.name;
    ...
    pn_socket_t sock =
        pn_connect(messenger->io, host, port ? port : default_port(scheme));
    ...
    pn_connection_t *connection = pn_messenger_connection(
        messenger, sock, scheme, user, pass, host, port, NULL);
...
pn_link_t *pn_messenger_get_link(pn_messenger_t *messenger, const char *address,
                                 bool sender) {
    char *name = NULL;
    pn_connection_t *connection =
        pn_messenger_resolve(messenger, address, &name);
    if (!connection) return NULL;
...
pn_link_t *pn_messenger_link(pn_messenger_t *messenger, const char *address,
    ...
    if ((sender && pn_messenger_get_outgoing_window(messenger)) ||
        ...
        if (messenger->snd_settle_mode ==
            -1) { /* Choose default based on sender/receiver */
            /* For a sender use MIXED so the application can decide whether each
               message is settled or not. For a receiver request UNSETTLED,
               since the user set an incoming_window which means they want to
               decide settlement.
            */
            ...
            pn_link_set_snd_settle_mode(
                link, (pn_snd_settle_mode_t)messenger->snd_settle_mode);
        }
    ...
    if (timeout > 0) {
        pn_terminus_set_expiry_policy(pn_link_target(link),
                                      PN_EXPIRE_WITH_LINK);
        pn_terminus_set_expiry_policy(pn_link_source(link),
                                      PN_EXPIRE_WITH_LINK);
    ...
    if (!sender) {
        ...
        assert(ctx);
        ctx->subscription =
            pn_subscription(messenger, cctx->scheme, cctx->host, cctx->port);
    }
    pn_link_open(link);
    return link;
}
...
static void pni_default_rewrite(pn_messenger_t *messenger, const char *address,
    ...
    if (address && strstr(address, "@")) {
        ...
        if (addr->user || addr->pass) {
            pn_string_format(
                messenger->rewritten, "%s%s%s%s%s%s%s",
                addr->scheme ? addr->scheme : "", addr->scheme ? "://" : "",
                addr->host, addr->port ? ":" : "", addr->port ? addr->port : "",
...
static void pni_rewrite(pn_messenger_t *messenger, pn_message_t *msg) {
    ...
    int err =
        pn_transform_apply(messenger->rewrites, address, messenger->rewritten);
    if (err) assert(false);
...
int pn_messenger_put(pn_messenger_t *messenger, pn_message_t *msg) {
    ...
    if (!entry) return pn_error_format(messenger->error, PN_ERR, "store error");
...
static bool work_pred(pn_messenger_t *messenger) { return messenger->worked; }
...
int pn_messenger_send(pn_messenger_t *messenger, int n) {
    if (n == -1) {
        ...
        if (messenger->send_threshold < 0) messenger->send_threshold = 0;
    }
...
int pn_messenger_recv(pn_messenger_t *messenger, int n) {
    ...
    if (messenger->blocking && !pn_list_size(messenger->listeners) &&
        ...
        return pn_error_format(messenger->error, PN_STATE_ERR,
                               "no valid sources");
    ...
    if (err) return err;
    if (!pn_messenger_incoming(messenger) && messenger->blocking &&
        !pn_list_size(messenger->listeners) &&
        !pn_list_size(messenger->connections)) {
        return pn_error_format(messenger->error, PN_STATE_ERR,
                               "no valid sources");
...
int pn_messenger_get(pn_messenger_t *messenger, pn_message_t *msg) {
    ...
    messenger->incoming_subscription =
        (pn_subscription_t *)pni_entry_get_context(entry);
...
int pn_messenger_outgoing(pn_messenger_t *messenger) {
    return pni_store_size(messenger->outgoing) +
           pn_messenger_queued(messenger, true);
}

int pn_messenger_incoming(pn_messenger_t *messenger) {
    return pni_store_size(messenger->incoming) +
           pn_messenger_queued(messenger, false);
}
...
int pn_messenger_rewrite(pn_messenger_t *messenger, const char *pattern,
                         const char *address) {
...
int pn_messenger_set_flags(pn_messenger_t *messenger, const int flags) {
    ...
    if (flags == 0) {
        messenger->flags = 0;
               ...
               (PN_FLAGS_CHECK_ROUTES | PN_FLAGS_ALLOW_INSECURE_MECHS)) {
...
pn_millis_t pn_messenger_get_remote_idle_timeout(pn_messenger_t *messenger,
    ...
    for (size_t i = 0; i < pn_list_size(messenger->connections); i++) {
        ...
        pn_connection_ctx_t *ctx =
            (pn_connection_ctx_t *)pn_connection_get_context(connection);
        if (pn_streq(addr.scheme, ctx->scheme) &&
            pn_streq(addr.host, ctx->host) && pn_streq(addr.port, ctx->port)) {
            pn_transport_t *transport = pn_connection_transport(connection);
            if (transport)
...
int pn_messenger_set_ssl_peer_authentication_mode(
    pn_messenger_t *messenger, const pn_ssl_verify_mode_t mode) {
    if (!messenger) return PN_ARG_ERR;
