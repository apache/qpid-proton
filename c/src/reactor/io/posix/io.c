...
pn_socket_t pn_accept(pn_io_t *io, pn_socket_t socket, char *name,
                      size_t size) {
    ...
    *name = '\0';
    ...
    if (sock == PN_INVALID_SOCKET) {
        ...
        int code;
        if ((code = getnameinfo((struct sockaddr *)&addr, addrlen, io->host,
                                MAX_HOST, io->serv, MAX_SERV, 0))) {
            pn_error_format(io->error, PN_ERR, "getnameinfo: %s\n",
                            gai_strerror(code));
            if (close(sock) == -1) pn_i_error_from_errno(io->error, "close");
            return PN_INVALID_SOCKET;
        } else {
            pn_configure_sock(io, sock);
...
ssize_t pn_send(pn_io_t *io, pn_socket_t socket, const void *buf, size_t len) {
    ...
    if (count < 0) {
        pn_i_error_from_errno(io->error, "send");
    }
    return count;
}
...
static inline int pn_create_socket(int af, int protocol) {
    ...
    int optval = 1;
    if (setsockopt(sock, SOL_SOCKET, SO_NOSIGPIPE, &optval, sizeof(optval)) ==
        -1) {
        close(sock);
        return -1;
    }
...
static ssize_t nosigpipe_send(int fd, const void *buffer, size_t size) {
    ...
    if (!sigpipeIsPending) {
        ...
        if (sigmaskErr =
                pthread_sigmask(SIG_BLOCK, (const sigset_t *)&newSignals,
                                (sigset_t *)&oldSignals)) {
            errno = sigmaskErr;
            return -1;
        }
    ...
    if (!sigpipeIsPending) {
        ...
        if (count == -1 && errno == EPIPE) {
            while (-1 == sigtimedwait(&newSignals, NULL,
                                      &(struct timespec){0, 0}) &&
                   errno == EINTR)
                ; // do nothing
        }
        if (sigmaskErr = pthread_sigmask(
                SIG_SETMASK, (const sigset_t *)&oldSignals, (sigset_t *)NULL)) {
            errno = sigmaskErr;
            return -1;
        }
        errno = sendErrno;
...
ssize_t pn_recv(pn_io_t *io, pn_socket_t socket, void *buf, size_t size) {
    ...
    if (count < 0) {
        pn_i_error_from_errno(io->error, "recv");
    }
    return count;
}
...
void pn_close(pn_io_t *io, pn_socket_t socket) { close(socket); }

bool pn_wouldblock(pn_io_t *io) { return io->wouldblock; }

pn_selector_t *pn_io_selector(pn_io_t *io) {
    if (io->selector == NULL) io->selector = pni_selector();
    return io->selector;
}
