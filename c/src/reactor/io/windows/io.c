...
void pn_io_free(pn_io_t *io) { pn_free(io); }
...
static void ensure_unique(pn_io_t *io, pn_socket_t new_socket) {
    ...
    if (iocpd) {
        if (io->trace) io_log("Stale external socket reference discarded\n");
        ...
        pni_iocpdesc_map_del(
            io->iocp, sock); // may free the iocpdesc_t depending on refcount
    }
}
...
pn_socket_t pn_connect(pn_io_t *io, const char *hostarg, const char *port) {
    ...
    int code = getaddrinfo(host, amqp_service(port), NULL, &addr);
    if (code) {
        pn_error_format(io->error, PN_ERR, "getaddrinfo(%s, %s): %s", host,
                        port, gai_strerror(code));
        return INVALID_SOCKET;
    }
    ...
    pn_socket_t sock = pni_create_socket(addr->ai_family, addr->ai_protocol);
    if (sock == INVALID_SOCKET) {
        pni_win32_error(io->error, "proton pni_create_socket",
                        WSAGetLastError());
        freeaddrinfo(addr);
        return INVALID_SOCKET;
    }
...
pn_socket_t pn_accept(pn_io_t *io, pn_socket_t listen_sock, char *name,
                      size_t size) {
    ...
    *name = '\0';
    if (listend)
        accept_sock = pni_iocp_end_accept(listend, (struct sockaddr *)&addr,
                                          &addrlen, &io->wouldblock, io->error);
    else {
    ...
    if (accept_sock == INVALID_SOCKET) return accept_sock;

    int code = getnameinfo((struct sockaddr *)&addr, addrlen, io->host,
                           NI_MAXHOST, io->serv, NI_MAXSERV, 0);
    if (code)
        code =
            getnameinfo((struct sockaddr *)&addr, addrlen, io->host, NI_MAXHOST,
...
ssize_t pn_send(pn_io_t *io, pn_socket_t sockfd, const void *buf, size_t len) {
    ...
    if (iocpd) {
        count =
            pni_iocp_begin_write(iocpd, buf, len, &io->wouldblock, io->error);
    } else {
...
ssize_t pn_write(pn_io_t *io, pn_socket_t socket, const void *buf,
                 size_t size) {
...
bool pn_wouldblock(pn_io_t *io) { return io->wouldblock; }
...
static int pni_socket_pair(pn_io_t *io, SOCKET sv[2]) {
    ...
    BOOL b = 1;
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&b,
                   sizeof(b)) == -1) {
        perror("setsockopt");
        closesocket(sock);
        return -1;
