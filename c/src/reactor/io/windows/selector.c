...
pn_selector_t *pni_selector() {
    ...
    pn_selector_t *selector =
        (pn_selector_t *)pn_class_new(&clazz, sizeof(pn_selector_t));
    return selector;
}
...
void pn_selector_update(pn_selector_t *selector, pn_selectable_t *selectable) {
    // A selectable's fd may switch from PN_INVALID_SCOKET to a working socket
    // between update calls.  If a selectable without a valid socket has a
    // deadline, we need a dummy iocpdesc_t to participate in the deadlines
    // list.
    ...
    pn_socket_t sock = pn_selectable_get_fd(selectable);
    iocpdesc_t *iocpd =
        (iocpdesc_t *)pn_list_get(selector->iocp_descriptors, idx);
    ...
    if (!iocpd && sock != PN_INVALID_SOCKET) {
        ...
        if (!iocpd) {
            ...
            assert(iocpd);
            if (iocpd) pni_iocpdesc_start(iocpd);
        }
        if (iocpd) {
            pn_list_set(selector->iocp_descriptors, idx, iocpd);
            iocpd->selector = selector;
...
void pn_selector_remove(pn_selector_t *selector, pn_selectable_t *selectable) {
    ...
    if (iocpd) {
        ...
        deadlines_update(iocpd, 0);
        assert(selector->triggered_list_head != iocpd &&
               !iocpd->triggered_list_prev);
    ...
    for (size_t i = idx; i < size; i++) {
        pn_selectable_t *sel =
            (pn_selectable_t *)pn_list_get(selector->selectables, i);
...
int pn_selector_select(pn_selector_t *selector, int timeout) {
    ...
    // Process all currently available completions, even if matched events
    // available
    ...
    while (true) {
        if (selector->triggered_list_head) break;
        ...
        if (zd)
            completion_deadline =
                completion_deadline ? pn_min(zd, completion_deadline) : zd;

        int completion_timeout =
            (!completion_deadline) ? -1 : completion_deadline - now;
        int rv = pni_iocp_wait_one(selector->iocp, completion_timeout,
                                   selector->error);
        if (rv < 0) return pn_error_code(selector->error);
    ...
    selector->current = 0;
    selector->awoken = now;
    for (iocpdesc_t *iocpd = selector->deadlines_head; iocpd;
         iocpd = iocpd->deadlines_next) {
...
static void interests_update(iocpdesc_t *iocpd, int interests) {
    ...
    if (old_interests == interests) return;
...
static void deadlines_update(iocpdesc_t *iocpd, pn_timestamp_t deadline) {
    if (deadline == iocpd->deadline) return;
