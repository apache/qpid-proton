...
static void begin_accept(pni_acceptor_t *acceptor, accept_result_t *result) {
    ...
    if (result->new_sock) {
        ...
        if (!success && WSAGetLastError() != ERROR_IO_PENDING) {
            ...
            pni_events_update(acceptor->listen_sock,
                              acceptor->listen_sock->events | PN_READABLE);
        } else {
...
pn_socket_t pni_iocp_end_accept(iocpdesc_t *ld, sockaddr *addr,
    ...
    if (pn_list_size(ld->acceptor->accepts) == 0) {
        if (ld->events & PN_READABLE && ld->iocp->iocp_trace)
            iocp_log("listen socket readable with no available accept "
                     "completions\n");
        *would_block = true;
        return INVALID_SOCKET;
    }
    ...
    accept_result_t *result =
        (accept_result_t *)pn_list_get(ld->acceptor->accepts, 0);
    ...
    if (result->base.status) {
        ...
        if (ld->iocp->iocp_trace) iocp_log("%s\n", pn_error_text(ld->error));
        ...
        setsockopt(accept_sock, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT,
                   (char *)&ld->socket, sizeof(SOCKET));
        if (addr && addrlen && *addrlen > 0) {
            sockaddr_storage *local_addr = NULL;
            sockaddr_storage *remote_addr = NULL;
            int local_addrlen, remote_addrlen;
            LPFN_GETACCEPTEXSOCKADDRS fn =
                ld->acceptor->fn_get_accept_ex_sockaddrs;
            fn(result->address_buffer, 0, IOCP_SOCKADDRMAXLEN,
               IOCP_SOCKADDRMAXLEN, (SOCKADDR **)&local_addr, &local_addrlen,
...
static connect_result_t *connect_result(iocpdesc_t *iocpd,
 ...
 * consuming peer.
 */
static void complete_write(write_result_t *result, DWORD xfer_count,
                           HRESULT status) {
    iocpdesc_t *iocpd = result->base.iocpd;
    if (iocpd->closing) {
        pni_write_pipeline_return(iocpd->pipeline, result);
...
static void complete_read(read_result_t *result, DWORD xfer_count,
                          HRESULT status) {
    iocpdesc_t *iocpd = result->base.iocpd;
...
static void start_reading(iocpdesc_t *iocpd) { begin_zero_byte_read(iocpd); }
...
static pn_list_t *iocp_map_close_all(iocp_t *iocp) {
    ...
    for (pn_handle_t entry = pn_hash_head(iocp->iocpdesc_map); entry;
        ...
        iocpdesc_t *iocpd =
            (iocpdesc_t *)pn_hash_value(iocp->iocpdesc_map, entry);
        ...
        if (is_listener(iocpd)) {
            if (iocpd->external) {
                ...
                if (iocpd->ops_in_progress) pn_list_add(externals, iocpd);
...
static void iocp_shutdown(iocpdesc_t *iocpd) {
    if (iocpd->socket == PN_INVALID_SOCKET) return; // Hard close in progress
...
void pni_iocp_finalize(void *obj) {
    ...
    // Now everything with ops_in_progress is in the zombie_list or the
    // externals list.
