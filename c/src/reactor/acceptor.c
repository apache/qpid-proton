...
void pni_acceptor_readable(pn_selectable_t *sel) {
    ...
    char name[1024];
    pn_socket_t sock = pn_accept(pni_reactor_io(reactor),
                                 pn_selectable_get_fd(sel), name, 1024);
    pn_handler_t *handler = (pn_handler_t *)pn_record_get(
        pn_selectable_attachments(sel), PNI_ACCEPTOR_HANDLER);
    if (!handler) {
        handler = pn_reactor_get_handler(reactor);
    }
    pn_record_t *record = pn_selectable_attachments(sel);
    pn_ssl_domain_t *ssl_domain =
        (pn_ssl_domain_t *)pn_record_get(record, PNI_ACCEPTOR_SSL_DOMAIN);
    pn_connection_t *conn = pn_reactor_connection(reactor, handler);
    if (name[0]) { // store the peer address of connection in <host>:<port>
                   // format
...
pn_acceptor_t *pn_reactor_acceptor(pn_reactor_t *reactor, const char *host,
                                   const char *port, pn_handler_t *handler) {
...
pn_acceptor_t *pn_connection_acceptor(pn_connection_t *conn) {
    // Return the acceptor that created the connection or NULL if an outbound
    // connection
