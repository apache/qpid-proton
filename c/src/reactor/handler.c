...
pn_handler_t *pn_handler_new(void (*dispatch)(pn_handler_t *, pn_event_t *,
                                              pn_event_type_t),
                             size_t size, void (*finalize)(pn_handler_t *)) {
    static const pn_class_t clazz = PN_CLASS(pn_handler);
    pn_handler_t *handler =
        (pn_handler_t *)pn_class_new(&clazz, sizeof(pn_handler_t) + size);
...
void *pn_handler_mem(pn_handler_t *handler) { return (void *)(handler + 1); }
...
void pn_handler_dispatch(pn_handler_t *handler, pn_event_t *event,
    ...
    if (handler->children) {
        ...
        for (size_t i = 0; i < n; i++) {
            pn_handler_t *child =
                (pn_handler_t *)pn_list_get(handler->children, i);
