...
void pn_reactor_set_global_handler(pn_reactor_t *reactor,
                                   pn_handler_t *handler) {
    assert(reactor);
    pn_decref(reactor->global);
...
static void pni_selectable_release(pn_selectable_t *selectable) {
    pn_reactor_t *reactor =
        (pn_reactor_t *)pni_selectable_get_context(selectable);
...
static void pni_reactor_dispatch_post(pn_reactor_t *reactor,
                                      pn_event_t *event) {
    assert(reactor);
    assert(event);
...
static pn_connection_t *pni_object_connection(const pn_class_t *clazz,
                                              void *object) {
    switch (pn_class_id(clazz)) {
    case CID_pn_delivery:
        return pn_session_connection(
            pn_link_session(pn_delivery_link((pn_delivery_t *)object)));
    case CID_pn_link:
...
pn_reactor_t *pn_class_reactor(const pn_class_t *clazz, void *object) {
    ...
    case CID_pn_transport:
        return pni_record_get_reactor(
            pn_transport_attachments((pn_transport_t *)object));
    case CID_pn_delivery:
    case CID_pn_link:
    case CID_pn_session:
    case CID_pn_connection: {
...
pn_handler_t *pn_event_handler(pn_event_t *event,
    ...
    case CID_pn_task:
        handler = pn_record_get_handler(
            pn_task_attachments((pn_task_t *)pn_event_context(event)));
        if (handler) {
            return handler;
        }
        break;
    case CID_pn_selectable:
        handler = pn_record_get_handler(pn_selectable_attachments(
            (pn_selectable_t *)pn_event_context(event)));
        if (handler) {
            return handler;
        }
        break;
...
pn_task_t *pn_reactor_schedule(pn_reactor_t *reactor, int delay,
                               pn_handler_t *handler) {
    ...
    if (reactor->selectable) {
        pn_selectable_set_deadline(reactor->selectable,
                                   pn_timer_deadline(reactor->timer));
...
bool pn_reactor_quiesced(pn_reactor_t *reactor) {
    ...
    // more than one event
    if (pn_collector_more(reactor->collector)) {
        return false;
    }
...
bool pn_reactor_process(pn_reactor_t *reactor) {
    ...
    while (true) {
        ...
        if (event) {
            ...
            if (previous != PN_REACTOR_QUIESCED &&
                reactor->previous != PN_REACTOR_FINAL) {
