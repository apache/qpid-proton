...
// define two sets of allowable ciphers: those that require authentication, and
// those that do not require authentication (anonymous).  See ciphers(1).
...
static int verify_callback(int preverify_ok, X509_STORE_CTX *ctx) {
    ...
    SSL *ssn = (SSL *)X509_STORE_CTX_get_ex_data(
        ctx, SSL_get_ex_data_X509_STORE_CTX_idx());
    if (!ssn) {
        pn_transport_logf(NULL, "Error: unexpected error - SSL session info "
                                "not available for peer verify!");
        return 0; // fail connection
    }

    pn_transport_t *transport =
        (pn_transport_t *)SSL_get_ex_data(ssn, ssl_ex_data_index);
    if (!transport) {
        pn_transport_logf(NULL, "Error: unexpected error - SSL context info "
                                "not available for peer verify!");
        return 0; // fail connection
    }

    pni_ssl_t *ssl = transport->ssl;
    if (ssl->domain->verify_mode != PN_SSL_VERIFY_PEER_NAME)
        return preverify_ok;
    if (!ssl->peer_hostname) {
        pn_transport_logf(transport,
                          "Error: configuration error: PN_SSL_VERIFY_PEER_NAME "
                          "configured, but no peer hostname set!");
        return 0; // fail connection
    }

    ssl_log(transport, "Checking identifying name in peer cert against '%s'",
            ssl->peer_hostname);
        ...
        for (i = 0; !matched && i < name_ct; ++i) {
            ...
            if (name->type == GEN_DNS) {
                ...
                if (asn1 && asn1->data && asn1->length) {
                    ...
                    if (len >= 0) {
                        ssl_log(transport,
                                "SubjectAltName (dns) from peer cert = '%.*s'",
                                len, str);
                        matched = match_dns_pattern(ssl->peer_hostname,
                                                    (const char *)str, len);
                        OPENSSL_free(str);
                    }
                }
    ...
    while (!matched &&
        ...
        if (name_asn1) {
            ...
            if (len >= 0) {
                ssl_log(transport, "commonName from peer cert = '%.*s'", len,
                        str);
                matched = match_dns_pattern(ssl->peer_hostname,
                                            (const char *)str, len);
                OPENSSL_free(str);
    ...
    if (!matched) {
        ssl_log(transport,
                "Error: no name matching %s found in peer cert - rejecting "
                "handshake.",
                ssl->peer_hostname);
        preverify_ok = 0;
...
static void ssn_restore(pn_transport_t *transport, pni_ssl_t *ssl) {
    ...
    for (int i = ssl_cache_ptr;;) {
        ...
        if (strcmp(ssl_cache[i].id, ssl->session_id) == 0) {
            ssl_log(transport, "Restoring previous session id=%s",
                    ssl->session_id);
            int rc = SSL_set_session(ssl->ssl, ssl_cache[i].session);
            if (rc != 1) {
                ssl_log(transport, "Session restore failed, id=%s",
                        ssl->session_id);
            }
            return;
        }
...
static void ssn_save(pn_transport_t *transport, pni_ssl_t *ssl) {
    if (ssl->session_id) {
        ...
        // So that if we find it in the cache later we can figure out the
        // session id
        ...
        if (session) {
            ...
            if (ssl_cache[ssl_cache_ptr].session)
                SSL_SESSION_free(ssl_cache[ssl_cache_ptr].session);
...
pn_ssl_domain_t *pn_ssl_domain(pn_ssl_mode_t mode) {
    ...
    if (pn_ssl_domain_set_peer_authentication(domain, PN_SSL_ANONYMOUS_PEER,
                                              NULL)) {
        pn_ssl_domain_free(domain);
        return NULL;
    }
...
int pn_ssl_domain_set_credentials(pn_ssl_domain_t *domain,
    ...
    if (SSL_CTX_use_certificate_chain_file(domain->ctx, certificate_file) !=
        1) {
    ...
    if (SSL_CTX_check_private_key(domain->ctx) != 1) {
        ssl_log_error(
            "The key file %s is not consistent with the certificate %s",
    ...
    // anonymous cipher was negotiated.  TLSv1 will reject such a request. Hack:
    // once a cert is configured, allow only authenticated ciphers.
    if (!domain->ciphers &&
        !SSL_CTX_set_cipher_list(domain->ctx, CIPHERS_AUTHENTICATE)) {
...
int pn_ssl_domain_set_protocols(pn_ssl_domain_t *domain,
    ...
    static const struct {
        const char *name;
        const long option;
    } protocol_options[] = {{"TLSv1", SSL_OP_NO_TLSv1},
    ...
    while (*token != 0) {
        ...
        for (size_t i = 0;
             i < sizeof(protocol_options) / sizeof(*protocol_options); ++i) {
...
int pn_ssl_domain_set_trusted_ca_db(pn_ssl_domain_t *domain,
    ...
    // is passed to SSL_CTX_load_verify_locations()
    struct stat sbuf;
    if (stat(certificate_db, &sbuf) != 0) {
        pn_transport_logf(NULL, "stat(%s) failed: %s", certificate_db,
                          strerror(errno));
        return -1;
    }
    ...
    if (SSL_CTX_load_verify_locations(domain->ctx, file, dir) != 1) {
        ssl_log_error("SSL_CTX_load_verify_locations( %s ) failed",
                      certificate_db);
        return -1;
    }
...
int pn_ssl_domain_set_peer_authentication(pn_ssl_domain_t *domain,
    ...
    case PN_SSL_VERIFY_PEER_NAME:
        ...
        if (!domain->has_ca_db) {
            pn_transport_logf(
                NULL, "Error: cannot verify peer without a trusted CA "
                      "configured, use pn_ssl_domain_set_trusted_ca_db()");
            return -1;
        }

        if (domain->mode == PN_SSL_MODE_SERVER) {
            // openssl requires that server connections supply a list of trusted
            // CAs which is sent to the client
            if (!trusted_CAs) {
                pn_transport_logf(
                    NULL, "Error: a list of trusted CAs must be provided.");
                return -1;
            }
            ...
            if (cert_names != NULL)
                ...
                pn_transport_logf(
                    NULL, "Error: Unable to process file of trusted CAs: %s",
                    trusted_CAs);
                return -1;
            }
        }

        SSL_CTX_set_verify(domain->ctx,
...
#if OPENSSL_VERSION_NUMBER >= 0x10100000
                                // Must use lowest OpenSSL security level to
...
const pn_io_layer_t ssl_layer = {process_input_ssl, process_output_ssl,
                                 handle_error_ssl, NULL, buffered_output};

const pn_io_layer_t ssl_input_closed_layer = {
    process_input_done, process_output_ssl, handle_error_ssl, NULL,
    buffered_output};

const pn_io_layer_t ssl_output_closed_layer = {
    process_input_ssl, process_output_done, handle_error_ssl, NULL,
    buffered_output};

const pn_io_layer_t ssl_closed_layer = {process_input_done, process_output_done,
                                        handle_error_ssl, NULL,
                                        buffered_output};
...
int pn_ssl_domain_allow_unsecured_client(pn_ssl_domain_t *domain) {
    if (!domain) return -1;
    if (domain->mode != PN_SSL_MODE_SERVER) {
        pn_transport_logf(NULL,
                          "Cannot permit unsecured clients - not a server.");
        return -1;
    }
...
// take data from the network, and pass it into SSL.  Attempt to read decrypted
// data from SSL socket and pass it to the application.
static ssize_t process_input_ssl(pn_transport_t *transport, unsigned int layer,
    ...
    do {
        ...
        if (available > 0) {
            ...
            if (written > 0) {
                ...
                ssl_log(transport, "Wrote %d bytes to BIO Layer, %d left over",
                        written, available);
            }
        } else if (shutdown_input) {
            // lower layer (caller) has closed.  Close the WRITE side of the
            // BIO.  This will cause an EOF to be passed to SSL once all pending
            // inbound data has been consumed.
            ssl_log(transport,
                    "Lower layer closed - shutting down BIO write side");
        ...
        if (!ssl->ssl_closed && ssl->in_count < ssl->in_size) {
            ...
            if (read > 0) {
                ssl_log(transport, "Read %d bytes from SSL socket for app",
                        read);
                ...
                if (!BIO_should_retry(ssl->bio_ssl)) {
                    ...
                    case SSL_ERROR_ZERO_RETURN:
                        ...
                        start_ssl_shutdown(transport); // KAG: not sure - this
        ...
        if (!ssl->app_input_closed) {
            if (ssl->in_count > 0 ||
                ssl->ssl_closed) { /* if ssl_closed, send 0 count */
                ssize_t consumed =
                    transport->io_layers[layer + 1]->process_input(
                        transport, layer + 1, ssl->inbuf, ssl->in_count);
                if (consumed > 0) {
                    ssl->in_count -= consumed;
                    if (ssl->in_count)
                        memmove(ssl->inbuf, ssl->inbuf + consumed,
                                ssl->in_count);
                    work_pending = true;
                    ssl_log(transport,
                    ...
                    if (ssl->app_output_closed && ssl->out_count == 0) {
                        // both sides of app closed, and no more app output
                        // pending:
                        start_ssl_shutdown(transport);
                    }
                } else {
                    // app did not consume any bytes, must be waiting for a full
                    // frame
                    if (ssl->in_count == ssl->in_size) {
                        // but the buffer is full, not enough room for a full
                        // frame. can we grow the buffer?
                        uint32_t max_frame =
                            pn_transport_get_max_frame(transport);
                        if (!max_frame)
                            max_frame = ssl->in_size * 2; // no limit
                        if (ssl->in_size < max_frame) {
                            // no max frame limit - grow it.
                            size_t newsize =
                                pn_min(max_frame, ssl->in_size * 2);
                            ...
                            // This is a bug - since SSL can buffer up to
                            // max-frame, the application _must_ have enough
                            // data to process.  If this is an oversized frame,
                            // the app _must_ handle it by returning an error
                            // code to SSL.
                            pn_transport_log(
                                transport,
                                "Error: application unable to consume input.");
                        }
                    }
                }
            }
    ...
    // PROTON-82: Instead, close the input side as soon as we've completed
    // enough of the SSL shutdown handshake to send the close_notify.  We're not
    // requiring the response, as some implementations never reply.
    // ---
    // tell transport our input side is closed if the SSL socket cannot be read
    // from any longer, AND any pending input has been written up to the
    // application (or the application is closed)
    // if (ssl->ssl_closed && ssl->app_input_closed) {
    //  consumed = ssl->app_input_closed;
    //}
    if (ssl->app_input_closed &&
        (SSL_get_shutdown(ssl->ssl) & SSL_SENT_SHUTDOWN)) {
        consumed = ssl->app_input_closed;
...
static ssize_t process_output_ssl(pn_transport_t *transport, unsigned int layer,
    ...
    do {
        ...
        if (!ssl->app_output_closed && ssl->out_count < ssl->out_size) {
            ssize_t app_bytes = transport->io_layers[layer + 1]->process_output(
                transport, layer + 1, &ssl->outbuf[ssl->out_count],
                ssl->out_size - ssl->out_count);
            if (app_bytes > 0) {
                ...
                if (app_bytes < 0) {
                    ssl_log(transport,
                            "Application layer closed its output, error=%d (%d "
                            "bytes pending send)",
        ...
        if (!ssl->ssl_closed) {
            ...
            if (ssl->out_count > 0) {
                ...
                if (wrote > 0) {
                    ...
                    ssl->out_count -= wrote;
                    work_pending = true;
                    ssl_log(transport, "Wrote %d bytes from app to socket",
                            wrote);
                } else {
                    if (!BIO_should_retry(ssl->bio_ssl)) {
                        ...
                        case SSL_ERROR_ZERO_RETURN:
                            ...
                            ssl_log(transport, "SSL connection has closed");
                            start_ssl_shutdown(
                                transport); // KAG: not sure - this may not be
                                            // necessary
                            ssl->out_count =
                                0; // can no longer write to socket, so erase
                                   // app output data
                            ssl->ssl_closed = true;
                            break;
                        default:
            ...
            if (ssl->out_count == 0) {
                if (ssl->app_input_closed && ssl->app_output_closed) {
                    // application is done sending/receiving data, and all
                    // buffered output data has been written to the SSL socket
                    start_ssl_shutdown(transport);
                }
    ...
    // PROTON-82: close the output side as soon as we've sent the SSL
    // close_notify. We're not requiring the response, as some implementations
    // never reply.
    // ----
    ...
    //}
    if (written == 0 && (SSL_get_shutdown(ssl->ssl) & SSL_SENT_SHUTDOWN) &&
        BIO_pending(ssl->bio_net_io) == 0) {
        written = ssl->app_output_closed ? ssl->app_output_closed : PN_EOS;
...
pn_ssl_resume_status_t pn_ssl_resume_status(pn_ssl_t *ssl0) {
    ...
    case 0:
        return PN_SSL_RESUME_NEW;
    case 1:
        return PN_SSL_RESUME_REUSED;
    default:
        break;
    }
...
#ifdef SSL_CTRL_SET_TLSEXT_HOSTNAME
        if (ssl->ssl && ssl->domain &&
            ssl->domain->mode == PN_SSL_MODE_CLIENT) {
            SSL_set_tlsext_host_name(ssl->ssl, ssl->peer_hostname);
        }
#endif
    }
...
pn_ssl_get_remote_subject_subfield(pn_ssl_t *ssl0,
    ...
    // common name like this - Subject: CN=www.domain1.com, CN=www.domain2.com,
    // see https://bugzilla.mozilla.org/show_bug.cgi?id=380656 For now, we will
    // only return the first common name if there is more than one common name
    // in the cert
...
static inline unsigned long id_callback(void) {
...
static inline int pni_mutex_init(pni_mutex_t *m) {
    return pthread_mutex_init(m, NULL);
}
static inline int pni_mutex_lock(pni_mutex_t *m) {
    return pthread_mutex_lock(m);
}
static inline int pni_mutex_unlock(pni_mutex_t *m) {
...
static void initialize(void) {
    ...
    ssl_ex_data_index = SSL_get_ex_new_index(
        0, (void *)"org.apache.qpid.proton.ssl", NULL, NULL, NULL);
    ssn_init();
    locks = (pni_mutex_t *)malloc(CRYPTO_num_locks() * sizeof(pni_mutex_t));
    if (!locks) return;
    for (i = 0; i < CRYPTO_num_locks(); i++) pni_mutex_init(&locks[i]);
    CRYPTO_set_id_callback(&id_callback);
    CRYPTO_set_locking_callback(&locking_callback);
    /* In recent versions of openssl, the set_callback functions are no-op
       macros, so we need to take steps to stop the compiler complaining about
       unused functions. */
    (void)&id_callback;
...
/* TODO aconway 2017-10-16: There is no opportunity to clean up the locks as
   proton has no final shut-down call. If it did, we should call this: */
/*
