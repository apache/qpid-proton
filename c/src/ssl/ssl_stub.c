 ...
 * implementation is used if there is no SSL/TLS support in the system's
 * environment.
 */

bool pn_ssl_present(void) { return false; }

pn_ssl_t *pn_ssl(pn_transport_t *transport) { return NULL; }
...
ssize_t pn_ssl_input(pn_transport_t *transport, unsigned int layer,
                     const char *bytes, size_t available) {
    return PN_EOS;
}

ssize_t pn_ssl_output(pn_transport_t *transport, unsigned int layer,
                      char *buffer, size_t max_size) {
    return PN_EOS;
}

const pn_io_layer_t ssl_layer = {pn_ssl_input, pn_ssl_output, NULL, NULL};
...
pn_ssl_domain_t *pn_ssl_domain(pn_ssl_mode_t mode) { return NULL; }
...
int pn_ssl_domain_allow_unsecured_client(pn_ssl_domain_t *domain) { return -1; }
...
bool pn_ssl_allow_unsecured(pn_ssl_t *ssl) { return true; }

pn_ssl_resume_status_t pn_ssl_resume_status(pn_ssl_t *s) {
    return PN_SSL_RESUME_UNKNOWN;
}

int pn_ssl_set_peer_hostname(pn_ssl_t *ssl, const char *hostname) { return -1; }

int pn_ssl_get_peer_hostname(pn_ssl_t *ssl, char *hostname, size_t *bufsize) {
    return -1;
}

const char *pn_ssl_get_remote_subject(pn_ssl_t *ssl) { return NULL; }

int pn_ssl_get_ssf(pn_ssl_t *ssl) { return 0; }
