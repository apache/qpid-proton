...
static HCERTSTORE open_cert_db(const char *store_name, const char *passwd,
                               int *error);
...
struct win_credential_t {
    ...
    HCERTSTORE server_CA_certs;  // CAs advertised by server (may be a duplicate
                                 // of the trust_store)
...
static void win_credential_finalize(void *object) {
    ...
    if (c->cert_context) CertFreeCertificateContext(c->cert_context);
    if (c->trust_store) CertCloseStore(c->trust_store, 0);
    if (c->server_CA_certs) CertCloseStore(c->server_CA_certs, 0);
...
static win_credential_t *win_credential(pn_ssl_mode_t m) {
    ...
    win_credential_t *c =
        (win_credential_t *)pn_class_new(&clazz, sizeof(win_credential_t));
    c->mode = m;
    csguard g(&c->cslock);
...
static bool win_credential_decref(win_credential_t *c) {
    // Call with lock held.  Caller MUST call win_credential_delete if this
    // returns true.
...
static int win_credential_load_cert(win_credential_t *cred,
                                    const char *store_name,
                                    const char *cert_name, const char *passwd) {
    if (!store_name) return -2;

    int ec = 0;
    HCERTSTORE cert_store = open_cert_db(store_name, passwd, &ec);
    if (!cert_store) return ec;
    ...
    while (tmpctx = CertEnumCertificatesInStore(cert_store, tmpctx)) {
        ...
        if (cert_name && *cert_name) {
            DWORD len = CertGetNameString(
                tmpctx, CERT_NAME_FRIENDLY_DISPLAY_TYPE, 0, NULL, NULL, 0);
            if (len != name_len + 1) continue;
            CertGetNameString(tmpctx, CERT_NAME_FRIENDLY_DISPLAY_TYPE, 0, NULL,
                              fn, len);
            ...
            if (cert_count == 1) {
                ...
                ssl_log_error(
                    "Multiple certificates to choose from certificate store %s",
                    store_name);
                found_ctx = NULL;
                break;
            }
...
static CredHandle win_credential_cred_handle(win_credential_t *cred,
                                             pn_ssl_verify_mode_t verify_mode,
                                             const char *session_id,
                                             SECURITY_STATUS *status) {
    if (cred->mode == PN_SSL_MODE_SERVER &&
        SecIsValidHandle(&cred->cred_handle)) {
        *status = SEC_E_OK;
        return cred->cred_handle; // Server always reuses cached value
    }
    // TODO: if (is_client && session_id != NULL) create or use cached value
    // based on session_id+server_host_name (per domain? reclaimed after X
    // hours?)
    ...
    *status =
        AcquireCredentialsHandle(NULL, UNISP_NAME, direction, NULL, &descriptor,
                                 NULL, NULL, &tmp_handle, &expiry);
...
struct pni_ssl_t {
    ...
    ssize_t
        app_input_closed; // error code returned by upper layer process input
    ssize_t
        app_output_closed; // error code returned by upper layer process output
    ...
    char *
        in_data; // Just the plaintext data part of sc_inbuf, decrypted in place
    size_t in_data_size;
    ...
    pn_bytes_t
        app_inbytes; // Virtual decrypted datastream, presented to app layer
...
static ssize_t process_input_ssl(pn_transport_t *transport, unsigned int layer,
                                 const char *input_data, size_t len);
static ssize_t process_output_ssl(pn_transport_t *transport, unsigned int layer,
                                  char *input_data, size_t len);
static ssize_t process_input_done(pn_transport_t *transport, unsigned int layer,
                                  const char *input_data, size_t len);
static ssize_t process_output_done(pn_transport_t *transport,
                                   unsigned int layer, char *input_data,
                                   size_t len);
...
static HRESULT verify_peer(pni_ssl_t *ssl, HCERTSTORE root_store,
                           const char *server_name, bool tracing);
...
static size_t _pni_min(size_t a, size_t b) { return (a < b) ? a : b; }
...
static int ssl_failed(pn_transport_t *transport, const char *reason) {
    ...
    if (!reason) {
        ...
        FormatMessage(FORMAT_MESSAGE_MAX_WIDTH_MASK |
                          FORMAT_MESSAGE_FROM_SYSTEM,
                      0, status, 0, buf, sizeof(buf), 0);
        reason = buf;
    }
    ...
    pn_do_error(transport, "amqp:connection:framing-error", "SSL Failure: %s",
                reason);
    return PN_EOS;
}

static pn_ssl_session_t *ssn_cache_find(pn_ssl_domain_t *domain,
                                        const char *id) {
    // TODO:
    return NULL;
}
...
pn_ssl_domain_t *pn_ssl_domain(pn_ssl_mode_t mode) {
    pn_ssl_domain_t *domain =
        (pn_ssl_domain_t *)calloc(1, sizeof(pn_ssl_domain_t));
...
int pn_ssl_domain_set_credentials(pn_ssl_domain_t *domain,
    ...
    if (win_credential_has_certificate(domain->cred)) {
        ...
        if (!domain->cred) return -1;
    }
    return win_credential_load_cert(domain->cred, certificate_file,
...
int pn_ssl_domain_set_peer_authentication(pn_ssl_domain_t *domain,
    ...
    if (!domain->has_ca_db &&
        (mode == PN_SSL_VERIFY_PEER || mode == PN_SSL_VERIFY_PEER_NAME)) {
    ...
    case PN_SSL_VERIFY_PEER_NAME:
        if (domain->mode == PN_SSL_MODE_SERVER) {
            ...
            if (!win_credential_has_certificate(domain->cred)) {
                ssl_log_error(
                    "Error: Server cannot verify peer without configuring a "
...
const pn_io_layer_t ssl_layer = {process_input_ssl, process_output_ssl, NULL,
                                 NULL, buffered_output};

const pn_io_layer_t ssl_input_closed_layer = {
    process_input_done, process_output_ssl, NULL, NULL, buffered_output};

const pn_io_layer_t ssl_output_closed_layer = {
    process_input_ssl, process_output_done, NULL, NULL, buffered_output};

const pn_io_layer_t ssl_closed_layer = {process_input_done, process_output_done,
                                        NULL, NULL, buffered_output};

int pn_ssl_init(pn_ssl_t *ssl0, pn_ssl_domain_t *domain,
                const char *session_id) {
    ...
    ssl->state =
        (domain->mode == PN_SSL_MODE_CLIENT) ? CLIENT_HELLO : NEGOTIATING;
...
int pn_ssl_get_ssf(pn_ssl_t *ssl0) {
    ...
    if (ssl && ssl->state == RUNNING && SecIsValidHandle(&ssl->ctxt_handle) &&
...
bool pn_ssl_get_cipher_name(pn_ssl_t *ssl0, char *buffer, size_t size) {
    ...
    *buffer = '\0';
    ...
    if (QueryContextAttributes(&ssl->ctxt_handle, SECPKG_ATTR_CONNECTION_INFO,
        ...
        pni_snprintf(buffer, size, "%x_%x:%x_%x:%x_%x", info.aiExch,
                     info.dwExchStrength, info.aiCipher, info.dwCipherStrength,
...
bool pn_ssl_get_protocol_name(pn_ssl_t *ssl0, char *buffer, size_t size) {
    ...
    *buffer = '\0';
    ...
    if (QueryContextAttributes(&ssl->ctxt_handle, SECPKG_ATTR_CONNECTION_INFO,
        ...
        // TLSV1.1 and TLSV1.2 are supported as of XP-SP3, but not defined until
        // VS2010
    ...
    if (transport->connection) {
        if (pn_string_size(transport->connection->hostname)) {
            pn_ssl_set_peer_hostname(
                (pn_ssl_t *)transport,
                pn_string_get(transport->connection->hostname));
        }
    }
...
const char *pn_ssl_get_remote_subject(pn_ssl_t *ssl0) {
    // RFC 2253 compliant, but differs from openssl's subject string with space
    // separators and ordering of multicomponent RDNs.  Made to work as
    // similarly as possible with choice of flags.
    pni_ssl_t *ssl = get_ssl_internal(ssl0);
    if (!ssl || !SecIsValidHandle(&ssl->ctxt_handle)) return NULL;
    if (!ssl->subject) {
        SECURITY_STATUS status;
        PCCERT_CONTEXT peer_cc = 0;
        status = QueryContextAttributes(
            &ssl->ctxt_handle, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &peer_cc);
        if (status != SEC_E_OK) {
            ssl_log_error_status(status,
                                 "can't obtain remote certificate subject");
            return NULL;
        }
        DWORD flags = CERT_X500_NAME_STR | CERT_NAME_STR_REVERSE_FLAG;
        DWORD strlen =
            CertNameToStr(peer_cc->dwCertEncodingType,
                          &peer_cc->pCertInfo->Subject, flags, NULL, 0);
        if (strlen > 0) {
            ssl->subject = (char *)malloc(strlen);
            if (ssl->subject) {
                DWORD len = CertNameToStr(peer_cc->dwCertEncodingType,
                                          &peer_cc->pCertInfo->Subject, flags,
                                          ssl->subject, strlen);
                if (len != strlen) {
                    free(ssl->subject);
                    ssl->subject = NULL;
                    ssl_log_error(
                        "pn_ssl_get_remote_subject failure in CertNameToStr");
                }
            }
        }
...
const char *tls_version_check(pni_ssl_t *ssl) {
    ...
    return (info.dwProtocol < SP_PROT_TLS1_SERVER)
               ? "peer does not support TLS 1.0 security"
               : NULL;
}

static void ssl_encrypt(pn_transport_t *transport, char *app_data,
                        size_t count) {
    pni_ssl_t *ssl = transport->ssl;
    ...
    SECURITY_STATUS status =
        EncryptMessage(&ssl->ctxt_handle, 0, &buff_desc, 0);
...
static bool ssl_decrypt(pn_transport_t *transport) {
    ...
    SECURITY_STATUS status =
        DecryptMessage(&ssl->ctxt_handle, &buff_desc, 0, NULL);
...
static void client_handshake_init(pn_transport_t *transport) {
    ...
    ULONG ctxt_requested =
        ISC_REQ_STREAM | ISC_REQ_USE_SUPPLIED_CREDS | ISC_REQ_EXTENDED_ERROR;
    ULONG ctxt_attrs;
    ...
    SECURITY_STATUS status = InitializeSecurityContext(
        &ssl->cred_handle, NULL, host, ctxt_requested, 0, 0, NULL, 0,
        &ssl->ctxt_handle, &send_buff_desc, &ctxt_attrs, NULL);
...
static void client_handshake(pn_transport_t *transport) {
    pni_ssl_t *ssl = transport->ssl;
    // Feed SChannel ongoing responses from the server until the handshake is
    // complete.
    SEC_CHAR *host = (SEC_CHAR *)(ssl->peer_hostname);
    ...
    SECURITY_STATUS status;
    {
        ...
        status = InitializeSecurityContext(
            &ssl->cred_handle, &ssl->ctxt_handle, host, ctxt_requested, 0, 0,
            &token_buff_desc, 0, NULL, &send_buff_desc, &ctxt_attrs, NULL);
    }
    ...
    case SEC_E_OK:
        ...
        if (token_buffs[1].BufferType == SECBUFFER_EXTRA &&
            ...
            // decrypt message since the pvBuffer value is not set.  Grrr.
            ssl->extra_count = token_buffs[1].cbBuffer;
            ssl->inbuf_extra =
                ssl->sc_inbuf + (ssl->sc_in_count - ssl->extra_count);
        }

        QueryContextAttributes(&ssl->ctxt_handle, SECPKG_ATTR_STREAM_SIZES,
                               &ssl->sc_sizes);
        ...
        if (max > ssl->sc_out_size) {
            ...
            pn_do_error(transport, "amqp:connection:framing-error",
                        "SSL Failure: buffer size");
            break;
        }

        ssl->state = RUNNING;
        ssl->max_data_size =
            max - ssl->sc_sizes.cbHeader - ssl->sc_sizes.cbTrailer;
    ...
    if (token_buffs[1].BufferType == SECBUFFER_EXTRA &&
        token_buffs[1].cbBuffer > 0 && !ssl->ssl_closed) {
...
static void server_handshake(pn_transport_t *transport) {
    ...
    if (!ssl->protocol_detected) {
        // SChannel fails less aggressively than openssl on client hello,
        // causing hangs waiting for more bytes.  Help out here.
        pni_protocol_type_t type =
            pni_sniff_header(ssl->sc_inbuf, ssl->sc_in_count);
    ...
    send_buff_desc.cBuffers = 2;
    send_buff_desc.pBuffers = send_buffs;
    PCtxtHandle ctxt_handle_ptr =
        (SecIsValidHandle(&ssl->ctxt_handle)) ? &ssl->ctxt_handle : 0;

    SECURITY_STATUS status;
    {
        csguard g(&ssl->cred->cslock);
        status = AcceptSecurityContext(&ssl->cred_handle, ctxt_handle_ptr,
                                       &token_buff_desc, ctxt_requested, 0,
    ...
    case SEC_E_OK:
        ...
        if (ssl->verify_mode == PN_SSL_VERIFY_PEER ||
            ...
            bool tracing = PN_TRACE_DRV & transport->trace;
            HRESULT ec =
                verify_peer(ssl, ssl->cred->trust_store, NULL, tracing);
            if (ec) {
                ssl_log_error_status(ec, "certificate verification failed");
        ...
        if (send_buffs[0].cbBuffer != 0) outbound_token = true;
...
static bool grow_inbuf2(pn_transport_t *transport, size_t minimum_size) {
    ...
    while (new_capacity < minimum_size) new_capacity *= 2;
...
// Peer initiated a session end by sending us a shutdown alert (and we should
// politely reciprocate), or else we are initiating the session end (and will
// not bother to wait for the peer shutdown alert). Stop processing input
// immediately, and stop processing output once this is sent.

static void start_ssl_shutdown(pn_transport_t *transport) {
    pni_ssl_t *ssl = transport->ssl;
    assert(ssl->network_out_pending == 0);
    if (ssl->queued_shutdown) return;
...
static void rewind_sc_inbuf(pni_ssl_t *ssl) {
    // Decrypted bytes have been drained or double buffered.  Prepare for the
    // next SSL Record.
    assert(ssl->in_data_count == 0);
    if (ssl->decrypting) return;
    ssl->decrypting = true;
    if (ssl->inbuf_extra) {
        // A previous read picked up more than one Record.  Move it to the
        // beginning.
...
static void app_inbytes_add(pn_transport_t *transport) {
    ...
    if (ssl->double_buffered) {
        ...
        size_t count =
            _pni_min(ssl->in_data_count, pn_buffer_available(ssl->inbuf2));
...
static void app_inbytes_progress(pn_transport_t *transport, size_t minimum) {
    ...
    if (ssl->double_buffered) {
        if (ssl->app_inbytes.size == 0) {
            ...
            if (consumed > 0) {
                memmove((void *)ib2.start, ib2.start + consumed,
                        ssl->app_inbytes.size);
    ...
    if (ssl->in_data_count == 0) rewind_sc_inbuf(ssl);
}
...
static void app_inbytes_advance(pn_transport_t *transport, size_t consumed) {
    ...
    if (ssl->app_inbytes.size == 0) app_inbytes_progress(transport, 0);
}

static void read_closed(pn_transport_t *transport, unsigned int layer,
                        ssize_t error) {
    pni_ssl_t *ssl = transport->ssl;
    if (ssl->app_input_closed) return;
    if (ssl->state == RUNNING && !error) {
        // Signal end of stream
        ssl->app_input_closed = transport->io_layers[layer + 1]->process_input(
            transport, layer + 1, ssl->app_inbytes.start, 0);
    }
    if (!ssl->app_input_closed) ssl->app_input_closed = error ? error : PN_ERR;
...
// Read up to "available" bytes from the network, decrypt it and pass plaintext
// to application.

static ssize_t process_input_ssl(pn_transport_t *transport, unsigned int layer,
                                 const char *input_data, size_t available) {
    pni_ssl_t *ssl = transport->ssl;
    ...
    do {
        ...
        // sc_inbuf should be ready for new or additional network encrypted
        // bytes. i.e. no straggling decrypted bytes pending.
        ...
        if (ssl->state != RUNNING) {
            ...
            if (ssl->sc_in_count < 5) {
                ...
                available -= hc;
                consumed += hc;
                if (ssl->sc_in_count < 5 || available == 0) break;
            }

            // Top up sc_inbuf from network input_data hoping for a complete TLS
            // Record We try to guess the length as an optimization, but let
            // SChannel ultimately decide if there is spoofing going on.
            ...
            if (rec_len < 5 || rec_len == ssl->sc_in_count ||
        ...
        if (new_app_input &&
            (ssl->state == RUNNING || ssl->state == SHUTTING_DOWN)) {
            // present app_inbytes to io_next only if it has new content
            while (ssl->app_inbytes.size > 0) {
                if (!ssl->app_input_closed) {
                    ssize_t count =
                        transport->io_layers[layer + 1]->process_input(
                            transport, layer + 1, ssl->app_inbytes.start,
                            ssl->app_inbytes.size);
                    if (count > 0) {
                        forwarded += count;
                        // advance() can increase app_inbytes.size if double
                        // buffered
                        app_inbytes_advance(transport, count);
                        ssl_log(transport,
                                "Application consumed %d bytes from peer",
                                (int)count);
                    } else if (count == 0) {
                        size_t old_size = ssl->app_inbytes.size;
                        app_inbytes_advance(transport, 0);
                        if (ssl->app_inbytes.size == old_size) {
                            break; // no additional contiguous decrypted data
                                   // available, get more network data
                        }
                    } else {
                        // count < 0
                        ssl_log(transport,
                                "Application layer closed its input, error=%d "
                                "(discarding %d bytes)",
                                (int)count, (int)ssl->app_inbytes.size);
                        app_inbytes_advance(transport,
                                            ssl->app_inbytes.size); // discard
    ...
    if (ssl->state >= SHUTTING_DOWN) {
        if (ssl->app_input_closed || ssl->sc_input_shutdown) {
            // Next layer doesn't want more bytes, or it can't process without
            // more data than it has seen so far but the ssl stream has ended
...
static ssize_t process_output_ssl(pn_transport_t *transport, unsigned int layer,
    ...
    if (ssl->state == CLIENT_HELLO) {
        // output buffers eclusively for internal handshake use until
        // negotiation complete
        client_handshake_init(transport);
        if (ssl->state == SSL_CLOSED) return PN_EOS;
    ...
    do {
        ...
        if (ssl->network_out_pending == 0 && ssl->state == RUNNING &&
            ...
            ssize_t app_bytes;
            do {
                app_bytes = transport->io_layers[layer + 1]->process_output(
                    transport, layer + 1, app_outp, remaining);
                if (app_bytes > 0) {
                    app_outp += app_bytes;
                    remaining -= app_bytes;
                    ssl_log(transport,
                            "Gathered %d bytes from app to send to peer",
                            app_bytes);
                } else {
                    if (app_bytes < 0) {
                        ssl_log(transport,
                                "Application layer closed its output, error=%d "
                        ...
                        ssl_log(transport,
                                "Application layer blocked on input, closing");
                        ssl->state = SHUTTING_DOWN;
...
static HCERTSTORE open_cert_db(const char *store_name, const char *passwd,
    ...
    *error = 0;
    ...
    if (sys_store_type) {
        ...
        cert_store =
            CertOpenStore(CERT_STORE_PROV_SYSTEM_A, 0, NULL,
                          CERT_STORE_OPEN_EXISTING_FLAG |
                              CERT_STORE_READONLY_FLAG | sys_store_type,
                          store_name);
        if (!cert_store) {
            ssl_log_error_status(GetLastError(),
                                 "Failed to open system certificate store %s",
                                 store_name);
            *error = -3;
            return NULL;
        }
        ...
        // PKCS#12 file
        HANDLE cert_file =
            CreateFile(store_name, GENERIC_READ, 0, NULL, OPEN_EXISTING,
                       FILE_ATTRIBUTE_NORMAL, NULL);
        if (INVALID_HANDLE_VALUE == cert_file) {
            HRESULT status = GetLastError();
            ssl_log_error_status(
                status, "Failed to open the file holding the private key: %s",
                store_name);
            *error = -4;
            return NULL;
        }
        ...
        char *buf = NULL;
        if (INVALID_FILE_SIZE != file_size) buf = (char *)malloc(file_size);
        if (!buf || !ReadFile(cert_file, buf, file_size, &nread, NULL) ||
            ...
            free(buf);
            ssl_log_error_status(status,
                                 "Reading the private key from file failed %s",
                                 store_name);
            *error = -5;
            return NULL;
        }
        ...
        if (passwd) {
            ...
            if (!nwc) {
                ssl_log_error_status(GetLastError(),
                                     "Error converting password from UTF8");
                free(buf);
                free(pwUCS2);
                *error = -6;
                return NULL;
        ...
        if (cert_store == NULL) {
            ssl_log_error_status(
                GetLastError(),
                "Failed to import the file based certificate store");
            free(buf);
            *error = -7;
            return NULL;
        }
...
static bool store_contains(HCERTSTORE store, PCCERT_CONTEXT cert) {
    ...
    PCCERT_CONTEXT tcert = CertFindCertificateInStore(
        store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, find_type, cert, 0);
    if (tcert) {
...
/* Match the DNS name pattern from the peer certificate against our configured
   peer hostname */
static bool match_dns_pattern(const char *hostname, const char *pattern,
                              int plen) {
    ...
    while (plen > 0 && slen > 0) {
        ...
        if (!star) {
            ...
            *star = '\0';
            ...
            if (prefix_len && pn_strncasecmp(prefix, slabel, prefix_len))
                return false;
            if (suffix_len &&
                pn_strncasecmp(suffix, slabel + (strlen(slabel) - suffix_len),
                               suffix_len))
                return false;
        }
    }
...
static char *wide_to_utf8(LPWSTR wstring) {
    ...
    if (!p) return NULL;
    if (WideCharToMultiByte(CP_UTF8, 0, wstring, -1, p, len, 0, 0)) return p;
    ssl_log_error_status(GetLastError(), "converting UCS2 to UTF8");
    free(p);
    return NULL;
}

static bool server_name_matches(const char *server_name,
                                CERT_EXTENSION *alt_name_ext,
                                PCCERT_CONTEXT cert) {
    ...
    if (alt_name_ext) {
        ...
        if (!CryptDecodeObjectEx(
                ...
                szOID_SUBJECT_ALT_NAME2, alt_name_ext->Value.pbData,
                alt_name_ext->Value.cbData,
                CRYPT_DECODE_ALLOC_FLAG | CRYPT_DECODE_NOCOPY_FLAG, 0,
                &alt_name_info, &size)) {
            ssl_log_error_status(GetLastError(),
                                 "Alternative name match internal error");
            return false;
        }
            ...
            if (alt_name_info->rgAltEntry[i].dwAltNameChoice ==
                ...
                if (alt_name) {
                    matched = match_dns_pattern(
                        server_name, (const char *)alt_name, strlen(alt_name));
                    free(alt_name);
                }
            }
        }
...
int pn_ssl_get_cert_fingerprint(pn_ssl_t *ssl0, char *fingerprint,
...
static HRESULT verify_peer(pni_ssl_t *ssl, HCERTSTORE root_store,
    ...
    if (status != SEC_E_OK) {
        ssl_log_error_status(
            status, "can't obtain remote peer certificate information");
        return status;
    }
    ...
    // anchor is not in the official root store.  We ignore CRL completely if it
    // doesn't apply to any untrusted certs in the chain, and defer to
    // SChannel's veto otherwise.  To rely on CRL, the CA must be in both the
    // official system trusted root store and the Proton cred->trust_store.  To
    // defeat CRL, the most distal cert with CRL must be placed in the Proton
    // cred->trust_store. Similarly, certificate usage checking is overly strict
    // at times.
    ...
    if (!CertGetCertificateChain(
            0, peer_cc, 0, peer_cc->hCertStore, &desc,
    ...
    PCCERT_CONTEXT trunk_cert =
        chain_context->rgpChain[0]->rgpElement[chain_len - 1]->pCertContext;
    if (tracing)
    ...
    do {
        ...
        bool revocable =
            false; // unless we see any untrusted certs that could be
        for (int i = 0; i < chain_len; i++) {
            ...
            if (cc->pCertInfo->dwVersion != CERT_V3) {
                if (tracing)
                    ssl_log_error(
                        "certificate chain element %d is not version 3", i);
            ...
            if (tracing) {
                char name[512];
                const char *is_anchor =
                    (cc == trust_anchor) ? " trust anchor" : "";
                if (!CertNameToStr(
                        cc->dwCertEncodingType, &cc->pCertInfo->Subject,
                        CERT_X500_NAME_STR | CERT_NAME_STR_NO_PLUS_FLAG, name,
                        sizeof(name)))
                    strcpy(name, "[too long]");
                ssl_log_error(
                    "element %d (name: %s)%s error bits %x info bits %x", i,
                    name, is_anchor, ce->TrustStatus.dwErrorStatus,
        ...
        if (!trust_anchor) {
            // We don't trust any of the certs in the chain, see if the last
            // cert is issued by a Proton trusted CA.
            DWORD flags = CERT_STORE_NO_ISSUER_FLAG ||
                          CERT_STORE_SIGNATURE_FLAG ||
                          CERT_STORE_TIME_VALIDITY_FLAG;
            trust_anchor = CertGetIssuerCertificateFromStore(
                root_store, trunk_cert, 0, &flags);
            if (trust_anchor) {
                if (tracing) {
        ...
        if (leaf_cert != trust_anchor) {
            ...
            for (int ii = 0; ii < n_ext; ii++) {
                ...
                if (!strcmp(p->pszObjId, szOID_SUBJECT_ALT_NAME2))
                    if (p->Value.pbData) leaf_alt_names = p;
            }
        }
        ...
        if (server_name) {
            ...
            if (!nwc) {
                error = GetLastError();
                ssl_log_error_status(error,
                                     "Error converting server name from UTF8");
                break;
            }
        }
        ...
        if (!revocable) ssl_desc.fdwChecks |= SECURITY_FLAG_IGNORE_REVOCATION;
        if (!strict_usage)
        ...
        if (!CertVerifyCertificateChainPolicy(CERT_CHAIN_POLICY_SSL,
                                              chain_context, &chain_desc,
                                              &chain_status)) {
            error = GetLastError();
            // Failure to complete the check, does not (in)validate the cert.
            ssl_log_error_status(error,
                                 "Supplemental certificate chain check failed");
            break;
        }
        ...
        if (chain_status.dwError) {
            ...
            if (tracing) {
                ssl_log_error_status(chain_status.dwError,
                                     "Certificate chain verification error");
                if (chain_status.lChainIndex == 0 &&
                    chain_status.lElementIndex != -1) {
                    int idx = chain_status.lElementIndex;
                    CERT_CHAIN_ELEMENT *ce =
                        chain_context->rgpChain[0]->rgpElement[idx];
                    ssl_log_error("  chain failure at %d error/info: %x %x",
                                  idx, ce->TrustStatus.dwErrorStatus,
                                  ce->TrustStatus.dwInfoStatus);
                }
            }
            break;
        ...
        if (server_name && ssl->verify_mode == PN_SSL_VERIFY_PEER_NAME &&
            ...
            break;
        } else if (ssl->verify_mode == PN_SSL_VERIFY_PEER_NAME &&
                   !server_name) {
            ssl_log_error("Error: configuration error: PN_SSL_VERIFY_PEER_NAME "
    ...
    if (tracing && !error) ssl_log_error("peer certificate authenticated");

    // Lots to clean up.
    if (peer_cc) CertFreeCertificateContext(peer_cc);
    if (trust_anchor) CertFreeCertificateContext(trust_anchor);
