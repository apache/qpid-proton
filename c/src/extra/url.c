...
void pni_parse_url(char *url, char **scheme, char **user, char **pass,
    ...
    *host = url;
    ...
    char *colon = strrchr(
        url,
        ':'); /* Be flexible about unbracketed IPV6 literals like ::1:<port> */
    if (colon) {
        *colon = '\0';
        *port = colon + 1;
    }
...
static int pn_url_inspect(void *obj, pn_string_t *dst) {
    ...
    int err = 0;
    err = pn_string_addf(dst, "Url(");
    if (err) return err;
    err = pn_inspect(pn_url_string(url), dst);
    if (err) return err;
...
const char *pn_url_str(pn_url_t *url) {
    if (pn_string_get(url->str) == NULL) {
        ...
        if (url->host) {
            if (strchr(url->host, ':'))
                pn_string_addf(url->str, "[%s]", url->host);
            else
                pn_string_addf(url->str, "%s", url->host);
        }
