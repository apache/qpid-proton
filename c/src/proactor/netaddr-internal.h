...
/* Common code for proactors that use the POSIX/Winsock sockaddr library for
 * socket addresses. */
...
int pn_netaddr_host_port(const pn_netaddr_t *na, char *host, size_t hlen,
                         char *port, size_t plen) {
    return getnameinfo(pn_netaddr_sockaddr(na), pn_netaddr_socklen(na), host,
                       hlen, port, plen, NI_NUMERICHOST | NI_NUMERICSERV);
}
...
/* If want has port=0 and got has port > 0 then return port of got, else return
 * 0 */
static uint16_t check_dynamic_port(const struct sockaddr *want,
