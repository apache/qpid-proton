...
/*
  libuv functions are thread unsafe, we use a"leader-worker-follower" model as
  follows:

  - At most one thread at a time is the "leader". The leader runs the UV loop
  till there are events to process and then becomes a "worker"

  - Concurrent "worker" threads process events for separate connections or
  listeners. When they run out of work they become "followers"

  - A "follower" is idle, waiting for work. When the leader becomes a worker,
  one follower takes over as the new leader.

  Any thread that calls pn_proactor_wait() or pn_proactor_get() can take on any
  of the roles as required at run-time. Monitored sockets (connections or
  listeners) are passed between threads on thread-safe queues.
...
/* pn_proactor_t and pn_listener_t are plain C structs with normal memory
   management. CLASSDEF is for identification when used as a pn_event_t context.
*/
...
typedef struct addr_t {
    ...
    uv_getaddrinfo_t
        getaddrinfo; /* UV getaddrinfo request, contains list of addrinfo */
...
struct pn_listener_t {
    ...
    /* Locked for thread-safe access. uv_listen can't be stopped or cancelled so
     * we can't detach a listener from the UV loop to prevent concurrent access.
     */
    uv_mutex_t lock;
...
struct pn_proactor_t {
    ...
    /* Owner thread: proactor collector and batch can belong to leader or a
     * worker */
    pn_collector_t *collector;
    pn_event_batch_t batch;
    ...
    uv_mutex_t lock;
    work_queue_t
        worker_q; /* ready for work, to be returned via pn_proactor_wait()  */
...
static inline void notify(pn_proactor_t *p) { uv_async_send(&p->notify); }
...
/* Notify that this work item needs attention from the leader at the next
 * opportunity */
static void work_notify(work_t *w) {
    uv_mutex_lock(&w->proactor->lock);
    /* If the socket is in use by a worker or is already queued then leave it
       where it is. It will be processed in pn_proactor_done() or when the queue
       it is on is processed.
    */
...
static void parse_addr(addr_t *addr, const char *str) {
    pni_parse_addr(str, addr->addr_buf, sizeof(addr->addr_buf), &addr->host,
                   &addr->port);
}
...
static void
global_init_fn(void) { /* Call via uv_once(&global_init_once, global_init_fn) */
...
static pconnection_t *pconnection(pn_proactor_t *p, pn_connection_t *c,
                                  pn_transport_t *t, bool server) {
...
static void pconnection_free(pconnection_t *pc) {
    ...
    if (pc->addr.getaddrinfo.addrinfo) {
        uv_freeaddrinfo(
            pc->addr.getaddrinfo.addrinfo); /* Interrupted after resolve */
    }
...
static inline pconnection_t *batch_pconnection(pn_event_batch_t *batch) {
    ...
    return d ? (pconnection_t *)((char *)d - offsetof(pconnection_t, driver))
             : NULL;
}
...
static void on_close_pconnection_final(uv_handle_t *h) {
    /* Free resources associated with a pconnection_t.
       If the life of the pn_connection_t has been extended with reference
       counts we want the pconnection_t to have the same lifespan so calls to
       pn_connection_wake() will be valid, but no-ops.
    */
...
static inline void pconnection_bad_connect(pconnection_t *pc, int err) {
    if (!pc->connected) {
        pc->connected =
            err; /* Remember first connect error in case they all fail  */
    }
}

/* Set the error condition, but don't close the driver. */
static void pconnection_set_error(pconnection_t *pc, int err,
                                  const char *what) {
    pn_connection_driver_t *driver = &pc->driver;
    pn_connection_driver_bind(
        driver); /* Make sure we are bound so errors will be reported */
    pni_proactor_set_cond(pn_transport_condition(driver->transport), what,
                          pc->addr.host, pc->addr.port, uv_strerror(err));
}
...
static void listener_error_lh(pn_listener_t *l, int err, const char *what) {
    ...
    if (!pn_condition_is_set(l->condition)) {
        pni_proactor_set_cond(l->condition, what, l->addr.host, l->addr.port,
                              uv_strerror(err));
    }
    listener_close_lh(l);
}
...
static void on_connect_fail(uv_handle_t *handle) {
    ...
    if (err) {
        pc->connected = err;
        pc->addr.addrinfo =
            NULL; /* No point in trying anymore, we can't create a socket */
    } else {
        try_connect(pc);
    }
}
...
static void on_connect(uv_connect_t *connect, int err) {
    ...
    if (!err) {
        ...
        work_notify(&pc->work);
        uv_freeaddrinfo(
            pc->addr.getaddrinfo.addrinfo); /* Done with address info */
        pc->addr.getaddrinfo.addrinfo = NULL;
    } else {
        pconnection_bad_connect(pc, err);
        uv_safe_close((uv_handle_t *)&pc->tcp,
                      on_connect_fail); /* Try the next addr if there is one */
    }
}

/* Incoming connection ready to be accepted */
static void on_connection(uv_stream_t *server, int err) {
    /* Unlike most on_* functions, this can be called by the leader thread when
     * the listener is ON_WORKER or ON_LEADER, because
     *
    ...
    if (err) {
        ...
        uv_mutex_lock(&l->lock);
        pn_collector_put(l->collector, lsocket__class(), ls,
                         PN_LISTENER_ACCEPT);
        uv_mutex_unlock(&l->lock);
    }
    work_notify(&l->work);
...
static int leader_resolve(pn_proactor_t *p, addr_t *addr, bool listen) {
    ...
    int err = uv_getaddrinfo(&p->loop, &addr->getaddrinfo, NULL, addr->host,
                             addr->port, &hints);
    addr->addrinfo =
        addr->getaddrinfo.addrinfo; /* Start with the first addrinfo */
    return err;
}

/* Try to connect to the current addrinfo. Called by leader and via callbacks
 * for retry.*/
static void try_connect(pconnection_t *pc) {
    ...
    if (!ai) { /* End of list, connect fails */
        ...
        int err =
            uv_tcp_connect(&pc->connect, &pc->tcp, ai->ai_addr, on_connect);
        if (err) {
            pconnection_bad_connect(pc, err);
            uv_close((uv_handle_t *)&pc->tcp,
                     on_connect_fail); /* Queue up next attempt */
        }
    }
}
...
static int lsocket(pn_listener_t *l, struct addrinfo *ai) {
    ...
    if (err) {
        ...
        if (!err)
            err = uv_listen((uv_stream_t *)&ls->tcp, l->backlog, on_connection);
        if (!err) {
            ...
            if (na ==
                l->addrs) { /*  First socket, check for dynamic port bind */
                l->dynamic_port =
                    check_dynamic_port(ai->ai_addr, pn_netaddr_sockaddr(na));
            } else {
            ...
            uv_close((uv_handle_t *)&ls->tcp,
                     on_close_lsocket); /* Freed by on_close_lsocket */
        }
    }
    return err;
}
...
static void leader_listen_lh(pn_listener_t *l) {
    ...
    if (!err) {
        ...
        size_t len = 0;
        for (struct addrinfo *ai = l->addr.getaddrinfo.addrinfo; ai;
             ai = ai->ai_next) {
            ++len;
        }
        ...
        if (l->lsockets) { /* Ignore errors if we got at least one good
                              listening socket */
            err = 0;
        }
    }
...
static bool leader_process_listener(pn_listener_t *l) {
    ...
    for (pconnection_t *pc = pconnection_pop(&l->accept); pc;
         pc = pconnection_pop(&l->accept)) {
        int err = pconnection_init(pc);
        if (!err)
            err = uv_accept((uv_stream_t *)&pc->lsocket->tcp,
                            (uv_stream_t *)&pc->tcp);
        if (!err) {
            pconnection_addresses(pc);
        } else {
        ...
        work_start(
            &pc->work); /* Process events for the accepted/failed connection */
    }
...
/* Generate tick events and return millis till next tick or 0 if no tick is
 * required */
...
static void on_timeout(uv_timer_t *timer) {
    ...
    uv_stop(
        &p->loop); /* UV does not always stop after on_timeout without this */
    uv_mutex_unlock(&p->lock);
}

/* Read buffer allocation function for uv, just returns the transports read
 * buffer. */
...
static pn_event_batch_t *proactor_batch_lh(pn_proactor_t *p,
                                           pn_event_type_t t) {
        ...
        if (!pn_connection_driver_has_event(&pc->driver)) {
            ...
            if (!err) {
                ...
                if (wbuf.size > 0) {
                    ...
                    err = uv_write(&pc->write, (uv_stream_t *)&pc->tcp, &buf, 1,
                                   on_write);
                    if (!err) {
...
void pconnection_detach(pconnection_t *pc) {
    if (pc->connected &&
        !pc->writing) { /* Can't detach while a write is pending */
...
static pn_event_batch_t *leader_lead_lh(pn_proactor_t *p, uv_run_mode mode) {
    ...
    if (p->disconnect) {
        ...
        if (p->active) {
            ...
            p->need_inactive =
                true; /* Send INACTIVE right away, nothing to do. */
        }
    }
    ...
    for (work_t *w = work_pop(&p->leader_q); w; w = work_pop(&p->leader_q)) {
        ...
        uv_mutex_unlock(&p->lock); /* Unlock to process each item, may add more
                                      items to leader_q */
        bool has_work = false;
        switch (w->type) {
        case T_CONNECTION:
...
pn_event_batch_t *pn_proactor_wait(struct pn_proactor_t *p) {
    ...
    if (!batch) { /* Become leader */
        ...
        p->has_leader = false;
        uv_cond_broadcast(
            &p->cond); /* Signal a followers. One takes over, many can work. */
    }
    uv_mutex_unlock(&p->lock);
    return batch;
}
...
static void on_proactor_free(uv_handle_t *h, void *v) {
    ...
    if (h->type == UV_TCP) { /* Put the corresponding work item on the leader_q
                                for cleanup */
        work_t *w = NULL;
        switch (*(struct_type *)h->data) {
        case T_CONNECTION:
            ...
            break;
        case T_LSOCKET:
            w = &((lsocket_t *)h->data)->parent->work;
            break;
        default:
            break;
        }
        if (w && w->next == work_unqueued) {
            work_push(&w->proactor->leader_q,
                      w); /* Save to be freed after all closed */
        }
    }
}
...
static void work_free(work_t *w) {
    switch (w->type) {
    case T_CONNECTION:
        pconnection_free((pconnection_t *)w);
        break;
    case T_LISTENER:
        pn_listener_free((pn_listener_t *)w);
        break;
    default:
        break;
    }
...
void pn_proactor_free(pn_proactor_t *p) {
    ...
    while (uv_loop_alive(&p->loop)) {
        uv_run(&p->loop,
               UV_RUN_DEFAULT); /* Finish closing the proactor handles */
    }
    /* Free all work items */
    for (work_t *w = work_pop(&p->leader_q); w; w = work_pop(&p->leader_q)) {
        work_free(w);
...
void *pn_listener_get_context(pn_listener_t *l) { return l->context; }
