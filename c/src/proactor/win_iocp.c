...
//       change INACTIVE to be from begin_close instead of zombie reap, to be
//       more like Posix make the global write lock window much smaller 2
//       exclusive write buffers per connection make the zombie processing
//       thread safe configurable completion port concurrency proton objects and
//       ref counting: just say no.
...
// Code borrowed from old messenger/reactor io.c iocp.c write_pipeline.c
// select.c Mostly unchanged except fro some thread safety (i.e. dropping the
// old descriptor map)
...
void pni_shared_pool_create(iocp_t *iocp) {
    // TODO: more pools (or larger one) when using multiple non-loopback
    // interfaces
    ...
    if (iocp->shared_pool_size) {
        iocp->shared_pool_memory =
            (char *)VirtualAlloc(NULL, IOCP_WBUFSIZE * iocp->shared_pool_size,
        ...
        iocp->shared_results = (write_result_t **)malloc(
            iocp->shared_pool_size * sizeof(write_result_t *));
        iocp->available_results = (write_result_t **)malloc(
            iocp->shared_pool_size * sizeof(write_result_t *));
        ...
        for (unsigned i = 0; i < iocp->shared_pool_size; i++) {
            iocp->shared_results[i] = iocp->available_results[i] =
                pni_write_result(NULL, mem, IOCP_WBUFSIZE);
            mem += IOCP_WBUFSIZE;
        }
    }
}
...
static write_result_t *shared_pool_pop(iocp_t *iocp) {
    return iocp->shared_available_count
               ? iocp->available_results[--iocp->shared_available_count]
               : NULL;
}
...
write_pipeline_t *pni_write_pipeline(iocpdesc_t *iocpd) {
    ...
    write_pipeline_t *pipeline =
        (write_pipeline_t *)pn_class_new(&clazz, sizeof(write_pipeline_t));
...
/*
 * Optimal depth will depend on properties of the NIC, server, and driver.  For
 * now, just distinguish between loopback interfaces and the rest. Optimizations
 * in the loopback stack allow decent performance with depth 1 and actually
 * cause major performance hiccups if set to large values.
 */
static void set_depth(write_pipeline_t *pl) {
    ...
    if (getsockname(pl->iocpd->socket, (sockaddr *)&sa, &salen) == 0 &&
        getnameinfo((sockaddr *)&sa, salen, buf, buflen, NULL, 0,
                    NI_NUMERICHOST) == 0) {
        if ((sa.ss_family == AF_INET6 && strcmp(buf, "::1")) ||
            ...
            if (iocp->loopback_bufsize) {
                const char *p = (const char *)realloc(
                    (void *)pl->primary->buffer.start, iocp->loopback_bufsize);
                if (p) {
...
size_t pni_write_pipeline_reserve(write_pipeline_t *pl, size_t count) {
    if (pl->primary->in_use) return 0; // I.e. io->wouldblock
    if (!pl->depth) set_depth(pl);
    if (pl->depth == 1) {
    ...
    size_t shared_count =
        (iocp->shared_available_count + writers - 1) / writers;
    bufs = pni_min(bufs, shared_count + 1);
    pl->reserved_count = pending + bufs;

    if (bufs == wanted && pl->reserved_count < (pl->depth / 2) &&
        ...
        if (pl->next_primary_index < pending) pl->next_primary_index = pending;
    }
    return bufs;
}
...
write_result_t *pni_write_pipeline_next(write_pipeline_t *pl) {
    ...
    if (sz >= pl->reserved_count) return NULL;
...
void pni_write_pipeline_return(write_pipeline_t *pl, write_result_t *result) {
    ...
    pl->pending_count--;
    pl->reserved_count = 0;
    if (result != pl->primary) shared_pool_push(result);
    if (pl->pending_count == 0) remove_as_writer(pl);
}
...
int pni_win32_error(pn_error_t *error, const char *msg, HRESULT code) {
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS |
                  ...
                  NULL, code, 0, (LPSTR)&err, sizeof(err), NULL);
...
static void pni_events_update(iocpdesc_t *iocpd, int events) {
    ...
    if (iocpd->poll_error) events |= PN_ERROR;
    if (iocpd->events == events) return;
...
iocpdesc_t *create_same_type_socket(iocpdesc_t *iocpd) {
    sockaddr_storage sa;
    socklen_t salen = sizeof(sa);
    if (getsockname(iocpd->socket, (sockaddr *)&sa, &salen) == -1) return NULL;
    SOCKET s = socket(sa.ss_family, SOCK_STREAM,
                      0); // Currently only work with SOCK_STREAM
    if (s == INVALID_SOCKET) return NULL;
    return pni_iocpdesc_create(iocpd->iocp, s);
}

static bool is_listener(iocpdesc_t *iocpd) { return iocpd && iocpd->acceptor; }

// === Async accept processing

static accept_result_t *accept_result(iocpdesc_t *listen_sock) {
    accept_result_t *result =
        (accept_result_t *)calloc(1, sizeof(accept_result_t));
    if (result) {
...
static void pni_acceptor_finalize(void *object) {
    ...
    for (size_t i = 0; i < len; i++) free(pn_list_get(acceptor->accepts, i));
...
static pni_acceptor_t *pni_acceptor(iocpdesc_t *iocpd) {
    ...
    pni_acceptor_t *acceptor =
        (pni_acceptor_t *)pn_class_new(&clazz, sizeof(pni_acceptor_t));
...
void begin_accept(pni_acceptor_t *acceptor, accept_result_t *result) {
    ...
    if (acceptor->listen_sock->closing) {
        if (result) {
            if (mp && result->new_sock &&
                result->new_sock->socket != INVALID_SOCKET)
                closesocket(result->new_sock->socket);
            free(result);
            acceptor->accept_queue_size--;
        }
        if (acceptor->accept_queue_size == 0) acceptor->signalled = true;
        return;
    }

    if (result) {
        if (!mp) reset_accept_result(result);
    } else {
        if (acceptor->accept_queue_size < IOCP_MAX_ACCEPTS &&
            ...
            // an async accept is still pending or max concurrent accepts
            // already hit
            return;
        }
    }
    ...
    if (result->new_sock) {
        ...
        bool success = acceptor->fn_accept_ex(
            acceptor->listen_sock->socket, result->new_sock->socket,
        ...
        if (!success) {
            ...
            if (err != ERROR_IO_PENDING) {
                if (err == WSAECONNRESET) {
                    ...
                    iocpdesc_fail(acceptor->listen_sock, err,
                                  "AcceptEX call failure");
                    return;
                }
            }
        ...
        iocpdesc_fail(acceptor->listen_sock, WSAGetLastError(),
...
static void complete_accept(accept_result_t *result, HRESULT status) {
    ...
    if (ld->read_closed) {
        if (!result->new_sock->closing) pni_iocp_begin_close(result->new_sock);
...
static void connect_result_finalize(void *object) {
    ...
    if (result->addrinfo) freeaddrinfo(result->addrinfo);
}
...
connect_result_t *connect_result(iocpdesc_t *iocpd, struct addrinfo *addr) {
    ...
    connect_result_t *result =
        (connect_result_t *)pn_class_new(&clazz, sizeof(connect_result_t));
    if (result) {
...
pn_socket_t pni_iocp_begin_connect(iocp_t *iocp, pn_socket_t sock,
                                   struct addrinfo *addr, pn_error_t *error) {
    // addr lives for the duration of the async connect.  Caller has passed
    // ownership here. See connect_result_finalize(). Use of Windows-specific
    // ConnectEx() requires our socket to be "loosely" pre-bound:
    sockaddr_storage sa;
    ...
    DWORD unused;
    bool success = fn_connect_ex(iocpd->socket, result->addrinfo->ai_addr,
                                 result->addrinfo->ai_addrlen, NULL, 0, &unused,
                                 (LPOVERLAPPED)result);
...
void complete_connect(connect_result_t *result, HRESULT status) {
    ...
    if (status) {
        ...
        if (setsockopt(iocpd->socket, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT,
                       NULL, 0)) {
            iocpdesc_fail(iocpd, WSAGetLastError(),
                          "Internal connect failure (update context)");
        } else {
...
write_result_t *pni_write_result(iocpdesc_t *iocpd, const char *buf,
                                 size_t buflen) {
    write_result_t *result =
        (write_result_t *)calloc(sizeof(write_result_t), 1);
    if (result) {
...
ssize_t pni_iocp_begin_write(iocpdesc_t *iocpd, const void *buf, size_t len,
                             bool *would_block, pn_error_t *error) {
    if (len == 0) return 0;
    *would_block = false;
...
void complete_write(write_result_t *result, DWORD xfer_count, HRESULT status) {
    ...
    if (status == 0 && xfer_count > 0) {
        if (xfer_count != result->requested) {
            // Is this recoverable?  How to preserve order if multiple
            // overlapped writes?
            pni_write_pipeline_return(iocpd->pipeline, result);
            iocpdesc_fail(iocpd, WSA_OPERATION_ABORTED,
                          "Partial overlapped write on socket");
            return;
        } else {
            // Success.
            pni_write_pipeline_return(iocpd->pipeline, result);
    ...
    // Other error
    pni_write_pipeline_return(iocpd->pipeline, result);
    if (status == WSAECONNABORTED || status == WSAECONNRESET ||
        status == WSAENOTCONN || status == ERROR_NETNAME_DELETED) {
        iocpd->write_closed = true;
        iocpd->poll_error = true;
        pni_events_update(iocpd, iocpd->events & ~PN_WRITABLE);
...
void complete_read(read_result_t *result, DWORD xfer_count, HRESULT status) {
    ...
    if (iocpd->closing) {
        // Application no longer reading, but we are looking for a zero length
        // read
        if (!iocpd->read_closed) drain_until_closed(iocpd);
        reap_check(iocpd);
        return;
    }
...
void start_reading(iocpdesc_t *iocpd) { begin_zero_byte_read(iocpd); }
...
static void bind_to_completion_port(iocpdesc_t *iocpd) {
    ...
    if (!iocpd->iocp->completion_port) {
        iocpdesc_fail(iocpd, WSAEINVAL,
                      "Incomplete setup, no completion port.");
        return;
    }

    if (CreateIoCompletionPort((HANDLE)iocpd->socket,
                               iocpd->iocp->completion_port, 0, 0))
        iocpd->bound = true;
    else {
...
static void release_sys_sendbuf(SOCKET s) {
    ...
    int sz = 0;
    int status =
        setsockopt(s, SOL_SOCKET, SO_SNDBUF, (const char *)&sz, sizeof(int));
    assert(status == 0);
}
...
static void complete(iocp_result_t *result, bool success,
                     DWORD num_transferred) {
...
void pni_iocp_drain_completions(iocp_t *iocp) {
    while (true) {
        ...
        bool good_op =
            GetQueuedCompletionStatus(iocp->completion_port, &num_transferred,
                                      &completion_key, &overlapped, timeout_ms);
        if (!overlapped) return; // timed out
        iocp_result_t *result = (iocp_result_t *)overlapped;
        if (!completion_key) complete(result, good_op, num_transferred);
    }
}

// returns: -1 on error, 0 on timeout, 1 successful completion
// proactor layer uses completion_key, but we don't, so ignore those (shutdown
// in progress)
int pni_iocp_wait_one(iocp_t *iocp, int timeout, pn_error_t *error) {
    ...
    if (!overlapped)
        if (GetLastError() == WAIT_TIMEOUT)
            ...
            if (error)
                pni_win32_error(error, "GetQueuedCompletionStatus",
                                GetLastError());
            return -1;
        }

    if (completion_key) return pni_iocp_wait_one(iocp, timeout, error);
    iocp_result_t *result = (iocp_result_t *)overlapped;
...
// zombie_list is for sockets transitioning out of iocp on their way to zero
// ops_in_progress and fully closed.
...
void pni_iocp_reap_check(iocpdesc_t *iocpd) { reap_check(iocpd); }
...
void pni_zombie_check(iocp_t *iocp, pn_timestamp_t now) {
    ...
    for (size_t idx = 0; idx < pn_list_size(zl); idx++) {
        ...
        if (iocpd->reap_time > now) return;
        if (iocpd->socket == INVALID_SOCKET) continue;
        if (iocp->iocp_trace)
...
static void drain_zombie_completions(iocp_t *iocp) {
    // No more pn_selector_select() from App, but zombies still need care and
    // feeding until their outstanding async actions complete.
    ...
    if (override) {
        ...
        if (grace > 0 && grace < 60000) shutdown_grace = (unsigned)grace;
    }
...
static void iocp_shutdown(iocpdesc_t *iocpd) {
    if (iocpd->socket == INVALID_SOCKET) return; // Hard close in progress
...
void pni_iocp_begin_close(iocpdesc_t *iocpd) {
    assert(!iocpd->closing);
    if (is_listener(iocpd)) {
        // Listening socket is easy.  Close the socket which will cancel async
        // ops.
        ...
        // Continue async operation looking for graceful close confirmation or
        // timeout.
        pn_socket_t old_sock = iocpd->socket;
        iocpd->closing = true;
        if (!iocpd->write_closed && !write_in_progress(iocpd))
...
class csguard {
  public:
    csguard(CRITICAL_SECTION *cs) : cs_(cs), set_(true) {
        EnterCriticalSection(cs_);
    }
    ~csguard() {
        if (set_) LeaveCriticalSection(cs_);
    }
    void release() {
        if (set_) {
            set_ = false;
...
static int pgetaddrinfo(const char *host, const char *port, int flags,
                        struct addrinfo **res) {
...
/* pn_proactor_t and pn_listener_t are plain C structs with normal memory
   management. Class definitions are for identification as pn_event_t context
   only.
*/
...
typedef enum { PROACTOR, PCONNECTION, LISTENER, WAKEABLE } pcontext_type_t;
...
typedef struct pcontext_t {
    ...
    struct pcontext_t
        *wake_next; // wake list, guarded by proactor eventfd_mutex
    bool closing;
...
static void psocket_init(psocket_t *ps, pn_listener_t *listener, bool is_reaper,
                         const char *addr) {
...
struct pn_proactor_t {
    ...
    pcontext_t
        *contexts; /* in-use contexts for PN_PROACTOR_INACTIVE and cleanup */
    ...
    bool timeout_set; /* timeout has been set by user and not yet cancelled or
                         generated event */
    bool timeout_processed; /* timout event dispatched in the most recent event
                               batch */
...
struct pn_listener_t {
    ...
    pcontext_t context;
    std::queue<accept_result_t *>
        *pending_accepts; // sockets awaiting a pn_listener_accept
...
static pn_event_batch_t *listener_process(pn_listener_t *l,
                                          iocp_result_t *result);
...
static void post_completion(iocp_t *iocp, ULONG_PTR arg1, void *arg2) {
    ...
    PostQueuedCompletionStatus(iocp->completion_port, nxfer, arg1,
                               (LPOVERLAPPED)arg2);
}
...
void do_complete(iocp_result_t *result) {
    ...
    case IOCP_ACCEPT:
         ...
         */
        assert(iocpd->closing);
        complete_accept((accept_result_t *)result,
                        result->status); // free's result and retires new_sock
        break;
    case IOCP_CONNECT:
        complete_connect((connect_result_t *)result, result->status);
        break;
    case IOCP_WRITE:
        complete_write((write_result_t *)result, result->num_transferred,
                       result->status);
        break;
    case IOCP_READ:
        complete_read((read_result_t *)result, result->num_transferred,
                      result->status);
        break;
    default:
        assert(false);
...
static inline pn_listener_t *as_listener(psocket_t *ps) { return ps->listener; }

static inline pconnection_t *pcontext_pconnection(pcontext_t *c) {
    return c->type == PCONNECTION
               ? (pconnection_t *)((char *)c - offsetof(pconnection_t, context))
               : NULL;
}
static inline pn_listener_t *pcontext_listener(pcontext_t *c) {
    return c->type == LISTENER
               ? (pn_listener_t *)((char *)c - offsetof(pn_listener_t, context))
               : NULL;
}

static pcontext_t *psocket_context(psocket_t *ps) {
    if (ps->listener) return &ps->listener->context;
...
class reaper {
  public:
    ...
    // Called when all competing threads have terminated except our own
    // reap_check timer.
    void final_shutdown() {
        running = false;
        DeleteTimerQueueEx(timer_queue_, INVALID_HANDLE_VALUE);
        // No pending or active timers from thread pool remain.  Truly single
        // threaded now.
        pn_free((void *)iocp_); // calls pni_iocp_finalize(); cleans up all
                                // sockets, completions, completion port.
  ...
  private:
    void reap_timer() {
        ...
        if (zd) {
            ...
            if (!CreateTimerQueueTimer(&timer_, timer_queue_, reap_check_cb,
                                       this, tm, 0, WT_EXECUTEONLYONCE)) {
...
static void psocket_error_str(psocket_t *ps, const char *msg,
                              const char *what) {
    if (ps->is_reaper) return;
    if (!ps->listener) {
...
static bool start_timer(HANDLE tqueue, HANDLE *timer, WAITORTIMERCALLBACK cb,
                        void *cb_arg, DWORD time) {
    if (*timer) {
        // TODO: log err
        return false;
    }
    return CreateTimerQueueTimer(timer, tqueue, cb, cb_arg, time, 0,
                                 WT_EXECUTEONLYONCE);
}
...
pn_listener_t *pn_event_listener(pn_event_t *e) {
    return (pn_event_class(e) == pn_listener__class())
               ? (pn_listener_t *)pn_event_context(e)
               : NULL;
}
...
static inline bool pconnection_can_free(pconnection_t *pc) {
    return pc->psocket.iocpd == NULL && pc->context.completion_ops == 0 &&
           !pc->stop_timer_required && !pconnection_has_event(pc) &&
...
static void pconnection_final_free(pconnection_t *pc) {
    ...
    pn_connection_driver_destroy(&pc->driver);
    pn_decref(pc);
    /* Now pc is freed iff the connection is, otherwise remains till the
...
// call with lock held.  return true if caller must call
// pconnection_final_free()
...
static bool pconnection_write(pconnection_t *pc, pn_bytes_t wbuf) {
    ...
    bool wouldblock;
    {
        ...
        n = pni_iocp_begin_write(pc->psocket.iocpd, wbuf.start, wbuf.size,
                                 &wouldblock, pc->psocket.iocpd->error);
    }
    if (n > 0) {
        ...
        psocket_error(&pc->psocket, WSAGetLastError(), "on write to");
               ...
               pn_connection_driver_write_closed(&pc->driver)) {
        if (pc->context.proactor->reaper->add(pc->psocket.iocpd)) {
            pc->psocket.iocpd = NULL;
            return false;
        }
...
// Queue the result (if any) for the one worker thread.  Become the worker if
// possible. NULL result is a wakeup or a topup. topup signifies that the
// working thread is the caller looking for additional events.
static pn_event_batch_t *
pconnection_process(pconnection_t *pc, iocp_result_t *result, bool topup) {
    bool first = true;
    bool waking = false;
    ...
    while (true) {
        {
            ...
            if (first) {
                ...
                if (!topup) {
                    if (pc->context.working) return NULL;
                ...
                if (pconnection_has_event(pc)) return &pc->batch;
            ...
            if (pc->queued_disconnect) { // From pn_proactor_disconnect()
                ...
                if (!pc->context.closing) {
                    if (pc->disconnect_condition) {
                        pn_condition_copy(
                            pn_transport_condition(pc->driver.transport),
                            pc->disconnect_condition);
                    }
                    pn_connection_driver_close(&pc->driver);
                }
            }
            ...
            if (pc->wake_count) {
                waking = open && pc->can_wake &&
                         !pn_connection_driver_finished(&pc->driver);
                pc->wake_count = 0;
            }
            if (pc->tick_pending) {
                pc->tick_pending = false;
                if (open) tick_required = true;
            }
        }
        ...
        while (pc->work_queue->size()) {
            ...
            if (result->iocpd->closing) {
                ...
                if (is_write_result(result)) {
                    ...
                    if (pc->psocket.iocpd &&
                        (pc->psocket.iocpd->events & PN_WRITABLE)) {
                        pc->connecting = false;
                        if (pc->started) open = true;
                    }
                } else
                    do_complete(result);
            }
        ...
        if (!open) {
            ...
            /* Ticks and checking buffers can generate events, process before
             * proceeding */
            bool ready = pconnection_has_event(pc);
            if (waking) {
                pn_connection_t *c = pc->driver.connection;
                pn_collector_put(pn_connection_collector(c), PN_OBJECT, c,
                                 PN_CONNECTION_WAKE);
                waking = false;
            }
            if (ready) {
            ...
            if (wbuf.size >= 16384 &&
                (pc->psocket.iocpd->events & PN_WRITABLE)) {
                if (!pconnection_write(pc, wbuf)) continue;
            }
            if (rbuf.size > 0 && !pc->psocket.iocpd->read_in_progress) {
                bool wouldblock;
                ssize_t n =
                    pni_iocp_recv(pc->psocket.iocpd, rbuf.start, rbuf.size,
                                  &wouldblock, pc->psocket.iocpd->error);
                ...
                if (n > 0) {
                    ...
                    psocket_error(&pc->psocket, WSAGetLastError(),
                                  "on read from");
            }
            ...
            if (wbuf.size > 0 && (pc->psocket.iocpd->events & PN_WRITABLE)) {
                if (!pconnection_write(pc, wbuf)) open = false;
            }
        }

        if (topup) return NULL; // regardless if new events made available
    }
}
...
static pn_event_t *pconnection_batch_next(pn_event_batch_t *batch) {
    ...
    if (e && !pc->started && pn_event_type(e) == PN_CONNECTION_BOUND)
        pc->started = true; // SSL will be set up on return and safe to do IO
                            // with correct transport layers
    return e;
}
...
static inline bool is_inactive(pn_proactor_t *p) {
    return (!p->contexts && !p->disconnects_pending && !p->timeout_set &&
            !p->need_timeout && !p->shutting_down);
}
...
void pn_proactor_done(pn_proactor_t *p, pn_event_batch_t *batch) {
    ...
    if (bp == p) {
        ...
        if (proactor_update_batch(p)) proactor_wake(p);
        return;
    }
}
...
static pn_event_batch_t *psocket_process(psocket_t *ps, iocp_result_t *result,
                                         reaper *rpr) {
    if (ps) {
        pconnection_t *pc = as_pconnection_t(ps);
        if (pc) {
            ...
            pn_listener_t *l = as_listener(ps);
            if (l) return listener_process(l, result);
        }
    }
    rpr->process(result);
    return NULL;
...
static pn_event_batch_t *proactor_completion_loop(struct pn_proactor_t *p,
                                                  bool can_block) {
    ...
    while (true) {
        ...
        bool good_op = GetQueuedCompletionStatus(p->iocp->completion_port,
                                                 &num_xfer, &completion_key,
                                                 &overlapped, win_timeout);
        ...
        if (!good_op && !overlapped) {
            ...
            // We aren't expecting a timeout, closed completion port, or other
            // error here.
            pn_logf(
                "%s",
                errno_str("Windows Proton proactor internal failure\n", false)
                    .c_str());
            abort();
        }
        ...
        if (completion_key == NULL) {
            ...
            // completion_key on our completion port is always null unless set
            // by us in PostQueuedCompletionStatus.  In which case, we hijack
            // the overlapped data structure for our own use.
            if (completion_key == psocket_wakeup_key)
                batch =
                    psocket_process((psocket_t *)overlapped, NULL, p->reaper);
...
VOID CALLBACK timeout_cb(PVOID arg, BOOLEAN /* ignored*/) {
    ...
    csguard g(&p->context.cslock);
    if (p->timeout_set) p->need_timeout = true; // else cancelled
    p->timeout_set = false;
    if (p->need_timeout) proactor_wake(p);
}
    ...
    while (pc->ai) { /* Have an address */
        ...
        if (fd != INVALID_SOCKET) {
            ...
            if (!bind(fd, (SOCKADDR *)&sa, ai->ai_addrlen)) {
                ...
                if (CreateIoCompletionPort((HANDLE)iocpd->socket,
                                           iocpd->iocp->completion_port, 0,
                                           0)) {
                    LPFN_CONNECTEX fn_connect_ex =
                        lookup_connect_ex2(iocpd->socket);
                    // addrinfo is owned by the pconnection so pass NULL to the
                    // connect result
                    connect_result_t *result = connect_result(iocpd, NULL);
                    DWORD unused;
                    bool success = fn_connect_ex(iocpd->socket, ai->ai_addr,
                                                 ai->ai_addrlen, NULL, 0,
                                                 &unused, (LPOVERLAPPED)result);
                    if (success || WSAGetLastError() == ERROR_IO_PENDING) {
                        iocpd->ops_in_progress++;
                        iocpd->active_completer = &pc->psocket;
                        // getpeername unreliable for outgoing connections, but
                        // we know it at this point
...
static void connect_step_done(pconnection_t *pc, connect_result_t *result) {
    ...
    complete_connect(
        result,
        result->base.status); // frees result, starts regular IO if connected
    ...
    if (!saved_status) {
        ...
        if (pc->addrinfo) {
            socklen_t len = sizeof(pc->local.ss);
            getsockname(pc->psocket.iocpd->socket,
                        (struct sockaddr *)&pc->local.ss, &len);
            freeaddrinfo(pc->addrinfo);
            pc->addrinfo = NULL;
        }
        pc->ai = NULL;
...
void pn_proactor_connect2(pn_proactor_t *p, pn_connection_t *c,
                          pn_transport_t *t, const char *addr) {
...
void pn_proactor_listen(pn_proactor_t *p, pn_listener_t *l, const char *addr,
    ...
    if (!gai_err) {
        ...
        for (struct addrinfo *ai = addrinfo; ai; ai = ai->ai_next) {
            ...
            unique_socket fd(
                ::socket(ai->ai_family, SOCK_STREAM, ai->ai_protocol));
            if (fd != INVALID_SOCKET) {
                bool yes = 1;
                if (!::bind(fd, ai->ai_addr, ai->ai_addrlen))
                    if (!::listen(fd, backlog)) {
                        ...
                        if (iocpd) {
                            ...
                            (void)getsockname(
                                sock, (struct sockaddr *)&ps->listen_addr.ss,
                                &len);
                            if (ps == l->psockets) { /* First socket, check for
                                                        dynamic port */
                                dynamic_port = check_dynamic_port(
                                    ai->ai_addr,
                                    pn_netaddr_sockaddr(&ps->listen_addr));
                            } else {
                                (ps - 1)->listen_addr.next =
                                    &ps->listen_addr; /* Link into list */
                            }
                        }
                    }
            }
    ...
    if (l->psockets_size ==
        0) { /* All failed, create dummy socket with an error */
        ...
        pn_collector_put(l->collector, pn_listener__class(), l,
                         PN_LISTENER_OPEN);
    }
    wakeup(l->psockets);
}
...
static void listener_close_all(pn_listener_t *l) {
    for (size_t i = 0; i < l->psockets_size; ++i) {
        ...
        if (ps->iocpd && !ps->iocpd->closing)
            if (l->context.proactor->reaper->add(ps->iocpd)) ps->iocpd = NULL;
    }
}
...
static void internal_listener_free(pn_listener_t *l, csguard &g) {
    ...
    g.release();
    if (can_free) listener_final_free(l);
...
static pn_event_batch_t *listener_process(pn_listener_t *l,
    ...
    psocket_t *ps = NULL;
    {
        ...
        if (!l->context.closing && result->status) {
            psocket_error(ps, WSAGetLastError(),
                          "listen on "); // initiates close/multi-reap
        }
        ...
        l->context.completion_ops++; // prevent accidental deletion while lock
    ...
    setsockopt(accept_sock, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT,
               (char *)&ps->iocpd->socket, sizeof(SOCKET));
    // Connected.
...

    {
        ...
        if (!ps->iocpd->ops_in_progress)
            begin_accept(ps->iocpd->acceptor,
                         NULL); // Start another, up to IOCP_MAX_ACCEPTS
        l->pending_events++;
        if (l->context.closing) release_pending_accepts(l);
        if (listener_maybe_free(l, g)) return NULL;
        return batch_owned(l);
    }
}
...
pn_proactor_t *pn_proactor() {
    ...
    if (tq) {
        ...
        if (p && c) {
            ...
            if (!WSAStartup(wsa_ver, &unused)) {
                wsa = true;
                if (iocp = pni_iocp()) {
                    InitializeCriticalSectionAndSpinCount(&p->context.cslock,
                                                          4000);
    ...
    fprintf(stderr, "%s\n",
            errno_str("Windows Proton proactor OS resource failure", false)
                .c_str());
...
pn_condition_t *pn_listener_condition(pn_listener_t *l) { return l->condition; }

void *pn_listener_get_context(pn_listener_t *l) { return l->listener_context; }
...
static void release_pending_accepts(pn_listener_t *l) {
    ...
    while (!l->pending_accepts->empty()) {
        ...
        psocket_t *ps =
            (psocket_t *)accept_result->base.iocpd->active_completer;
        accept_result->new_sock->ops_in_progress--;
        ps->iocpd->ops_in_progress--;
        l->context.proactor->reaper->add(accept_result->new_sock);
        begin_accept(accept_result->base.iocpd->acceptor,
                     accept_result); // does proper disposal when closing
    }
    l->pending_events = 0;
}
...
static void recycle_result(accept_result_t *accept_result) {
    ...
    accept_result->new_sock = create_same_type_socket(ps->iocpd);
    {
        ...
        begin_accept(ps->iocpd->acceptor,
                     accept_result); // cleans up if closing
        l->context.completion_ops--;
        if (l->context.closing && listener_maybe_free(l, g)) return;
    }
}

void pn_listener_accept2(pn_listener_t *l, pn_connection_t *c,
                         pn_transport_t *t) {
    accept_result_t *accept_result = NULL;
    DWORD err = 0;
    psocket_t *ps = NULL;
    ...
    {
        csguard g(&l->context.cslock);
        pconnection_t *pc = (pconnection_t *)pn_class_new(
            &pconnection_class, sizeof(pconnection_t));
        assert(pc); // TODO: memory safety
...
static bool proactor_update_batch(pn_proactor_t *p) {
    if (proactor_has_event(p)) return true;
    if (p->need_timeout) {
...
static bool proactor_remove(pcontext_t *ctx) {
    ...
    if (ctx->disconnecting) {
        ...
        if (ctx->prev)
            ...
            p->contexts = ctx->next;
            ctx->next = NULL;
            if (p->contexts) p->contexts->prev = NULL;
        }
        if (ctx->next) {
...
static void proactor_shutdown(pn_proactor_t *p) {
    // Called from free(), no competing threads except our own timer queue
    // callbacks.
    ...
    // Graceful close of the sockets (zombies), closes completion port, free
    // iocp related resources
...
void pn_proactor_disconnect(pn_proactor_t *p, pn_condition_t *cond) {
    ...
    pcontext_t *ctx = NULL;
    {
        ...
        p->contexts = NULL;
        // First pass: mark each pcontext as disconnecting and update global
        // pending count.
        pcontext_t *ctx = disconnecting_pcontexts;
        while (ctx) {
            ctx->disconnecting = true;
            ctx->disconnect_ops =
                2; // Second pass below and proactor_remove(), in any order.
            p->disconnects_pending++;
            ctx = ctx->next;
        }
    }
    ...
    while (next) {
        ...
        CRITICAL_SECTION *ctx_cslock =
            pc ? &pc->context.cslock : &l->context.cslock;
        ...
        if (pc) {
            ...
            if (!ctx->closing) {
                if (ctx->working) {
                    ...
                    if (cond) {
                        pn_condition_copy(
                            pn_transport_condition(pc->driver.transport), cond);
                    }
                    pn_connection_driver_close(&pc->driver);
                }
            }
        ...
        if (do_free) {
            if (pc)
                ...
                listener_final_free(pcontext_listener(ctx));
        }
    }
}
