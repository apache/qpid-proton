...
int pn_proactor_addr(char *buf, size_t len, const char *host,
                     const char *port) {
...
void pni_proactor_set_cond(pn_condition_t *cond, const char *what,
    ...
    if (!pn_condition_is_set(cond)) { /* Preserve older error information */
        pn_condition_format(cond, PNI_IO_CONDITION, "%s - %s %s:%s", msg, what,
                            nonull(host), nonull(port));
    }
}

// Backwards compatibility signatures.

void pn_proactor_connect(pn_proactor_t *p, pn_connection_t *c,
                         const char *addr) {
