...
static void ptimer_set_lh(ptimer_t *pt, uint64_t t_millis) {
    ...
    if (pt->timer_active && oldt.it_value.tv_nsec == 0 &&
        oldt.it_value.tv_sec == 0) {
...
// Return true if timerfd has and will have no pollable expiries in the current
// armed state
...
static void stop_polling(epoll_extended_t *ee, int epollfd) {
    ...
    // closed?
    if (ee->fd == -1 || !ee->polling || epollfd == -1) return;
    struct epoll_event ev = {0};
    ev.data.ptr = ee;
    ev.events = 0;
    memory_barrier(ee);
...
static void pcontext_finalize(pcontext_t *ctx) { pmutex_finalize(&ctx->mutex); }
...
struct pn_proactor_t {
    ...
    bool timeout_set; /* timeout has been set by user and not yet cancelled or
                         generated event */
    bool timeout_processed; /* timeout event dispatched in the most recent event
                               batch */
...
static inline void wake_notify(pcontext_t *ctx) {
    if (ctx->proactor->eventfd == -1) return;
...
static void psocket_init(psocket_t *ps, pn_proactor_t *p,
...
struct pn_listener_t {
    ...
    int pending_count;
    bool unclaimed; /* attach event dispatched but no pn_listener_attach() call
                       yet */
    size_t backlog;
...
static pn_event_batch_t *pconnection_process(pconnection_t *pc, uint32_t events,
                                             bool timeout, bool topup,
                                             bool is_io_2);
...
// Add an overflowing listener to the overflow list. Called with listener
// context lock held.
...
/* TODO aconway 2017-06-08: we should also call proactor_rearm_overflow after a
   fixed delay, even if the proactor has not freed any file descriptors, since
   other parts of the process might have*/

// Activate overflowing listeners, called when there may be available file
// descriptors.
...
static const char *pconnection_setup(pconnection_t *pc, pn_proactor_t *p,
    ...
    /* Set the pconnection_t backpointer last.
       ...
       not reveal themselves to be re-associated with a proactor till setup is
       complete.
     */
...
// Call with lock held and closing == true (i.e. pn_connection_driver_finished()
// == true), timer cancelled. Return true when all possible outstanding epoll
// events associated with this pconnection have been processed.
...
static void pconnection_cleanup(pconnection_t *pc) {
    ...
    if (can_free) pconnection_final_free(pc);
...
static void pconnection_begin_close(pconnection_t *pc) {
    if (!pc->context.closing) {
        ...
        if (ptimer_shutdown(&pc->timer, pc->timer_armed))
            pc->timer_armed = false; // disarmed in the sense that the timer
...
static bool pconnection_rearm_check(pconnection_t *pc) {
    ...
    uint32_t wanted_now =
        (pc->read_blocked && !pconnection_rclosed(pc)) ? EPOLLIN : 0;
    if (!pconnection_wclosed(pc)) {
        if (pc->write_blocked)
            wanted_now |= EPOLLOUT;
        else {
            pn_bytes_t wbuf = pn_connection_driver_write_buffer(&pc->driver);
            if (wbuf.size > 0) wanted_now |= EPOLLOUT;
        }
    }
...
static inline bool pconnection_work_pending(pconnection_t *pc) {
    if (pc->new_events || pc->new_events_2 || pc->wake_count ||
        pc->tick_pending || pc->queued_disconnect)
        return true;
    if (!pc->read_blocked && !pconnection_rclosed(pc)) return true;
    pn_bytes_t wbuf = pn_connection_driver_write_buffer(&pc->driver);
...
static void pconnection_done(pconnection_t *pc) {
    ...
    lock(&pc->context.mutex);
    pc->context.working =
        false; // So we can wake() ourself if necessary.  We remain the de facto
    ...
    if (!pc->write_blocked && !pconnection_wclosed(pc)) {
        ...
        if (wbuf.size > 0) {
            if (!pconnection_write(pc, wbuf)) {
                psocket_error(&pc->psocket, errno,
...
static pn_event_batch_t *pconnection_process(pconnection_t *pc, uint32_t events,
    ...
    if (rearm_timer) pc->timer_armed = false;
...
retry:
    ...
    if (update_events) {
        if (!pc->context.closing) {
            if ((update_events & (EPOLLHUP | EPOLLERR)) &&
                !pconnection_rclosed(pc) && !pconnection_wclosed(pc))
                pconnection_maybe_connect_lh(pc);
            else
                pconnection_connected_lh(
                    pc); /* Non error event means we are connected */
            if (update_events & EPOLLOUT) pc->write_blocked = false;
            if (update_events & EPOLLIN) pc->read_blocked = false;
        }
    }
    ...
    bool rearm_pc = pconnection_rearm_check(
        pc); // holds rearm_mutex until pconnection_rearm() below

    unlock(&pc->context.mutex);
    if (rearm_pc)
        pconnection_rearm(
            pc); // May free pc on another thread.  Return right away.
    return NULL;
}
...
static void configure_socket(int sock) {
    ...
    int tcp_nodelay = 1;
    (void)setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (void *)&tcp_nodelay,
                     sizeof(tcp_nodelay));
}
...
static void pconnection_start(pconnection_t *pc) {
    ...
    (void)getsockname(pc->psocket.sockfd, (struct sockaddr *)&pc->local.ss,
                      &len);

    epoll_extended_t *ee = &pc->psocket.epoll_io;
    if (ee->polling) { /* This is not the first attempt, stop polling and close
                          the old FD */
...
static void pconnection_maybe_connect_lh(pconnection_t *pc) {
    ...
    if (!pc->connected) { /* Not yet connected */
        while (pc->ai) {  /* Have an address */
            ...
            pc->ai =
                pc->ai->ai_next; /* Move to next address in case this fails */
            ...
            /* connect failed immediately, go round the loop to try the next
             * addr */
        }
        freeaddrinfo(pc->addrinfo);
        pc->addrinfo = NULL;
        /* If there was a previous attempted connection, let the poller discover
           the errno from its socket, otherwise set the current error. */
        if (pc->psocket.sockfd < 1) {
...
void pn_proactor_connect2(pn_proactor_t *p, pn_connection_t *c,
    ...
    if (pc->disconnected) {
        ...
        int gai_error =
            pgetaddrinfo(pc->psocket.host, pc->psocket.port, 0, &pc->addrinfo);
        if (!gai_error) {
            pn_connection_open(pc->driver.connection); /* Auto-open */
            pc->ai = pc->addrinfo;
        ...
        for (struct addrinfo *ai = addrinfo; ai; ai = ai->ai_next) {
            ...
            if (fd >= 0) {
                if (!setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &on,
                                sizeof(on)) &&
                    /* We listen to v4/v6 on separate sockets, don't let v6
                       listen for v4 */
                    ...
                    (void)getsockname(
                        fd, (struct sockaddr *)(&acceptor->addr.ss), &len);
                    if (acceptor == l->acceptors) { /* First acceptor, check for
                                                       dynamic port */
                        dynamic_port = check_dynamic_port(
                            ai->ai_addr, pn_netaddr_sockaddr(&acceptor->addr));
                    ...
                    start_polling(
                        &ps->epoll_io,
                        ps->proactor->epollfd); // TODO: check for error
                    l->active_count++;
                    acceptor->armed = true;
                    unlock(&l->rearm_mutex);
                } else {
...
void pn_listener_free(pn_listener_t *l) {
    /* Note at this point either the listener has never been used (freed by
       user) or it has been closed, so all its sockets are closed.
    */
    if (l) {
        bool can_free = true;
    ...
    if (!l->context.closing) {
        ...
        for (size_t i = 0; i < l->acceptors_size; ++i) {
            ...
            if (ps->sockfd >= 0) {
                ...
                if (a->armed) {
                    shutdown(ps->sockfd,
                             SHUT_RD); // Force epoll event and callback
                } else {
...
static void listener_forced_shutdown(pn_listener_t *l) {
    // Called by proactor_free, no competing threads, no epoll activity.
    lock(&l->context.mutex); // needed because of interaction with
                             // proactor_rearm_overflow
    listener_begin_close(l);
    unlock(&l->context.mutex);
    // pconnection_process will never be called again.  Zero everything.
...
/* Accept a connection as part of listener_process(). Called with listener
 * context lock held. */
static void listener_accept_lh(psocket_t *ps) {
    pn_listener_t *l = psocket_listener(ps);
    acceptor_t *acceptor = psocket_acceptor(ps);
    assert(acceptor->accepted_fd <
           0); /* Shouldn't already have an accepted_fd */
...
static pn_event_t *listener_batch_next(pn_event_batch_t *batch) {
    ...
    if (e && pn_event_type(e) == PN_LISTENER_CLOSE) l->close_dispatched = true;
    unlock(&l->context.mutex);
...
void pn_listener_accept2(pn_listener_t *l, pn_connection_t *c,
    ...
    assert(pc); // TODO: memory safety
    const char *err =
        pconnection_setup(pc, pn_listener_proactor(l), c, t, true, "");
    if (err) {
...
static void epoll_secondary_init(epoll_extended_t *ee, int epoll_fd_2,
                                 int epollfd) {
    ee->psocket = NULL;
    ee->fd = epoll_fd_2;
...
// There can be multiple interrupts but only one inside the collector to avoid
// coalescing. Return true if there is an event in the collector.
...
static pn_event_batch_t *proactor_process(pn_proactor_t *p,
                                          pn_event_type_t event) {
    bool timer_fired =
        (event == PN_PROACTOR_TIMEOUT) && ptimer_callback(&p->timer) != 0;
    lock(&p->context.mutex);
    ...
    p->timer_armed = true;
    unlock(&p->context.mutex);
    if (rearm_timer) rearm(p, &p->timer.epoll_io);
    return NULL;
}
...
static pn_event_batch_t *proactor_chained_epoll_wait(pn_proactor_t *p) {
    ...
    if (n < 0) {
        if (errno != EINTR) perror("epoll_wait"); // TODO: proper log
    } else if (n > 0) {
        assert(n == 1);
...
static pn_event_batch_t *process_inbound_wake(pn_proactor_t *p,
                                              epoll_extended_t *ee) {
    if (ee->fd ==
        p->interruptfd) { /* Interrupts have their own dedicated eventfd */
    ...
    if (ctx) {
        ...
        case LISTENER:
            return listener_process(
                &((pn_listener_t *)ctx->owner)->acceptors[0].psocket, 0);
        default:
...
static pn_event_batch_t *proactor_do_epoll(struct pn_proactor_t *p,
                                           bool can_block) {
        ...
        if (ee->type == WAKE) {
            ...
            if (pc) {
                if (ee->type == PCONNECTION_IO) {
                    batch =
                        pconnection_process(pc, ev.events, false, false, false);
                } else {
                    assert(ee->type == PCONNECTION_TIMER);
                    batch = pconnection_process(pc, 0, true, false, false);
                }
            } else {
                // TODO: can any of the listener processing be parallelized like
                // IOCP?
...
void pn_proactor_done(pn_proactor_t *p, pn_event_batch_t *batch) {
    ...
    if (bp == p) {
        ...
        if (p->timeout_processed) {
            ...
            if (wake_if_inactive(p)) notify = true;
        }
        proactor_update_batch(p);
        if (proactor_has_event(p))
            if (wake(&p->context)) notify = true;
        unlock(&p->context.mutex);
        if (notify) wake_notify(&p->context);
        if (rearm_timer) rearm(p, &p->timer.epoll_io);
        return;
...
void pn_proactor_disconnect(pn_proactor_t *p, pn_condition_t *cond) {
    ...
    p->contexts = NULL;
    // First pass: mark each pcontext as disconnecting and update global pending
    // count.
    ...
    while (next) {
        ...
        if (--ctx->disconnect_ops == 0) {
            ...
            if (ctx_notify) ctx_notify = wake(ctx);
        }
        unlock(&p->context.mutex);
        unlock(ctx_mutex);
