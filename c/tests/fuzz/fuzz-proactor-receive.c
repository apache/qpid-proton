...
void serve_data(const uint8_t *Data, size_t Size) {
    ...
    if (bind(sockfd, (struct sockaddr *)&self, sizeof(self)) != 0) {
        ...
        // Lets unblock the old child that listens by starting new client to
        // read from it. It breaks the fuzzing somewhat, but it is better to
        // mess up one than many inputs.
