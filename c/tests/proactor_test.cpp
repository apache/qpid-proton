...
TEST_CASE("proactor_connection_wake") {
    ...
    for (pn_event_type_t et = p.run(); et != PN_PROACTOR_INACTIVE;
         et = p.run()) {
        switch (et) {
        case PN_TRANSPORT_ERROR:
    ...
    bool handle(pn_event_t *e) {
        ...
        case PN_CONNECTION_REMOTE_OPEN:
            ...
            pn_transport_close_tail(
                pn_connection_transport(pn_event_connection(e)));
...
#define EXPECT_CONNECT(LISTENER, HOST)                                         \
    ...
    /* Listen on ipv6 loopback, if it fails skip ipv6 tests.
       ...
       platforms may allow listening on "::" without complaining. However they
       won't have a local ipv6 loopback configured, so "::1" will force an
       error.
    */
...
struct ssl_handler : public common_handler {
    ...
    bool handle(pn_event_t *e) {
        ...
        case PN_CONNECTION_BOUND:
            CHECK(0 ==
                  pn_ssl_init(pn_ssl(pn_event_transport(e)), ssl_domain, NULL));
            return false;
...
TEST_CASE("proactor_netaddr") {
    ...
    CHECK(0 ==
          pn_netaddr_host_port(na, host, sizeof(host), serv, sizeof(serv)));
    ...
    bool handle(pn_event_t *e) {
        ...
        case PN_DELIVERY: { /* Receive a delivery - smaller than a
            ...
            if (pn_delivery_readable(dlv)) {
                ...
                REQUIRE(n == pn_link_recv(pn_event_link(e),
                                          recv_buf.start + received, n));
                received += n;
            }
