...
*/
#define debug(...)                                                             \
    if (debug_enable) debug_impl(__VA_ARGS__)
...
#define DECLARE_POOL(P, T)                                                     \
    ...
    void P##_init(P *p, size_t max, void (*free_fn)(T *)) {                    \
        pool_init((pool *)p, max, (pool_free_fn)free_fn);                      \
    }                                                                          \
    bool P##_add(P *p, T *entry) {                                             \
        return pool_add((pool *)p, &entry->entry);                             \
    }                                                                          \
...
typedef struct connection_ctx {
    ...
    pthread_mutex_t
        lock; /* Lock PN_TRANSPORT_CLOSED vs. pn_connection_wake() */
} connection_ctx;
...
static bool handle(global *g, pn_event_t *e) {
    ...
    connection_ctx *cctx =
        c ? (connection_ctx *)pn_connection_get_context(c) : NULL;
    ...
    case PN_LISTENER_OPEN: {
        ...
        cpool_connect(&g->connections_active, g->proactor,
                      lctx->addr); /* Initial connection */
        break;
    }
...
void usage(const char **argv, const char **arg) {
    ...
    fprintf(stderr, "  -time TIME: total run-time in seconds (default %d)\n",
            default_runtime);
    fprintf(stderr,
            "  -threads THREADS: total number of threads (default %d)\n",
            default_threads);
    fprintf(stderr, "  -debug: print debug messages\n");
    fprintf(stderr,
            "Flags to enable specific actions (all enabled by default)\n");
    fprintf(stderr, " ");
    for (int i = 0; i < (int)action_size; ++i)
        fprintf(stderr, " -%s", action_name[i]);
    fprintf(stderr, "\n");
    fprintf(stderr,
            "Flags to disable specific actions (all enabled by default)\n");
    for (int i = 0; i < (int)action_size; ++i)
        fprintf(stderr, " -no-%s", action_name[i]);
    fprintf(stderr, "\n\n");
...
int main(int argc, const char *argv[]) {
    ...
    while (arg < end) {
        if (!strcmp(*arg, "-time") && ++arg < end) {
            ...
            if (threads % 2)
                threads += 1; /* Round up to even: half proactor, half user */
        } else if (!strcmp(*arg, "-debug")) {
            debug_enable = true;
        } else if (!strncmp(*arg, "-no-", 4)) {
            action_enabled[find_action((*arg) + 4, argv, arg)] = false;
        } else if (!strncmp(*arg, "-", 1)) {
            if (action_default) { /* First enable-action flag, switch to
                                     default-off */
                action_default = false;
                for (size_t i = 0; i < action_size; ++i)
                    action_enabled[i] = action_default;
            }
    ...
    pthread_t *user_threads =
        (pthread_t *)calloc(threads / 2, sizeof(pthread_t));
    pthread_t *proactor_threads =
        (pthread_t *)calloc(threads / 2, sizeof(pthread_t));
    int i;
    ...
    for (i = 0; i < threads / 2; ++i)
        pthread_join(proactor_threads[i], &ignore);
