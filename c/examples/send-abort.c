...
static bool handle(app_data_t *app, pn_event_t *event) {
        ...
        while (app->in_progress ||
            ...
            if (!app->in_progress) {
                ...
                pn_delivery(sender, pn_dtag((const char *)&app->sent,
                                            sizeof(app->sent)));
                pn_link_send(sender, msgbuf.start,
                             msgbuf.size -
                                 HOLDBACK); /* Send some part of message */
                app->in_progress = true;
                /* Return from this link flow event and abort the message on
                 * next, */
                break;
            } else {
    ...
    case PN_DELIVERY: {
        ...
        fprintf(stderr,
                "Aborted deliveries should not receive delivery events. "
                "Delivery state %d\n",
    ...
    case PN_TRANSPORT_CLOSED:
        check_condition(event,
                        pn_transport_condition(pn_event_transport(event)));
        break;
