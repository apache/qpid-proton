...
/* The ssl-certs subdir must be in the current directory for an ssl-enabled
 * broker */
...
#define SET_CREDENTIALS(DOMAIN, NAME)                                          \
    pn_ssl_domain_set_credentials(DOMAIN, SSL_FILE(NAME "-full.p12"), "",      \
                                  SSL_PW)
#else
...
typedef struct queue_t {
    ...
    VEC(pn_connection_t *)
    waiting; /* Connections waiting to send messages from this queue */
...
static void queue_destroy(queue_t *q) {
    ...
    for (i = 0; i < q->messages.len; ++i) free(q->messages.data[i].start);
    VEC_FINAL(q->messages);
    for (i = 0; i < q->waiting.len; ++i) pn_decref(q->waiting.data[i]);
    VEC_FINAL(q->waiting);
    free(q->name);
}
...
static void queue_send(queue_t *q, pn_link_t *s) {
    ...
    if (m.start) {
        ...
        pn_link_advance(s);
        pn_delivery_settle(
            d); /* Pre-settled: unreliable, there will be no ack/ */
        free(m.start);
    }
}
...
/* Use a buffer per link to accumulate message data - message can arrive in
   multiple deliveries, and the broker can receive messages on many
   concurrently. */
...
static void session_unsub(broker_t *b, pn_session_t *ssn) {
    ...
    pn_link_t *l;
    for (l = pn_link_head(c, 0); l != NULL; l = pn_link_next(l, 0)) {
        if (pn_link_session(l) == ssn) link_unsub(b, l);
    }
}
...
const int WINDOW =
    5; /* Very small incoming credit window, to show flow control in action */
...
static bool handle(broker_t *b, pn_event_t *e) {
    ...
    case PN_LISTENER_ACCEPT: {
        /* Configure a transport to allow SSL and SASL connections. See
         * ssl_domain setup in main() */
    ...
    case PN_CONNECTION_WAKE: {
        if (get_check_queues(c)) {
            ...
            for (l = pn_link_head(c, flags); l != NULL;
                 l = pn_link_next(l, flags))
                link_send(b, l);
        }
        break;
    }
    ...
    case PN_LINK_REMOTE_OPEN: {
        ...
        if (pn_link_is_sender(l)) {
            const char *source =
                pn_terminus_get_address(pn_link_remote_source(l));
            pn_terminus_set_address(pn_link_source(l), source);
        } else {
            const char *target =
                pn_terminus_get_address(pn_link_remote_target(l));
    ...
    case PN_LINK_FINAL: {
        pn_rwbytes_t *buf =
            (pn_rwbytes_t *)pn_link_get_context(pn_event_link(e));
        if (buf) {
            free(buf->start);
            free(buf);
        }
    ...
    case PN_DELIVERY: { /* Incoming message data */
        ...
        if (pn_delivery_readable(d)) {
            ...
            pn_rwbytes_t *m =
                message_buffer(l); /* Append data to incoming message buffer */
            ssize_t recv;
            m->size += size;
            ...
            if (recv == PN_ABORTED) { /*  */
                ...
                pn_delivery_settle(d); /* Free the delivery so we can receive
                                          the next message */
                pn_link_flow(
                    l, WINDOW -
                           pn_link_credit(
                               l)); /* Replace credit for the aborted message */
    ...
    case PN_CONNECTION_REMOTE_CLOSE:
        check_condition(e,
                        pn_connection_remote_condition(pn_event_connection(e)));
...
int main(int argc, char **argv) {
    ...
    pn_ssl_domain_allow_unsecured_client(
        b.ssl_domain); /* Allow SSL and plain connections */
    {
        /* Listen on addr */
