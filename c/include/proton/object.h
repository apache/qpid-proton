...
#define PN_CLASSDEF(PREFIX)                                                    \
    ...
    const pn_class_t *PREFIX##__class(void) {                                  \
        static const pn_class_t clazz = {#PREFIX,                              \
                                         CID_##PREFIX,                         \
                                         pn_object_new,                        \
...
#define PN_CLASS(PREFIX)                                                       \
    {                                                                          \
#PREFIX, CID_##PREFIX, pn_object_new, PREFIX##_initialize,             \
            pn_object_incref, pn_object_decref, pn_object_refcount,            \
            PREFIX##_finalize, pn_object_free, pn_object_reify,                \
            PREFIX##_hashcode, PREFIX##_compare, PREFIX##_inspect              \
    }

#define PN_METACLASS(PREFIX)                                                   \
    {                                                                          \
#PREFIX, CID_##PREFIX, PREFIX##_new, PREFIX##_initialize,              \
            PREFIX##_incref, PREFIX##_decref, PREFIX##_refcount,               \
            PREFIX##_finalize, PREFIX##_free, PREFIX##_reify,                  \
            PREFIX##_hashcode, PREFIX##_compare, PREFIX##_inspect              \
    }

/* Class to identify a plain C struct in a pn_event_t. No refcounting or memory
 * management. */
#define PN_STRUCT_CLASSDEF(PREFIX, CID)                                        \
    const pn_class_t *PREFIX##__class(void);                                   \
    static const pn_class_t *PREFIX##_reify(void *p) {                         \
        return PREFIX##__class();                                              \
    }                                                                          \
    const pn_class_t *PREFIX##__class(void) {                                  \
        static const pn_class_t clazz = {#PREFIX,                              \
                                         CID,                                  \
...
PN_EXTERN pn_hash_t *pn_hash(const pn_class_t *clazz, size_t capacity,
                             float load_factor);
...
#endif
    ;
PN_EXTERN int pn_string_vaddf(pn_string_t *string, const char *format,
                              va_list ap);
...
/**
   PN_HANDLE is a trick to define a unique identifier by using the address of a
   static variable. You MUST NOT use it in a .h file, since it must be defined
   uniquely in one compilation unit. Your .h file can provide access to the
   handle (if needed) via a function. For example:
...
PN_EXTERN void pn_record_def(pn_record_t *record, pn_handle_t key,
