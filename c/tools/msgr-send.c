...
static void usage(int rc) {
    printf(
        ...
        " -p # \tSend batches of # messages (wait for replies before sending "
        "next batch if -R) [1024]\n"
...
static void parse_options(int argc, char **argv, Options_t *opts) {
    ...
    while ((c = getopt(argc, argv, "a:c:b:p:w:e:l:Rt:W:B:VN:T:C:K:P:")) != -1) {
        switch (c) {
        case 'a':
...
static int process_replies(pn_messenger_t *messenger, pn_message_t *message,
                           Statistics_t *stats, int max_count) {
    int received = 0;
...
int main(int argc, char **argv) {
    ...
    while (!opts.msg_count || (sent < opts.msg_count)) {
        ...
        sent++;
        if (opts.send_batch &&
            (pn_messenger_outgoing(messenger) >= (int)opts.send_batch)) {
            if (get_replies) {
                while (received < sent) {
                ...
                check((rc == 0 || rc == PN_TIMEOUT),
                      "pn_messenger_send() failed");
            }
        }
        check_messenger(messenger);
    }
    ...
    if (get_replies) {
        ...
        while (received < sent) {
            int count = process_replies(messenger, reply_message, &stats,
                                        opts.recv_count);
