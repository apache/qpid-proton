...
static void usage(int rc) {
    printf(
        ...
        " -F <addr>[,<addr>]* \tAddresses used for forwarding received "
        "messages\n"
        " -N <name> \tSet the container name to <name>\n"
        " -X <text> \tPrint '<text>\\n' to stdout after all subscriptions are "
        "created\n"
...
static void parse_options(int argc, char **argv, Options_t *opts) {
    ...
    while ((c = getopt(argc, argv, "a:c:b:w:t:e:RW:F:VN:X:T:C:K:P:")) != -1) {
        ...
        case 'W':
            ...
            break;
        case 'F':
            addresses_merge(&opts->forwarding_targets, optarg);
            break;
        case 'V':
            enable_logging();
            break;
...
int main(int argc, char **argv) {
    ...
    while (!opts.msg_count || received < opts.msg_count) {
        ...
        check_messenger(messenger);
        check(rc == 0 || (opts.timeout == 0 && rc == PN_TIMEOUT),
              "pn_messenger_recv() failed");
        ...
        while (pn_messenger_incoming(messenger)) {
            ...
            // uint64_t id = pn_message_get_correlation_id( message
            // ).u.as_ulong;
            ...
            if (forward) {
                const char *forward_addr =
                    opts.forwarding_targets.addresses[forwarding_index];
                forwarding_index =
                    NEXT_ADDRESS(opts.forwarding_targets, forwarding_index);
                LOG("Forwarding to: %s\n", forward_addr);
                pn_message_set_address(message, forward_addr);
                pn_message_set_reply_to(message,
                                        NULL); // else points to origin sender
                pn_message_set_creation_time(message, msgr_now());
                pn_messenger_put(messenger, message);
                sent++;
            }
