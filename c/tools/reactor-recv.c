...
static char *ensure_buffer(char *buf, size_t needed, size_t *actual) {
    ...
    if (needed + 1024 <= *actual) return buf;
    needed += 2048;
...
void global_shutdown(global_context_t *gc) {
    ...
    for (size_t i = 0; i < n; i++) {
        pn_connection_t *conn =
            (pn_connection_t *)pn_list_get(gc->active_connections, i);
...
void connection_dispatch(pn_handler_t *h, pn_event_t *event,
    ...
    case PN_LINK_REMOTE_OPEN: {
        ...
        if (pn_link_is_receiver(link)) {
            check(cc->recv_link == NULL,
                  "Multiple incoming links on one connection");
            cc->recv_link = link;
            ...
            if (replying) {
                // Set up a reply link and defer granting credit to the incoming
                // link
                pn_connection_t *conn =
    ...
    case PN_LINK_FLOW: {
        if (replying) {
            ...
            int delta =
                pn_link_credit(reply_link) - pn_link_credit(cc->recv_link);
            if (delta > 0) pn_link_flow(cc->recv_link, delta);
        }
    } break;
    case PN_DELIVERY: {
        ...
        if (pn_link_is_receiver(recv_link) && !pn_delivery_partial(dlv)) {
            ...
            cc->global->encoded_data =
                ensure_buffer(cc->global->encoded_data, encoded_size,
                              &cc->global->encoded_data_size);
            check(cc->global->encoded_data, "decoding buffer realloc failure");

            ssize_t n =
                pn_link_recv(recv_link, cc->global->encoded_data, encoded_size);
            ...
            if (replying) {
                ...
                if (reply_addr) {
                    ...
                    check(
                        pn_link_credit(rl) > 0,
                        "message received without corresponding reply credit");
                    LOG("Replying to: %s\n", reply_addr);
                    ...
                    *((uint64_t *)ptr) = cc->global->sent;
                    ...
                    int err =
                        pn_message_encode(msg, cc->global->encoded_data, &size);
...
pn_handler_t *connection_handler(global_context_t *gc) {
    pn_handler_t *h = pn_handler_new(
        connection_dispatch, sizeof(connection_context_t), connection_cleanup);
    connection_context_t *cc = connection_context(h);
...
void start_listener(global_context_t *gc, pn_reactor_t *reactor) {
    ...
    if (port == 0 || strlen(port) == 0) port = "5672";
    if (host == 0 || strlen(host) == 0) host = "0.0.0.0";
    if (*host == '~') host++;
...
pn_handler_t *listener_handler(Options_t *opts, Statistics_t *stats) {
    pn_handler_t *h = pn_handler_new(
        listener_dispatch, sizeof(global_context_t), listener_cleanup);
    global_context_t *gc = global_context(h);
...
static void parse_options(int argc, char **argv, Options_t *opts) {
    ...
    while ((c = getopt(argc, argv, "a:c:b:w:t:e:RW:F:VN:X:T:C:K:P:")) != -1) {
        switch (c) {
        case 'a': {
            // TODO: multiple addresses?
            char *comma = strchr(optarg, ',');
            check(comma == 0, "multiple addresses not implemented");
            check(opts->subscriptions.count == 0,
                  "multiple addresses not implemented");
