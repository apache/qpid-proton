...
typedef struct {
    ...
    int get_replies;
    int unique_message; // 1 -> create and free a pn_message_t for each
                        // send/recv
...
void sender_context_init(sender_context_t *sc, Options_t *opts,
                         Statistics_t *stats) {
    sc->opts = opts;
    sc->stats = stats;
    sc->sent = 0;
    ...
    if (port && strlen(port)) pn_string_addf(sc->hostname, ":%s", port);
}
...
pn_message_t *get_message(sender_context_t *sc, bool sending) {
    if (sc->opts->unique_message) {
        ...
        return sending ? sc->message
                       : sc->reply_message; // our simplified "message pool"
}

void return_message(sender_context_t *sc, pn_message_t *m) {
    if (sc->opts->unique_message) pn_message_free(m);
}
...
void sender_dispatch(pn_handler_t *h, pn_event_t *event, pn_event_type_t type) {
    ...
    case PN_LINK_FLOW: {
        ...
        while (pn_link_credit(snd) > 0 && sc->sent < sc->opts->msg_count) {
            if (sc->sent == 0) statistics_start(sc->stats);
    ...
    case PN_LINK_INIT: {
        ...
        if (pn_link_is_receiver(link)) {
            // Response messages link.  Could manage credit and deliveries in
            // this handler but a dedicated handler also works.
...
void replyto_dispatch(pn_handler_t *h, pn_event_t *event,
    ...
    case PN_DELIVERY: {
        ...
        if (pn_link_is_receiver(recv_link) && !pn_delivery_partial(dlv)) {
            ...
            check(encoded_size <= sc->encoded_data_size,
                  "decoding buffer too small");
...
pn_handler_t *replyto_handler(sender_context_t *sc) {
    pn_handler_t *h = pn_handler_new(
        replyto_dispatch, sizeof(sender_context_t *), replyto_cleanup);
    sender_context_t **p = (sender_context_t **)pn_handler_mem(h);
    *p = sc;
    return h;
}
...
static void parse_options(int argc, char **argv, Options_t *opts) {
    ...
    while ((c = getopt(argc, argv, "ua:c:b:p:w:e:l:Rt:W:B:VN:T:C:K:P:")) !=
           -1) {
        switch (c) {
        case 'a': {
            // TODO: multiple addresses?  To keep tests happy, accept multiple
            // for now, but ignore all but the first.
        ...
        case 'C':
            ...
            break;
        case 'K':
            opts->privatekey = optarg;
            break;
        case 'P':
            parse_password(optarg, &opts->password);
            break;
    ...
    if (opts->targets.count == 0)
        addresses_add(&opts->targets, "amqp://0.0.0.0");
}
