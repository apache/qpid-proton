...
public class TransportImpl extends EndpointImpl implements ProtonJTransport, FrameBody.FrameBodyHandler<Integer>, FrameHandler, TransportOutputWriter
{
    ...
    TransportImpl(int maxFrameSize)
    {
        ...
        _frameWriter = new FrameWriter(_encoder, _remoteMaxFrameSize, FrameWriter.AMQP_FRAME_TYPE, _protocolTracer, this);
    }
    ...
    public void setChannelMax(int channelMax)
    {
        ...
        {
            throw new NumberFormatException("Value \"" + channelMax +
                    "\" lies outside the range [0-" + (1 << 16) + ").");
        }
    ...
    private boolean processTransportWorkSender(DeliveryImpl delivery, SenderImpl snd)
    {
                ...
                (tpLink.getLocalHandle() != null) && !_frameWriter.isFull())
        {
            ...
            writeFrame(tpSession.getLocalChannel(), transfer, payload, new PartialTransfer(transfer));
        ...
        if ((_connectionEndpoint != null) && _isOpenSent && !_isCloseSent)
        {
            ...
            while (endpoint != null)
            {
                if (endpoint instanceof LinkImpl)
                {
                    ...
                    {
                        if (((link.getRemoteState() == EndpointState.ACTIVE) && !transportLink.isLocalHandleSet()) || (link.getRemoteState() == EndpointState.UNINITIALIZED))
                        {
                            SessionImpl session = link.getSession();
                            TransportSession transportSession = getTransportState(session);
    ...
    {
        if (!_isOpenSent && ((_condition != null) || ((_connectionEndpoint != null) && (_connectionEndpoint.getLocalState() != EndpointState.UNINITIALIZED))))
        {
            Open open = new Open();
    ...
    {
        if (((_condition != null) || ((_connectionEndpoint != null) && (_connectionEndpoint.getLocalState() == EndpointState.CLOSED))) && !_isCloseSent)
        {
    ...
    protected void writeFrame(int channel, FrameBody frameBody, ByteBuffer payload, Runnable onPayloadTooLarge)
    {
    ...
    public void closed(TransportException error)
    {
        if (!_closeReceived || (error != null))
        {
            if (error == null)
            {
                _condition = new ErrorCondition(ConnectionError.FRAMING_ERROR, "connection aborted");
            } else
            {
                _condition = new ErrorCondition(ConnectionError.FRAMING_ERROR, error.toString());
            }
    ...
    public long tick(long now)
    {
        ...
        if (_localIdleTimeout > 0)
        {
            ...
            } else if (_localIdleDeadline <= now)
            {
                ...
                if ((_connectionEndpoint != null) && (_connectionEndpoint.getLocalState() != EndpointState.CLOSED))
                {
    ...
    void log(String event, TransportFrame frame)
    {
        if (isTraceFramesEnabled())
        {
            ...
            msg.append("[").append(System.identityHashCode(this)).append(":").append(frame.getChannel()).append("]");
