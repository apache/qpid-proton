...
class SaslFrameParser2
{
    ...
    SaslFrameParser2(SaslImpl2 sasl, ByteArrayDecoder decoder)
    {
        _sasl = sasl;
        _decoder = decoder;
    }
        ...
        while(input.hasRemaining() && state != State.ERROR && !_sasl.isDone())
        {
            switch(state)
            {
                ...
                case PARSING:
                    ...
                    {
                        // temp workaround
                        byte[] body = new byte[input.remaining()];
                        input.get(body);
                        ...
                        if(input.hasRemaining())
                        {                          
                            payloadBytes = new byte[input.remaining()];
                            input.get(payloadBytes);
                        }
                        
                        if(obj instanceof SaslBody)
                        {
                            SaslBody frameBody = (SaslBody) obj;
                            _sasl.handle(frameBody, payloadBytes);
                        ...
                        else
                        {
                            ...
                            frameParsingError = new TransportException("Unexpected frame type encountered."
                                                                       ...
                                                                       obj == null ? "null" : obj.getClass(), SaslBody.class);
                        }
                    }
