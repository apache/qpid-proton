...
class options {
  public:
    options(int argc, char const *const *argv)
        : argc_(argc), argv_(argv), prog_(argv[0]), help_() {
    ...
    ~options() {
        for (opts::iterator i = opts_.begin(); i != opts_.end(); ++i) delete *i;
    }

    /** Updates value when parse() is called if option is present with a value.
     */
    template <class T>
    void add_value(T &value, char short_name, const std::string &long_name,
                   const std::string &description, const std::string var) {
        opts_.push_back(new option_value<T>(value, short_name, long_name,
                                            description, var));
    }

    /** Sets flag when parse() is called if option is present. */
    void add_flag(bool &flag, char short_name, const std::string &long_name,
                  const std::string &description) {
        opts_.push_back(
            new option_flag(flag, short_name, long_name, description));
    }

    /** Parse the command line, return the index of the first non-option
     *argument.
    ...
    int parse() {
        ...
        for (; arg < argc_ && argv_[arg][0] == '-'; ++arg) {
            ...
            while (i != opts_.end() && !(*i)->parse(argc_, argv_, arg)) ++i;
            if (i == opts_.end())
  ...
  private:
    class option {
      public:
        option(char s, const std::string &l, const std::string &d,
               const std::string v)
    ...
    template <class T> class option_value : public option {
      public:
        option_value(T &value, char s, const std::string &l,
                     const std::string &d, const std::string &v)
        ...
        bool parse(int argc, char const *const *argv, int &i) {
            ...
            if (arg.compare(0, long_.size(), long_) == 0 &&
                arg[long_.size()] == '=') {
    ...
    class option_flag : public option {
      public:
        option_flag(bool &flag, const char s, const std::string &l,
                    const std::string &d)
