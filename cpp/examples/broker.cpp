...
//
// This broker is multithread safe and if compiled with C++11 with a
// multithreaded Proton binding library will use as many threads as there are
// thread resources available (usually cores)
//
...
//     BoundQueue message back to the connection.
...
class Sender : public proton::messaging_handler {
  ...
  public:
    ...
    bool add(proton::work f) { return work_queue_.add(f); }
...
class Queue {
    ...
    void tryToSend() {
        DOUT(std::cerr << "Queue:    " << this
                       << " tryToSend: " << subscriptions_.size(););
        ...
        while (!messages_.empty() && outOfCredit < subscriptions_.size()) {
            ...
            if (current_->second > 0) {
                ...
                current_->first->add(make_work(
                    &Sender::sendMsg, current_->first, messages_.front()));
                messages_.pop_front();
                --current_->second;
                ++current_;
            } else {
  ...
  public:
    ...
    void unsubscribe(Sender *s) {
        ...
        if (current_ != subscriptions_.end() && current_->first == s)
            ++current_;
...
class QueueManager {
  ...
  public:
    ...
    template <class T> void findQueue(T &connection, std::string &qn) {
        if (qn.empty()) {
    ...
    void findQueueSender(Sender *s, std::string qn) { findQueue(*s, qn); }

    void findQueueReceiver(Receiver *r, std::string qn) { findQueue(*r, qn); }
};
...
class connection_handler : public proton::messaging_handler {
  ...
  public:
    connection_handler(QueueManager &qm) : queue_manager_(qm) {}
    ...
    void on_sender_open(proton::sender &sender) OVERRIDE {
        std::string qn =
            sender.source().dynamic() ? "" : sender.source().address();
        Sender *s = new Sender(sender, senders_);
        senders_[sender] = s;
        queue_manager_.add(
            make_work(&QueueManager::findQueueSender, &queue_manager_, s, qn));
    }
    ...
    void on_receiver_open(proton::receiver &receiver) OVERRIDE {
        ...
        queue_manager_.add(make_work(&QueueManager::findQueueReceiver,
  ...
  private:
    struct listener : public proton::listen_handler {
        ...
        proton::connection_options on_accept(proton::listener &) OVERRIDE {
            return proton::connection_options().handler(
                *(new connection_handler(queues_)));
        }
