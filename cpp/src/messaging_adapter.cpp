...
void message_decode(message &msg, proton::delivery delivery) {
    ...
    if (buf.empty()) throw error("message decode: no delivery pending on link");
    proton::receiver link = delivery.receiver();
    assert(!buf.empty());
    ssize_t n =
        pn_link_recv(unwrap(link), const_cast<char *>(&buf[0]), buf.size());
    ...
    if (pn_link_is_receiver(lnk)) {
        ...
        if (!pn_delivery_partial(dlv) && pn_delivery_readable(dlv)) {
            ...
            if (pn_link_state(lnk) & PN_LOCAL_CLOSED) {
                if (lctx.auto_accept) d.release();
            } else {
                handler.on_message(d, msg);
                if (lctx.auto_accept &&
                    pn_delivery_local_state(dlv) == 0) // Not set by handler
                    d.accept();
...
void on_link_local_open(messaging_handler &handler, pn_event_t *event) {
    ...
    } else if (pn_link_state(lnk) & PN_REMOTE_ACTIVE &&
               pn_link_credit(lnk) > 0) {
        sender s(make_wrapper<sender>(lnk));
        handler.on_sendable(s);
    }
}
...
void messaging_adapter::dispatch(messaging_handler &handler,
                                 pn_event_t *event) {
    ...
    case PN_CONNECTION_BOUND:
        on_connection_bound(handler, event);
        break;
    case PN_CONNECTION_REMOTE_OPEN:
        on_connection_remote_open(handler, event);
        break;
    case PN_CONNECTION_REMOTE_CLOSE:
        on_connection_remote_close(handler, event);
        break;

    case PN_SESSION_REMOTE_OPEN:
        on_session_remote_open(handler, event);
        break;
    case PN_SESSION_REMOTE_CLOSE:
        on_session_remote_close(handler, event);
        break;

    case PN_LINK_LOCAL_OPEN:
        on_link_local_open(handler, event);
        break;
    case PN_LINK_REMOTE_OPEN:
        on_link_remote_open(handler, event);
        break;
    case PN_LINK_REMOTE_CLOSE:
        on_link_remote_close(handler, event);
        break;
    case PN_LINK_REMOTE_DETACH:
        on_link_remote_detach(handler, event);
        break;
    case PN_LINK_FLOW:
        on_link_flow(handler, event);
        break;

    case PN_DELIVERY:
        on_delivery(handler, event);
        break;

    case PN_TRANSPORT_CLOSED:
        on_transport_closed(handler, event);
        break;

    case PN_CONNECTION_WAKE:
        on_connection_wake(handler, event);
        break;
