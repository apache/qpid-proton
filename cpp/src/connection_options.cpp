...
connection_options::connection_options() : impl_(new connection_options_impl()) {}

connection_options::connection_options(class messaging_handler& h) : impl_(new connection_options_impl()) { handler(h); }

connection_options::connection_options(const connection_options& x) : impl_(new connection_options_impl()) {
    *this = x;
}
...
void connection_options_impl::apply_unbound(connection& c) {
...
void connection_options_impl::apply_unbound_client(pn_transport_t *t) {
    apply_sasl(t);
    apply_ssl(t, true);
    apply_transport(t);
}

void connection_options_impl::apply_unbound_server(pn_transport_t *t) {
    apply_sasl(t);
    apply_ssl(t, false);
    apply_transport(t);
}

void connection_options_impl::apply_transport(pn_transport_t* pnt) {
...
void connection_options_impl::apply_sasl(pn_transport_t* pnt) {
...
void connection_options_impl::apply_ssl(pn_transport_t* pnt, bool client) {
    // Transport options.  pnt is NULL between reconnect attempts
    // and if there is a pipelined open frame.
    if (!pnt) return;
...
void connection_options_impl::update(const connection_options_impl& x) {
