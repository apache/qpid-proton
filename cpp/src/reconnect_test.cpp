...
class server_connection_handler : public proton::messaging_handler {
    ...
    struct listen_handler : public proton::listen_handler {
        ...
        listen_handler(proton::messaging_handler &h, waiter &w)
            : listen_waiter(w) {
            opts.handler(h);
        }
        ...
        on_accept(proton::listener &) PN_CPP_OVERRIDE {
            return opts;
        }
    };
    ...
    void close(proton::connection &c) {
        ...
        c.close(proton::error_condition("amqp:connection:forced",
                                        "Failover testing"));
        closing_ = true;
    }
  ...
  public:
    ...
    void on_transport_error(proton::transport &) PN_CPP_OVERRIDE {
        ...
        // - this will stop the listener if we didn't already accept a
        // connection
        listener_.stop();
    }
};
...
class tester : public proton::messaging_handler, public waiter {
  public:
    tester()
        : waiter(3), container_(*this, "reconnect_client"), start_count(0),
          open_count(0), reconnecting_count(0), link_open_count(0),
          transport_error_count(0), transport_close_count(0) {}
    ...
    void ready() PN_CPP_OVERRIDE {
        ...
        container_.connect(
            s1->url(), proton::connection_options().reconnect(
                           proton::reconnect_options().failover_urls(urls)));
    }
...
class stop_reconnect_tester : public proton::messaging_handler {
  public:
    stop_reconnect_tester() : container_(*this, "reconnect_tester") {}

    void deferred_stop() { container_.stop(); }

    void on_container_start(proton::container &c) PN_CPP_OVERRIDE {
        proton::reconnect_options reconnect_options;
        c.connect("this-is-not-going-to work.com",
                  proton::connection_options().reconnect(reconnect_options));
        c.schedule(
            proton::duration::SECOND,
            proton::make_work(&stop_reconnect_tester::deferred_stop, this));
    }
...
class authfail_reconnect_tester : public proton::messaging_handler,
                                  public waiter {
  public:
    ...
    void on_container_start(proton::container &c) PN_CPP_OVERRIDE {
        ...
        c.schedule(
            proton::duration::SECOND,
            proton::make_work(&authfail_reconnect_tester::deferred_stop, this));
    }
...
class test_reconnecting_close : public proton::messaging_handler,
                                public waiter {
  public:
    test_reconnecting_close()
        : waiter(1), container_(*this, "test_reconnecting_close"),
    ...
    void ready() PN_CPP_OVERRIDE {
        container_.connect(s1->url(), proton::connection_options().reconnect(
