...
void test_default_file() {
    ...
    ofstream("testdata/.config/messaging/connect.json")
        << "{ \"host\": \".config\" }" << endl;
    ...
    remove("connect.json");
    ASSERT_EQUAL("testdata/.config/messaging/connect.json",
                 connect_config::default_file());
...
void test_addr() {
    ...
    ASSERT_EQUAL("foo:bar",
                 configure(opts, "{ \"host\":\"foo\", \"port\":\"bar\" }"));
    ASSERT_EQUAL("foo:1234",
                 configure(opts, "{ \"host\":\"foo\", \"port\":\"1234\" }"));
    ASSERT_EQUAL("localhost:amqps", configure(opts, "{}"));
    ASSERT_EQUAL("localhost:amqp", configure(opts, "{\"scheme\":\"amqp\"}"));
    ASSERT_EQUAL(
        "foo:bar",
        configure(opts, "{ \"host\":\"foo\", /* inline comment "
                        "*/\"port\":\"bar\" // end of line comment\n}"));

    ASSERT_THROWS_MSG(error, "'scheme' must be",
                      configure(opts, "{\"scheme\":\"bad\"}"));
    ASSERT_THROWS_MSG(error, "'scheme' expected string, found boolean",
                      configure(opts, "{\"scheme\":true}"));
    ASSERT_THROWS_MSG(error, "'port' expected string or uint, found boolean",
                      configure(opts, "{\"port\":true}"));
    ASSERT_THROWS_MSG(error, "'host' expected string, found boolean",
                      configure(opts, "{\"host\":true}"));
}
...
void test_invalid() {
    ...
    ASSERT_THROWS_MSG(proton::error, "Missing ','",
                      configure(opts, RAW_STRING({
                                    "user" : "x"
                                             "host" : "y"
                                })));
    ASSERT_THROWS_MSG(proton::error, "expected string",
                      configure(opts, RAW_STRING({"scheme" : true})));
    ASSERT_THROWS_MSG(proton::error, "expected object",
                      configure(opts, RAW_STRING({"tls" : ""})));
    ASSERT_THROWS_MSG(proton::error, "expected object",
                      configure(opts, RAW_STRING({"sasl" : true})));
    ASSERT_THROWS_MSG(proton::error, "expected boolean",
                      configure(opts, RAW_STRING({"sasl" : {"enable" : ""}})));
    ASSERT_THROWS_MSG(proton::error, "expected boolean",
                      configure(opts, RAW_STRING({"tls" : {"verify" : ""}})));
}
...
class test_handler : public messaging_handler {
  ...
  public:
    ...
    string config_with_port(const string &bare_config) {
        ostringstream ss;
        ss << "{"
           << "\"port\":" << listener_.port() << ", " << bare_config << "}";
        return ss.str();
    }
...
class test_host_user_pass : public test_handler {
  public:
    void on_container_start(container &c) PN_CPP_OVERRIDE {
        test_handler::on_container_start(c);
        connect(c, RAW_STRING("scheme"
                              : "amqp", "host"
                              : "127.0.0.1", "user"
                              : "user@proton", "password"
                              : "password"));
    }
...
class test_tls_external : public test_handler {
    ...
    static connection_options make_opts() {
        ssl_certificate cert("testdata/certs/server-certificate-lh.pem",
                             ...
                             "server-password");
        connection_options opts;
        opts.ssl_server_options(ssl_server_options(
            cert, "testdata/certs/ca-certificate.pem",
            "testdata/certs/ca-certificate.pem", ssl::VERIFY_PEER));
        return opts;
    }
