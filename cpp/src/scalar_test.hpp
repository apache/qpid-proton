...
// conversion of scalar values via a proton::scalar or a proton::value.
...
void simple_type_test(T x, type_id tid, const std::string &s, T y) {
...
template <class V, class T> void simple_integral_test() {
    typedef typename internal::integer_type<
        sizeof(T), internal::is_signed<T>::value>::type int_type;
...
template <class V> void bad_get_test() {
    ...
    try {
        get<uint8_t>(V(true));
        FAIL("bool as uint8_t");
    } catch (const conversion_error &) {
    }
    try {
        get<uint8_t>(V(int8_t(1)));
        FAIL("int8 as uint8");
    } catch (const conversion_error &) {
    }
    try {
        get<int16_t>(V(uint16_t(1)));
        FAIL("uint16 as int16");
    } catch (const conversion_error &) {
    }
    try {
        get<int16_t>(V(int32_t(1)));
        FAIL("int32 as int16");
    } catch (const conversion_error &) {
    }
    try {
        get<symbol>(V(std::string()));
        FAIL("string as symbol");
    } catch (const conversion_error &) {
    }
    try {
        get<std::string>(V(binary()));
        FAIL("binary as string");
    } catch (const conversion_error &) {
    }
    try {
        get<binary>(V(symbol()));
        FAIL("symbol as binary");
    } catch (const conversion_error &) {
    }
    try {
        get<binary>(V(timestamp()));
        FAIL("timestamp as binary");
    } catch (const conversion_error &) {
    }
    try {
        get<int>(V(timestamp()));
        FAIL("timestamp as int");
    } catch (const conversion_error &) {
    }
    try {
        get<timestamp>(V(0));
        FAIL("int as timestamp");
    } catch (const conversion_error &) {
    }
    try {
        get<timestamp>(V(std::string()));
        FAIL("string as timestamp");
    } catch (const conversion_error &) {
    }
}
...
template <class V> void coerce_test() {
    ...
    try {
        coerce<double>(s);
        FAIL("string as double");
    } catch (const conversion_error &) {
    }

    try {
        coerce<std::string>(V(0));
        FAIL("int as string");
    } catch (const conversion_error &) {
    }
    try {
        coerce<symbol>(V(true));
        FAIL("bool as symbol");
    } catch (const conversion_error &) {
    }
    try {
        coerce<binary>(V(0.0));
        FAIL("double as binary");
    } catch (const conversion_error &) {
    }
    try {
        coerce<symbol>(V(binary()));
        FAIL("binary as symbol");
    } catch (const conversion_error &) {
    }
    try {
        coerce<binary>(V(symbol()));
        FAIL("symbol as binary");
    } catch (const conversion_error &) {
    }
    try {
...
template <class V> void null_test() {
    ...
    try {
        get<null>(v);
        FAIL("Expected conversion_error");
    } catch (const conversion_error &) {
    }
    v = null();
    get<null>(v);
...
template <class V> void scalar_test_group(int &failed) {
    ...
    RUN_TEST(failed,
             simple_type_test<V>(uint8_t(42), UBYTE, "42", uint8_t(50)));
    RUN_TEST(failed,
             simple_type_test<V>(int8_t(-42), BYTE, "-42", int8_t(-40)));
    RUN_TEST(failed, simple_type_test<V>(uint16_t(4242), USHORT, "4242",
                                         uint16_t(5252)));
    RUN_TEST(failed,
             simple_type_test<V>(int16_t(-4242), SHORT, "-4242", int16_t(3)));
    RUN_TEST(failed,
             simple_type_test<V>(uint32_t(4242), UINT, "4242", uint32_t(5252)));
    RUN_TEST(failed,
             simple_type_test<V>(int32_t(-4242), INT, "-4242", int32_t(3)));
    RUN_TEST(failed, simple_type_test<V>(uint64_t(4242), ULONG, "4242",
                                         uint64_t(5252)));
    RUN_TEST(failed,
             simple_type_test<V>(int64_t(-4242), LONG, "-4242", int64_t(3)));
    RUN_TEST(failed,
             simple_type_test<V>(wchar_t('X'), CHAR, "88", wchar_t('Y')));
    RUN_TEST(failed,
             simple_type_test<V>(float(1.234), FLOAT, "1.234", float(2.345)));
    RUN_TEST(failed, simple_type_test<V>(double(11.2233), DOUBLE, "11.2233",
                                         double(12)));
    RUN_TEST(failed, simple_type_test<V>(timestamp(1234), TIMESTAMP, "1234",
                                         timestamp(12345)));
    RUN_TEST(failed,
             simple_type_test<V>(make<decimal32>(1), DECIMAL32,
                                 "decimal32(0x01010101)", make<decimal32>(2)));
    RUN_TEST(failed, simple_type_test<V>(make<decimal64>(3), DECIMAL64,
                                         "decimal64(0x0303030303030303)",
                                         make<decimal64>(4)));
    RUN_TEST(failed, simple_type_test<V>(
                         make<decimal128>(5), DECIMAL128,
                         "decimal128(0x05050505050505050505050505050505)",
                         make<decimal128>(6)));
    RUN_TEST(failed, simple_type_test<V>(
                         uuid::copy("\x00\x11\x22\x33\x44\x55\x66\x77\x88\x99"
                                    "\xaa\xbb\xcc\xdd\xee\xff"),
                         UUID, "00112233-4455-6677-8899-aabbccddeeff",
                         uuid::copy("\xff\x11\x22\x33\x44\x55\x66\x77\x88\x99"
                                    "\xaa\xbb\xcc\xdd\xee\xff")));
    RUN_TEST(failed, simple_type_test<V>(std::string("xxx"), STRING, "xxx",
                                         std::string("yyy")));
    RUN_TEST(failed,
             simple_type_test<V>(symbol("aaa"), SYMBOL, "aaa", symbol("aaaa")));
    RUN_TEST(failed, simple_type_test<V>(binary("\010aaa"), BINARY,
                                         "b\"\\x08aaa\"", binary("aaaa")));
