...
template <class T> T check(T result) {
    if (result < 0) throw conversion_error(error_str(result));
    return result;
}
} // namespace
...
void decoder::decode(const char *i, size_t size) {
    ...
    while (i < end) i += check(pn_data_decode(pn_object(), i, size_t(end - i)));
}
...
decoder &decoder::extract(T &x, U (*get)(pn_data_t *)) {
    internal::state_guard sg(*this);
...
decoder &decoder::operator>>(start &s) {
    ...
    case ARRAY:
        ...
        s.element = type_id(pn_data_get_array_type(pn_object()));
        s.is_described = pn_data_is_array_described(pn_object());
        break;
    case LIST:
...
decoder &decoder::operator>>(internal::value_base &x) {
    if (*this == x.data_) throw conversion_error("extract into self");
    data d = x.data();
    d.clear();
    narrow();
    try {
...
decoder &decoder::operator>>(message_id &x) {
    ...
    if (got != ULONG && got != UUID && got != BINARY && got != STRING)
        throw conversion_error(
            msg() << "expected one of ulong, uuid, binary or string but found "
                  << got);
...
decoder &decoder::operator>>(annotation_key &x) {
    ...
    if (got != ULONG && got != SYMBOL)
        throw conversion_error(
            msg() << "expected one of ulong or symbol but found " << got);
    x.set(pn_data_get_atom(pn_object()));
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(uint8_t &x) {
    return extract(x, pn_data_get_ubyte);
}
...
decoder &decoder::operator>>(uint16_t &x) {
    ...
    switch (tid) {
    case UBYTE:
        x = pn_data_get_ubyte(pn_object());
        break;
    case USHORT:
        x = pn_data_get_ushort(pn_object());
        break;
    default:
        assert_type_equal(USHORT, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(int16_t &x) {
    ...
    switch (tid) {
    case BYTE:
        x = pn_data_get_byte(pn_object());
        break;
    case SHORT:
        x = pn_data_get_short(pn_object());
        break;
    default:
        assert_type_equal(SHORT, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(uint32_t &x) {
    ...
    case USHORT:
        ...
        break;
    case UINT:
        x = pn_data_get_uint(pn_object());
        break;
    default:
        assert_type_equal(UINT, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(int32_t &x) {
    ...
    case SHORT:
        ...
        break;
    case INT:
        x = pn_data_get_int(pn_object());
        break;
    default:
        assert_type_equal(INT, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(uint64_t &x) {
    ...
    case UINT:
        ...
        break;
    case ULONG:
        x = pn_data_get_ulong(pn_object());
        break;
    default:
        assert_type_equal(ULONG, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(int64_t &x) {
    ...
    case INT:
        ...
        break;
    case LONG:
        x = pn_data_get_long(pn_object());
        break;
    default:
        assert_type_equal(LONG, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(wchar_t &x) {
    return extract(x, pn_data_get_char);
}

decoder &decoder::operator>>(timestamp &x) {
    return extract(x, pn_data_get_timestamp);
}
...
decoder &decoder::operator>>(float &x) {
    ...
    switch (tid) {
    case FLOAT:
        x = pn_data_get_float(pn_object());
        break;
    case DOUBLE:
        x = float(pn_data_get_double(pn_object()));
        break;
    default:
        assert_type_equal(FLOAT, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(double &x) {
    ...
    switch (tid) {
    case FLOAT:
        x = static_cast<double>(pn_data_get_float(pn_object()));
        break;
    case DOUBLE:
        x = pn_data_get_double(pn_object());
        break;
    default:
        assert_type_equal(DOUBLE, tid);
    }
    sg.cancel();
    return *this;
}
...
decoder &decoder::operator>>(decimal32 &x) {
    return extract(x, pn_data_get_decimal32);
}
decoder &decoder::operator>>(decimal64 &x) {
    return extract(x, pn_data_get_decimal64);
}
decoder &decoder::operator>>(decimal128 &x) {
    return extract(x, pn_data_get_decimal128);
}

decoder &decoder::operator>>(uuid &x) { return extract(x, pn_data_get_uuid); }
decoder &decoder::operator>>(binary &x) {
    return extract(x, pn_data_get_binary);
}
decoder &decoder::operator>>(symbol &x) {
    return extract(x, pn_data_get_symbol);
}
...
decoder &decoder::operator>>(std::string &x) {
    ...
    switch (tid) {
    case STRING:
        x = str(pn_data_get_string(pn_object()));
        break;
    case SYMBOL:
        x = str(pn_data_get_symbol(pn_object()));
        break;
    default:
        assert_type_equal(STRING, tid);
    }
    sg.cancel();
    return *this;
}
