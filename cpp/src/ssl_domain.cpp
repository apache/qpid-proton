...
/// refcounted, with shared ownership for all pn_ssl_t objects that hold one
...
class ssl_domain_impl {
  public:
    ssl_domain_impl(bool is_server)
        : refcount_(1),
          pn_domain_(pn_ssl_domain(is_server ? PN_SSL_MODE_SERVER
                                             : PN_SSL_MODE_CLIENT)) {
...
ssl_domain::ssl_domain(const ssl_domain &x)
    : impl_(x.impl_), server_type_(x.server_type_) {
...
void set_cred(pn_ssl_domain_t *dom, const std::string &main,
              const std::string &extra, const std::string &pass, bool pwset) {
    ...
    if (pn_ssl_domain_set_credentials(dom, main.c_str(), cred2, pw))
        throw error(MSG("SSL certificate initialization failure for "
                        << main << ":" << (cred2 ? cred2 : "NULL") << ":"
                        << (pw ? pw : "NULL")));
}
} // namespace
...
ssl_server_options::ssl_server_options(ssl_certificate &cert,
    ...
    if (pn_ssl_domain_set_trusted_ca_db(dom, trust_db.c_str()))
        throw error(
            MSG("SSL trust store initialization failure for " << trust_db));
...
namespace {
void client_setup(pn_ssl_domain_t *dom, const std::string &trust_db,
                  enum ssl::verify_mode mode) {
    if (pn_ssl_domain_set_trusted_ca_db(dom, trust_db.c_str()))
        throw error(
            MSG("SSL trust store initialization failure for " << trust_db));
...
ssl_client_options::ssl_client_options(const std::string &trust_db,
                                       enum ssl::verify_mode mode)
    : ssl_domain(false) {
    client_setup(pn_domain(), trust_db, mode);
}

ssl_client_options::ssl_client_options(ssl_certificate &cert,
                                       const std::string &trust_db,
                                       enum ssl::verify_mode mode)
    : ssl_domain(false) {
    pn_ssl_domain_t *dom = pn_domain();
