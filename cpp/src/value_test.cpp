...
template <class T>
void sequence_test(type_id tid, const many<typename T::value_type> &values,
                   const string &s) {
    ...
    *y.begin() = *(++it); // Second element is bigger so y is lexicographically
                          // bigger than x
    value vy(y);
    ASSERT(vx != vy);
    ASSERT(vx < vy);
...
template <class T, class U> void map_test(const U &values, const string &s) {
    ...
    ASSERT_EQUAL(m, m2);
    if (!s.empty()) ASSERT_EQUAL(s, to_string(v));
}
...
int main(int, char **) {
    try {
        ...
        // Sequence tests
        RUN_TEST(failed,
                 sequence_test<list<bool>>(ARRAY, many<bool>() + false + true,
                                           "@PN_BOOL[false, true]"));
        RUN_TEST(failed, sequence_test<vector<int>>(ARRAY, many<int>() + -1 + 2,
                                                    "@PN_INT[-1, 2]"));
        RUN_TEST(failed,
                 sequence_test<deque<string>>(ARRAY, many<string>() + "a" + "b",
                                              "@PN_STRING[\"a\", \"b\"]"));
        RUN_TEST(failed,
                 sequence_test<deque<symbol>>(ARRAY, many<symbol>() + "a" + "b",
                                              "@PN_SYMBOL[:a, :b]"));
        RUN_TEST(failed, sequence_test<vector<value>>(
                             LIST, many<value>() + value(0) + value("a"),
                             "[0, \"a\"]"));
        RUN_TEST(failed, sequence_test<vector<scalar>>(
                             LIST, many<scalar>() + scalar(0) + scalar("a"),
                             "[0, \"a\"]"));
...
#if PN_CPP_HAS_CPP11
        RUN_TEST(failed,
                 sequence_test<forward_list<binary>>(
                     ARRAY, many<binary>() + binary("xx") + binary("yy"),
                     "@PN_BINARY[b\"xx\", b\"yy\"]"));
        RUN_TEST(failed,
                 (map_test<unordered_map<string, uint64_t>>(si_pairs, "")));
#endif
        return failed;
