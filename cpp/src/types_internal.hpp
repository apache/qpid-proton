...
typename internal::enable_if<sizeof(T) == sizeof(U)>::type
byte_copy(T &to, const U &from) {
    const char *p = reinterpret_cast<const char *>(&from);
    std::copy(p, p + sizeof(T), reinterpret_cast<char *>(&to));
}

inline conversion_error
make_conversion_error(type_id want, type_id got,
...
inline pn_bytes_t pn_bytes(const binary &s) {
    pn_bytes_t b = {s.size(),
                    s.empty() ? 0 : reinterpret_cast<const char *>(&s[0])};
    return b;
}

inline std::string str(const pn_bytes_t &b) {
    return std::string(b.start, b.size);
}
inline binary bin(const pn_bytes_t &b) {
    return binary(b.start, b.start + b.size);
}
...
// Convert a char (signed or unsigned) into an unsigned 1 byte integer that will
// ostream as a numeric byte value, not a character and will not get
// sign-extended.
