...
/** Print the error string from pn_error_t, or from code if pn_error_t has no
 * error. */
...
template <class T> class factory {
  public:
    ...
    static typename wrapped<T>::type *unwrap(const T &t) {
        return t.pn_object();
    }
};
...
template <class T>
inline void set_messaging_handler(T t, messaging_handler *mh) {
    context<T>::type::get(factory<T>::unwrap(t)).handler = mh;
}

template <class T> inline messaging_handler *get_messaging_handler(T *t) {
    return context<typename internal::wrapper<T>::type>::type::get(t).handler;
}

class returned_factory {
  public:
    template <class T>
    static returned<T> make(typename internal::wrapped<T>::type *pn) {
...
template <class T> typename internal::wrapper<T>::type make_wrapper(T *t) {
    return internal::factory<typename internal::wrapper<T>::type>::wrap(t);
}

template <class U> U make_wrapper(typename internal::wrapped<U>::type *t) {
    return internal::factory<U>::wrap(t);
}

template <class T> typename internal::wrapped<T>::type *unwrap(const T &t) {
    return internal::factory<T>::unwrap(t);
}

template <class T>
returned<T> make_returned(typename internal::wrapped<T>::type *pn) {
    return internal::returned_factory::make<T>(pn);
}

// Get an AMQP "multiple" field from a value. A "multiple" field can be encoded
// as a single value or as an array. This function always extracts it as a
// sequence, a sequence of one if it is encoded as a single value.
//
// T should be a valid sequence type for proton::get() with a T::value_type
// typedef.
//
template <class T> void get_multiple(const value &v, T &x) {
    if (v.empty()) {
        x.clear();
...
template <class T> T get_multiple(const value &v) {
    T x;
    get_multiple(v, x);
    return x;
}
