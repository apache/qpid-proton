...
class container::impl {
  public:
    ...
    returned<connection> connect(const std::string &,
                                 const connection_options &);
    returned<sender> open_sender(const std::string &,
                                 const proton::sender_options &,
                                 const connection_options &);
    returned<receiver> open_receiver(const std::string &,
  ...
  private:
    ...
    pn_connection_t *make_connection_lh(const url &url,
                                        const connection_options &);
...
template <class T> messaging_handler *container::impl::get_handler(T s) {
