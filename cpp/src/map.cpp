...
template <class K, class T> class map_type_impl : public std::map<K, T> {};

template <class K, class T> map<K, T>::map() {}

template <class K, class T> map<K, T>::map(const map &x) { *this = x; }

template <class K, class T> map<K, T>::map(pn_data_t *d) : value_(d) {
    // NOTE: for internal use. Don't verify that the data is valid here as that
    // would forcibly decode message maps immediately, we want to decode
    // on-demand.
}
...
template <class K, class T> map<K, T> &map<K, T>::operator=(const map &x) {
    if (&x != this) {
...
template <class K, class T> map<K, T> &map<K, T>::operator=(map &&x) {
    if (&x != this) {
...
template <class K, class T> value &map<K, T>::flush() const {
    if (map_.get()) {
        value_ = *map_;
        map_.reset();
...
template <class K, class T> void map<K, T>::value(const proton::value &x) {
    if (x.empty()) {
        clear();
    } else {
...
template <class K, class T> proton::value &map<K, T>::value() {
    return flush();
}

template <class K, class T> const proton::value &map<K, T>::value() const {
    return flush();
}

template <class K, class T> T map<K, T>::get(const K &k) const {
...
template <class K, class T> void map<K, T>::put(const K &k, const T &v) {
    cache()[k] = v;
}

template <class K, class T> size_t map<K, T>::erase(const K &k) {
    if (this->empty()) {
        return 0;
    } else {
...
template <class K, class T> bool map<K, T>::exists(const K &k) const {
    return this->empty() ? 0 : cache().find(k) != cache().end();
}

template <class K, class T> size_t map<K, T>::size() const {
...
template <class K, class T> bool map<K, T>::empty() const {
    if (map_.get()) {
        return map_->empty();
    }
    if (value_.empty()) {
...
template <class K, class T> void map<K, T>::reset(pn_data_t *d) {
...
// Force the necessary template instantiations so that the library exports the
// correct symbols
...
template PN_CPP_EXTERN proton::codec::decoder &operator>>
    <>(proton::codec::decoder &d, cm1 &m);
template PN_CPP_EXTERN proton::codec::encoder &
operator<<<>(proton::codec::encoder &e, const cm1 &m);
...
template PN_CPP_EXTERN proton::codec::decoder &operator>>
    <>(proton::codec::decoder &d, cm2 &m);
template PN_CPP_EXTERN proton::codec::encoder &
operator<<<>(proton::codec::encoder &e, const cm2 &m);

template class PN_CPP_CLASS_EXTERN map<symbol, value>;
typedef map<symbol, value> cm3;
template PN_CPP_EXTERN void swap<>(cm3 &, cm3 &);
template PN_CPP_EXTERN proton::codec::decoder &operator>>
    <>(proton::codec::decoder &d, cm3 &m);
template PN_CPP_EXTERN proton::codec::encoder &
operator<<<>(proton::codec::encoder &e, const cm3 &m);
