...
class container::impl::common_work_queue : public work_queue::impl {
  public:
    common_work_queue(container::impl &c)
        : container_(c), finished_(false), running_(false) {}
...
void container::impl::common_work_queue::schedule(duration d, work f) {
    ...
    if (finished_) return;
    container_.schedule(
        d, make_work(&work_queue::impl::add_void, (work_queue::impl *)this, f));
}
...
class container::impl::connection_work_queue : public common_work_queue {
  public:
    connection_work_queue(container::impl &ct, pn_connection_t *c)
        : common_work_queue(ct), connection_(c) {}
...
container::impl::impl(container &c, const std::string &id,
                      messaging_handler *mh)
    : threads_(0), container_(c), proactor_(pn_proactor()), handler_(mh),
      id_(id), reconnecting_(0), auto_stop_(true), stopping_(false) {}

container::impl::~impl() { pn_proactor_free(proactor_); }
...
void container::impl::remove_work_queue(
    container::impl::container_work_queue *l) {
    GUARD(work_queues_lock_);
    work_queues_.erase(l);
}

void container::impl::setup_connection_lh(const url &url,
                                          pn_connection_t *pnc) {
    pn_connection_set_container(pnc, id_.c_str());
    pn_connection_set_hostname(pnc, url.host().c_str());
    if (!url.user().empty()) pn_connection_set_user(pnc, url.user().c_str());
    if (!url.password().empty())
        pn_connection_set_password(pnc, url.password().c_str());
}

pn_connection_t *
container::impl::make_connection_lh(const url &url,
                                    const connection_options &user_opts) {
    if (stopping_) throw proton::error("container is stopping");
    ...
    cc.handler = mh;
    cc.work_queue_ =
        new container::impl::connection_work_queue(*container_.impl_, pnc);
...
//
// NOTE: After the call to start_connection() pnc is active in a proactor
// thread, and may even have been freed already. It is undefined to use pnc (or
// any object belonging to it) except in appropriate handlers.
//
...
void container::impl::start_connection(const url &url, pn_connection_t *pnc) {
    ...
    pn_proactor_connect2(proactor_, pnc, pnt,
                         caddr); // Takes ownership of pnc, pnt
}
...
void container::impl::reconnect(pn_connection_t *pnc) {
    ...
    // rc.current_url_ == -1 means try the url specified in connect, not a
    // failover url
    const proton::url url(rc.current_url_ == -1
                              ? cc.connected_address_
                              : roi.failover_urls[rc.current_url_]);

    // XXXX Debug:
    // std::cout << "Retries: " << rc.retries_ << " Delay: " << rc.delay_ << "
    // Trying: " << url << std::endl;
...
duration random_between(duration min, duration max) {
    ...
    std::uniform_int_distribution<duration::numeric_type> dist{
        min.milliseconds(), max.milliseconds()};
    return duration(dist(gen));
}
#else
duration random_between(duration, duration max) { return max; }
#endif
} // namespace
...
bool container::impl::can_reconnect(pn_connection_t *pnc) {
    ...
    if (!strcmp(pn_condition_get_name(condition), "amqp:unauthorized-access"))
        return false;

    // If too many reconnect attempts just fail
    if (roi.max_attempts != 0 && rc->retries_ >= roi.max_attempts) {
        pn_condition_format(condition, "proton:io",
                            "Too many reconnect attempts (%d)", rc->retries_);
        return false;
    }
    return true;
}
...
void container::impl::setup_reconnect(pn_connection_t *pnc) {
    ...
    // have the connection now anyway
    schedule(delay, make_work(&container::impl::reconnect, this, pnc));
    ++reconnecting_;
}

returned<connection>
container::impl::connect(const std::string &addr,
...
returned<sender> container::impl::open_sender(const std::string &urlstr,
                                              const proton::sender_options &o1,
                                              const connection_options &o2) {
...
returned<receiver>
container::impl::open_receiver(const std::string &urlstr,
                               const proton::receiver_options &o1,
                               const connection_options &o2) {
    proton::url url(urlstr);
    pn_link_t *pnl = 0;
    pn_connection_t *pnc = 0;
...
pn_listener_t *container::impl::listen_common_lh(const std::string &addr) {
    ...
    proton::url url(addr,
                    false); // Don't want un-helpful defaults like "localhost"
    ...
    pn_proactor_addr(&caddr[0], len + 1, url.host().c_str(),
                     url.port().c_str());
...
proton::listener
container::impl::listen(const std::string &addr,
...
void container::impl::schedule(duration delay, work f) {
    ...
    pn_millis_t timeout_ms =
        (now < next->time) ? (next->time - now).milliseconds() : 0;
    pn_proactor_set_timeout(proactor_, timeout_ms);
}

void container::impl::client_connection_options(
    const connection_options &opts) {
    GUARD(lock_);
    client_connection_options_ = opts;
}

void container::impl::server_connection_options(
    const connection_options &opts) {
    GUARD(lock_);
...
void container::impl::run_timer_jobs() {
    ...
    // avoid having tasks injected as we are running them (which could
    // potentially never end)
    {
        GUARD(deferred_lock_);

        // Figure out how many tasks we need to execute and pop them to the back
        // of the queue (in reverse order)
        unsigned i = 0;
        for (;;) {
        ...
        if (deferred_.size() == i) {
            ...
            tasks =
                std::vector<scheduled>(deferred_.end() - i, deferred_.end());
...
container::impl::dispatch_result container::impl::dispatch(pn_event_t *event) {
    ...
    case PN_PROACTOR_TIMEOUT: {
        // Can get an immediate timeout, if we have a container event loop
        // inject
        run_timer_jobs();

        // Run every container event loop job
        // This is not at all efficient and single threads all these jobs, but
        // it does correctly serialise them
        work_queues queues;
        {
            GUARD(work_queues_lock_);
            queues = work_queues_;
        }
        for (work_queues::iterator queue = queues.begin();
             queue != queues.end(); ++queue) {
            (*queue)->run_all_jobs();
        }
        return EndBatch;
    }
    ...
    case PN_LISTENER_ACCEPT: {
        ...
        cc.work_queue_ =
            new container::impl::connection_work_queue(*container_.impl_, c);
        pn_transport_t *pnt = pn_transport();
    ...
    // Connection driver will bind a new transport to the connection at this
    // point
    ...
    case PN_CONNECTION_REMOTE_OPEN: {
        ...
        // succeeded so it's the only place to reset the reconnection logic.
        //
...
void container::impl::thread() {
    ...
    while (!finished) {
        ...
        try {
            ...
            // If we caught an exception then shutdown the (other threads of
            // the) container
            error = error_condition("exception", e.what());
        } catch (...) {
            error = error_condition("exception",
                                    "container shut-down by unknown exception");
        }
...
void container::impl::run(int threads) {
    ...
    typedef std::vector<std::thread *>
        vt; // pointer vector to work around failures in older compilers
    vt ts(threads - 1);
    ...
    // Throw an exception if we disconnected the proactor because of an
    // exception
    {
        GUARD(lock_);
