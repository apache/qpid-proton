...
struct test_listen_handler : public proton::listen_handler {
    ...
    test_listen_handler(
        ...
        : on_open_(false), on_accept_(false), on_close_(false), host_(host),
          opts_(opts) {}
...
class test_handler : public proton::messaging_handler {
  public:
    ...
    void on_connection_open(proton::connection &c) PN_CPP_OVERRIDE {
        ...
        // First call is the incoming server-side of the connection, that we are
        // interested in. Second call is for the response to the client, ignore
        // that.
        if (!closing) {
...
class immediate_stop_tester : public proton::messaging_handler {
  public:
    void on_container_start(proton::container &c) PN_CPP_OVERRIDE { c.stop(); }
};
...
class test_mt_handler : public proton::messaging_handler {
  public:
    ...
    void on_error(const proton::error_condition &e) PN_CPP_OVERRIDE {
        err_ = e;
    }
};
