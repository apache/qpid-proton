...
class scalar_base : private internal::comparable<scalar_base> {
  public:
    ...
    /// Compare
    friend PN_CPP_EXTERN bool operator<(const scalar_base &x,
                                        const scalar_base &y);
    /// Compare
    friend PN_CPP_EXTERN bool operator==(const scalar_base &x,
                                         const scalar_base &y);
    /// Print the contained value
    friend PN_CPP_EXTERN std::ostream &operator<<(std::ostream &o,
                                                  const scalar_base &x);
  ...
  private:
    ...
    template <class T>
    struct putter<T, typename internal::enable_if<
                         internal::is_unknown_integer<T>::value>::type> {
    ...
    template <class T>
    struct getter<T, typename internal::enable_if<
                         internal::is_unknown_integer<T>::value>::type> {
        static T get(const scalar_base &s) {
            typename internal::known_integer<T>::type x;
            s.get_(x);
            return x;
        }
    };
...
template <class R, class F> R visit(const scalar_base &s, F f) {
    ...
    default:
        throw conversion_error("invalid scalar type " + type_name(s.type()));
    }
}

PN_CPP_EXTERN conversion_error make_coercion_error(const char *cpp_type,
                                                   type_id amqp_type);

template <class T> struct coerce_op {
    template <class U>
    typename enable_if<is_convertible<U, T>::value, T>::type
    operator()(const U &x) {
...
template <class T> T coerce(const scalar_base &s) {
