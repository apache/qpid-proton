...
// Separate value data from implicit conversion constructors to avoid template
// recursion.
class value_base {
  protected:
    internal::data &data();
...
class value : public internal::value_base, private internal::comparable<value> {
  ...
  public:
    ...
    template <class T> value(const T &x, typename assignable<T>::type * = 0) {
        *this = x;
    }

    /// Assign from any allowed type T.
    template <class T>
    typename assignable<T, value &>::type operator=(const T &x) {
    ...
    /// @cond INTERNAL
    template <class T>
    PN_CPP_DEPRECATED("Use 'proton::get'")
    void get(T &t) const;
...
///
/// @relatedalso proton::value
template <class T> void get(const value &v, T &x) {
    codec::decoder d(v, true);
    d >> x;
}

/// @relatedalso proton::value
template <class T, class U> inline void get(const U &u, T &x) {
    const value v(u);
    get(v, x);
}
...
/// @relatedalso proton::value
template <class T> T coerce(const value &v) {
    T x;
    coerce(v, x);
    return x;
}
...
/// @copybrief get<null>()
template <>
inline void get<decltype(nullptr)>(const value &v, decltype(nullptr) &) {
    assert_type_equal(NULL_TYPE, v.type());
}
#endif
