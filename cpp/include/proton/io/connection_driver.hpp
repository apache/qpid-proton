...
/// events to a proton::messaging_handler. It does no IO of its own, but allows
/// you to integrate AMQP protocol handling into any IO or concurrency
/// framework.
///
...
class PN_CPP_CLASS_EXTERN connection_driver {
  public:
    ...
    /// Configure a connection by applying exactly the options in opts
    /// (including proton::messaging_handler) Does not apply any default
    /// options, to apply container defaults use connect() or accept() instead.
    ...
    /// Options applied: container::id(),
    /// container::client_connection_options(), opts.
    PN_CPP_EXTERN void connect(const connection_options &opts);

    /// Call configure() with server options.
    /// Options applied: container::id(),
    /// container::server_connection_options(), opts.
    ///
    /// Note this does not call connection::open(). If there is a
    /// messaging_handler in the composed options it will receive
    /// messaging_handler::on_connection_open() and can respond with
    /// connection::open() or connection::close()
    PN_CPP_EXTERN void accept(const connection_options &opts);

    /// The engine's read buffer. Read data into this buffer then call
    /// read_done() when complete. Returns mutable_buffer(0, 0) if the engine
    /// cannot currently read data. Calling dispatch() may open up more buffer
    /// space.
    ...
    /// Indicate that the read side of the transport is closed and no more data
    /// will be read. Note that there may still be events to dispatch() or data
    /// to write.
    PN_CPP_EXTERN void read_close();

    /// The engine's write buffer. Write data from this buffer then call
    /// write_done() Returns const_buffer(0, 0) if the engine has nothing to
    /// write. Calling dispatch() may generate more data in the write buffer.
    PN_CPP_EXTERN const_buffer write_buffer();

    /// Indicate that the first n bytes of write_buffer() have been written
    /// successfully. This changes the buffer, call write_buffer() to get the
    /// updated buffer.
    PN_CPP_EXTERN void write_done(size_t n);

    /// Indicate that the write side of the transport has closed and no more
    /// data can be written. Note that there may still be events to dispatch()
    /// or data to read.
    ...
    /// arrange to call tick() no later than this expiration time. In practice
    /// this will mean calling tick() every time there is anything read or
    /// written, and if nothing is read or written then as soon as possible
    /// after the returned timestamp (so you will probably need to set a
    /// platform specific timeout to know when this occurs).
    ...
    /// time to dispatch the messaging_handler::on_transport_error() call and
    /// other final events.
    ///
    /// Note this does not close the connection() so that a
    /// proton::messaging_handler can distinguish between a connection close
    /// error sent by the remote peer and a transport failure.
    ///
    PN_CPP_EXTERN void
    disconnected(const error_condition & = error_condition());

    /// There are events to be dispatched by dispatch()
    PN_CPP_EXTERN bool has_events() const;

    /// Dispatch all available events and call the corresponding \ref
    /// messaging_handler methods.
    ///
    ...
    ///
    /// - both read_close() and write_close() have been called, no more IO is
    /// possible.
    ...
    /// Get the container associated with this connection_driver, if there is
    /// one.
