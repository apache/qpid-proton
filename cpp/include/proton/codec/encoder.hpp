...
class encoder : public internal::data {
  public:
    ...
    template <class T> struct list_cref {
        T &ref;
        list_cref(T &r) : ref(r) {}
    };
    template <class T> struct map_cref {
        T &ref;
        map_cref(T &r) : ref(r) {}
    };
    ...
    template <class T> struct array_cref {
        start array_start;
        T &ref;
        array_cref(T &r, type_id el, bool described)
            : array_start(ARRAY, el, described), ref(r) {}
    };

    template <class T> static list_cref<T> list(T &x) {
        return list_cref<T>(x);
    }
    template <class T> static map_cref<T> map(T &x) { return map_cref<T>(x); }
    template <class T>
    static array_cref<T> array(T &x, type_id element, bool described = false) {
  ...
  private:
    ...
    encoder &insert(const T &x, int (*put)(pn_data_t *, U));
    void check(long result);
};

/// Treat char* as string
inline encoder &operator<<(encoder &e, const char *s) {
...
operator<<(encoder &e, T i) {
    ...
    return e << static_cast<
               typename integer_type<sizeof(T), is_signed<T>::value>::type>(i);
}
...
struct is_encodable_map<
    ...
    typename internal::enable_if<
        ...
        is_encodable<M>::value>::type> : public internal::true_type {};
